...
// Exact prelaunch-only recovery delta; the launch opportunity and previous
// source/SQL audits are preserved, never generalized to arbitrary changes.
export const MAINTENANCE_PRELAUNCH_RECOVERY_SOURCE_PATHS = Object.freeze([
  ...
  "scripts/production-maintenance-prelaunch-recovery.mjs",
  "scripts/production-maintenance-prelaunch-recovery.test.mjs",
  "scripts/production-maintenance-prelaunch-recovery-evidence.mjs",
  "scripts/production-maintenance-prelaunch-recovery-evidence.test.mjs",
  "scripts/production-maintenance-prelaunch-recovery-workflow.mjs",
  "scripts/production-maintenance-prelaunch-recovery-workflow.test.mjs",
  "scripts/production-maintenance-prelaunch-inspection.mjs",
  "scripts/production-maintenance-prelaunch-inspection.test.mjs",
  "scripts/production-maintenance-prelaunch-handoff.mjs",
  "scripts/production-maintenance-prelaunch-handoff.test.mjs",
  "scripts/production-maintenance-prelaunch-control.test.mjs",
  "scripts/production-maintenance-prelaunch-reclose.test.mjs",
  "scripts/production-maintenance-prelaunch-disk.mjs",
  "scripts/production-maintenance-prelaunch-disk.test.mjs",
  "scripts/test-helpers/maintenance-prelaunch-fixture.mjs",
  "scripts/deploy.production.sh",
  "scripts/deploy-production-contract.test.mjs",
  "scripts/production-maintenance-workflow-contract.test.mjs",
  "scripts/production-maintenance-prelaunch-workflow-contract.test.mjs"
]);
export const MAINTENANCE_PRELAUNCH_RECOVERY_MIGRATION_WINDOW = Object.freeze({
  runId: "34721155156", attempt: 1, startedAt: "2026-09-12T21:52:38.000Z", endedBefore: "2026-09-12T21:52:46.000Z",
});
const PREVIOUS = "67ddb91bf618e9716b79df6bf97f08d3865919b7", CREATED_AT = 1789236034129;
...
const fail = () => { throw new Error("maintenance_prelaunch_recovery_evidence_unverified"); };
...
export function readMaintenancePrelaunchRecoverySourceProof(request, executeFixed = runGit) {
  try {
    ...
    if (!fields.length || fields.length % 2 || fields.length > MAINTENANCE_PRELAUNCH_RECOVERY_SOURCE_PATHS.length * 2) fail();
    for (let index = 0; index < fields.length; index += 2) {
      const match = /^:(000000|100644|100755) (100644|100755) ([0-9a-f]{40}) ([0-9a-f]{40}) ([AM])$/.exec(fields[index]), file = fields[index + 1];
      if (!match || !MAINTENANCE_PRELAUNCH_RECOVERY_SOURCE_PATHS.includes(file) || changes.some(value => value.path === file)) fail();
...
export const MAINTENANCE_PRELAUNCH_RECOVERY_MIGRATION_SQL = "BEGIN READ ONLY; SET LOCAL search_path=pg_catalog; SET LOCAL timezone='UTC'; " +
...
export function validateMaintenancePrelaunchRecoveryMigrationProof(value, databaseOid, createdAt, request, executeFixed = runGit) {
  try {
    ...
    const start = BigInt(Date.parse(MAINTENANCE_PRELAUNCH_RECOVERY_MIGRATION_WINDOW.startedAt)) * 1000n;
    const end = BigInt(Date.parse(MAINTENANCE_PRELAUNCH_RECOVERY_MIGRATION_WINDOW.endedBefore)) * 1000n;
