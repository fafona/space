...
const confirmation = "RECOVER_ATTEMPT_PRODUCTION_MAINTENANCE_UNTIL_20260914T040000Z";
const env = {
  GITHUB_REPOSITORY: "fafona/space", GITHUB_EVENT_NAME: "workflow_dispatch", GITHUB_REF: "refs/heads/main", GITHUB_RUN_ATTEMPT: "1",
  ACTION: "recover-attempt", CONFIRMATION: confirmation, TARGET_SHA: "a".repeat(40), GITHUB_SHA: "a".repeat(40),
  MAINTENANCE_OPERATION_ID: "eb81284a-09c4-4514-8f16-38eaf6acc1e4",
  PREVIOUS_TARGET_SHA: "f3104de19aa59e527c7b94a99850d151448da8cd", EXPECTED_OLD_SHA: "cd943076ebda758b70bf2f2270a508c774b726d6",
...
test("one fixed additional attempt requires exact manual authorization and current main", () => {
  ...
  for (const overrides of [
    ...
    { MAINTENANCE_OPERATION_ID: "ab81284a-09c4-4514-8f16-38eaf6acc1e4" }, { DEPLOY_RUN_ID: "34781392661" },
...
test("historical signed artifacts precede fresh private baseline and fully locked history", () => {
  const names = ["Verify Launched Incident Signed Backup And Readiness Bindings", "Verify Attempt Recovery Historical Additional Backup",
    "Inspect Stopped Launched Candidate Recovery State", "Verify Exact Single Attempt Recovery History Under Production Lock", "Execute Fixed Maintenance Transition"];
  const positions = names.map(name => steps.indexOf(step(name)));
  assert.deepEqual([...positions].sort((a, b) => a - b), positions);
  for (const name of names.slice(0, 4)) assert.equal(step(name).if, "inputs.action == 'recover-attempt'");
  const bindings = step(names[0]).run;
  for (const value of ["34778579797", "34781336277", "gh attestation verify", '--source-digest "$PREVIOUS_TARGET_SHA"',
  ...
  assert.match(historical, /validateMaintenanceAttemptRecoveryAdditionalBackup\(readMaintenanceAttemptRecoveryAdditionalBackup\(process\.argv\[2\]\), Date\.now\(\)\)/);
  const inspection = step(names[2]).run;
  assert.match(inspection, /umask 077/);
  assert.match(inspection, /inspect-attempt-recovery .*--target-sha .*--previous-target-sha .*--expected-old-sha .*--expected-operation-id/);
  assert.match(inspection, /> "\$capture_dir\/out" 2> "\$capture_dir\/err"/);
  assert.doesNotMatch(inspection, /cat |tee |recover-attempt --|fail-held/);
  const history = step(names[3]);
  assert.equal(history.env.INSPECTION_DIR, "${{ steps.attempt-recovery-inspection.outputs.capture_dir }}");
  assert.equal(history.env.PRIOR_BINDINGS_DIR, "${{ steps.attempt-recovery-prior-bindings.outputs.capture_dir }}");
  assert.equal(history.env.ADDITIONAL_BACKUP_DIR, "${{ steps.attempt-recovery-additional-backup.outputs.capture_dir }}");
  assert.match(history.run, /node scripts\/production-maintenance-attempt-recovery-workflow\.mjs/);
});
...
test("only the fixed transition receives the private attempt grant; end retains independent gates", () => {
  ...
  assert.equal(transition.env.ATTEMPT_RECOVERY_EVIDENCE, "${{ steps.attempt-recovery-evidence.outputs.attempt_recovery_evidence }}");
  assert.match(transition.run, /recover-attempt\) command=recover-attempt; expected_state=held/);
  assert.match(transition.run, /test "\$\{#ATTEMPT_RECOVERY_EVIDENCE\}" -le 16384/);
  assert.match(transition.run, /--previous-target-sha "\$PREVIOUS_TARGET_SHA" --attempt-recovery-evidence "\$ATTEMPT_RECOVERY_EVIDENCE"/);
  assert.match(transition.run, /else\n\s+test -z "\$ATTEMPT_RECOVERY_EVIDENCE"/);
...
test("attempt cleanup stays within its three exact private runner directories", () => {
  const cleanup = step("Remove Runner Attempt Recovery Evidence");
  assert.equal(cleanup.if, "always() && inputs.action == 'recover-attempt'");
  assert.deepEqual(Object.keys(cleanup.env), ["INSPECTION_DIR", "PRIOR_BINDINGS_DIR", "ADDITIONAL_BACKUP_DIR"]);
  for (const prefix of ["maintenance-attempt-recovery", "maintenance-attempt-prior-bindings", "maintenance-attempt-additional-backup"])
    assert.ok(cleanup.run.includes('"$RUNNER_TEMP"/' + prefix + '.????????'));
  assert.doesNotMatch(cleanup.run, /rm -rf|sudo|ssh |state\.json|maintenance\.json|fail-held|recover-attempt --/);
...
test("every new embedded shell block parses without running a deployment", () => {
  for (const item of steps.filter(value => value.if?.includes("recover-attempt") || value.name === "Execute Fixed Maintenance Transition" || value.name === "Validate Fixed Manual Transition")) {
