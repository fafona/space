...
import { validateWindowRenewalHandoffReport } from "./production-maintenance-window-handoff.mjs";
...
function fixture() {
  ...
  const stoppedBaseline = { version: 3, stateDigest: pin, candidateDigest: "a".repeat(64), launchDiskDigest: "b".repeat(64),
    ...
    bootId: "e6531ec9-db4a-4216-b87a-7cc858197eaa", pm2RegistryDigest: "e".repeat(64), observedAt: Date.parse("2026-09-14T13:00:00Z") };
...
const rejects = (value, bound = request) => assert.throws(() => validateWindowRenewalHandoffReport(value, bound), { message: "maintenance_window_handoff_unverified" });
test("small report retains fixed T7 physical baseline without complete predecessors", () => {
  const value = fixture(), result = validateWindowRenewalHandoffReport(value, request);
...
test("even a matching request cannot select any historical target as the new attempt", () => {
  for (const targetSha of [T7, "3af8fa6ba6644593e10bef0a391389b2b34e926a", request.expectedOldSha, "f3104de19aa59e527c7b94a99850d151448da8cd",
    ...
    "78ca8104442172baf046b62f9184cf9c7f6a9279", "3b6c55ea4397c4505e75c59bb3bb56d7dd7d1cc0"]) {
...
test("fresh window report retains the original reader shape but rejects old or future baseline clocks", async () => {
  ...
  const value = fixture();
  assert.deepEqual(validateWindowRenewalHandoffReport(value, request), validateBudgetRecoveryHandoffReport(value, request));
  for (const time of ["2026-09-14T09:07:00Z", "2026-09-14T12:31:03.999Z", "2026-09-14T16:00:00Z"]) {
