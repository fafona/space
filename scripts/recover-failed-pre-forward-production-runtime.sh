...
readonly EXPECTED_INCIDENT_DEPLOY_RUN_ID="32657417536"
readonly EXPECTED_INCIDENT_SHA="f7ef0c0793be2c41d981fabae5ee44af35654904"
readonly EXPECTED_INCIDENT_READINESS_RUN_ID="32657364781"
readonly EXPECTED_INCIDENT_READINESS_RUN_ATTEMPT="1"
readonly EXPECTED_OLD_BUILD_ID="50a54b0bb04ca5c34b5ded2bd4419dae6701501a"
readonly EXPECTED_OLD_RELEASE_PREFIX="${EXPECTED_OLD_BUILD_ID:0:12}"
readonly EXPECTED_CONFIRMATION="RECOVER_FAILED_PRE_FORWARD_DEPLOY_32657417536"
...
[[ "$EXPECTED_OLD_RELEASE_PREFIX" =~ ^[0-9a-f]{12}$ ]] || exit 1
[[ "$PREVIOUS_RELEASE_NAME" =~ ^([0-9a-f]{12})-([0-9]{14})$ ]] || exit 1
[ "${BASH_REMATCH[1]}" = "$EXPECTED_OLD_RELEASE_PREFIX" ] || exit 1
...
verify_web_launch_contract() {
  ...
  FAOLLA_EXPECTED_WEB_NAME="$APP_NAME" \
    ...
    FAOLLA_EXPECTED_NEXT="$NEXT_ENTRY" \
    FAOLLA_EXPECTED_NODE="$NODE_ENTRY" \
    ...
    timeout --signal=TERM --kill-after=1s 3s node -e '
      ...
      let list;
      try { list = JSON.parse(fs.readFileSync(0, "utf8")); } catch { process.exit(1); }
      const expectedName = process.env.FAOLLA_EXPECTED_WEB_NAME;
      const expectedPid = process.env.FAOLLA_EXPECTED_WEB_PID;
      const expectedNext = process.env.FAOLLA_EXPECTED_NEXT;
      const expectedNode = process.env.FAOLLA_EXPECTED_NODE;
      const expectedCwd = process.env.FAOLLA_EXPECTED_CWD;
      const expectedPort = process.env.FAOLLA_EXPECTED_PORT;
      if (
        !Array.isArray(list) ||
        !expectedName ||
        ...
        !expectedNode ||
        !expectedCwd ||
        !/^[1-9][0-9]{0,4}$/.test(expectedPort || "") ||
        Number(expectedPort) > 65535
      ) process.exit(1);
      ...
      const observedInterpreter = environment.exec_interpreter === "node"
        ? expectedNode
        : environment.exec_interpreter;
      if (
        ...
        environment.pm_exec_path !== expectedNext ||
        environment.pm_cwd !== expectedCwd ||
        observedInterpreter !== expectedNode ||
        environment.exec_mode !== "fork_mode" ||
        !Array.isArray(environment.args) ||
        environment.args.length !== 3 ||
        environment.args[0] !== "start" ||
        environment.args[1] !== "-p" ||
        environment.args[2] !== expectedPort ||
...
NEXT_ENTRY="$PREVIOUS_RUNTIME_DIR/node_modules/next/dist/bin/next"
NODE_ENTRY="$(command -v node 2>/dev/null || true)"
[ -n "$NODE_ENTRY" ] && [ -f "$NEXT_ENTRY" ] && [ ! -L "$NEXT_ENTRY" ] || exit 1
NEXT_ENTRY="$(readlink -f -- "$NEXT_ENTRY" 2>/dev/null || true)"
NODE_ENTRY="$(readlink -f -- "$NODE_ENTRY" 2>/dev/null || true)"
[ "$NEXT_ENTRY" = "$PREVIOUS_RUNTIME_DIR/node_modules/next/dist/bin/next" ] \
  && [ -f "$NEXT_ENTRY" ] && [ ! -L "$NEXT_ENTRY" ] \
  && [ -n "$NODE_ENTRY" ] && [ -f "$NODE_ENTRY" ] && [ -x "$NODE_ENTRY" ] \
  && [ ! -L "$NODE_ENTRY" ] || exit 1
readonly NEXT_ENTRY NODE_ENTRY
revalidate_frozen_runtime || exit 1
revalidate_deploy_lock || exit 1
  ...
  NODE_OPTIONS="" NODE_PATH="" \
    ...
    PORT="$APP_PORT" timeout --signal=TERM --kill-after=5s 30s \
      pm2 start "$NEXT_ENTRY" --name "$APP_NAME" \
        --interpreter "$NODE_ENTRY" --cwd "$PREVIOUS_RUNTIME_DIR" \
        -- start -p "$APP_PORT" \
      >/dev/null 2>&1
) >/dev/null 2>&1 || exit 1
...
verify_stable_local_old_build() {
  ...
  for _ in $(seq 1 30); do
    if verify_web_launch_contract "$STARTED_WEB_PID" \
