...
import { MAINTENANCE_PRELAUNCH_RECOVERY_PRIOR_RUNS as SPECS,
  inspectMaintenancePrelaunchRecoveryHistory, createMaintenancePrelaunchRecoveryWorkflowEvidence } from "./production-maintenance-prelaunch-recovery-workflow.mjs";
import { validateMaintenancePrelaunchRecoveryInspection, MAINTENANCE_PRELAUNCH_RECOVERY_INCIDENT as INCIDENT,
  MAINTENANCE_PRELAUNCH_RECOVERY_AUTHORIZATION as AUTH, MAINTENANCE_PRELAUNCH_RECOVERY_AUTHORIZATION_DIGEST,
  encodeMaintenancePrelaunchRecoveryEvidence, decodeMaintenancePrelaunchRecoveryEvidence } from "./production-maintenance-prelaunch-recovery.mjs";
...
const NOW = Date.parse("2026-09-14T15:30:00Z"), TARGET = "a".repeat(40), SELF = "99999999999", CI = "99999999998";
...
function inspection() {
  ...
  return validateMaintenancePrelaunchRecoveryInspection({ version: 1, state: "prelaunch-recovery-inspected",
    ...
    revision: 35, stateDigest: INCIDENT.stateDigest, createdAt: INCIDENT.createdAt, activeAttempt: 3,
    sourceDiffDigest: "f".repeat(64), migrationDigest: "a".repeat(64), predecessorJournalDigest: hash(null),
    stoppedBaselineDigest: hash(stoppedBaseline), stoppedBaseline, authorizationDigest: MAINTENANCE_PRELAUNCH_RECOVERY_AUTHORIZATION_DIGEST,
...
const env = () => ({ GITHUB_REPOSITORY: "fafona/space", GITHUB_EVENT_NAME: "workflow_dispatch", GITHUB_REF: "refs/heads/main",
  ...
  EXPECTED_OLD_SHA: INCIDENT.expectedOldSha, MAINTENANCE_OPERATION_ID: INCIDENT.operationId, ACTION: "recover-prelaunch",
  CONFIRMATION: "RECOVER_PRELAUNCH_PRODUCTION_MAINTENANCE_UNTIL_20260914T200000Z" });
...
function fixture(mutate = (_key, value) => value) {
  ...
  const self = addRepo({ ...SPECS[3].run, id: Number(SELF), head_sha: TARGET, status: "in_progress", conclusion: null,
    created_at: "2026-09-14T15:10:00Z", run_started_at: "2026-09-14T15:10:00Z", updated_at: "2026-09-14T15:10:01Z" });
  const ci = addRepo({ ...SPECS[4].run, id: Number(CI), head_sha: TARGET,
    created_at: "2026-09-14T15:01:00Z", run_started_at: "2026-09-14T15:01:00Z", updated_at: "2026-09-14T15:09:00Z" });
  ...
  db.set("jobs:" + CI, { total_count: 10, jobs: SPECS[4].jobs.map((job, index) => ({ ...clone(job), id: 9000 + index,
    run_id: Number(CI), head_sha: TARGET, started_at: "2026-09-14T15:01:05Z", completed_at: "2026-09-14T15:08:59Z" })) });
...
const history = mutate => inspectMaintenancePrelaunchRecoveryHistory(inspection(), fixture(mutate).api, NOW, SELF, () => NOW);
const evidence = (mutate, override = {}) => createMaintenancePrelaunchRecoveryWorkflowEvidence(inspection(), { ...env(), ...override }, fixture(mutate).api, NOW, () => NOW);

test("prelaunch recovery binds exact B9/R9/failed D9, prior renewal and full CI without issuing a new deploy attempt", async () => {
  const result = await history();
  assert.equal(result.records.length, 5);
  assert.deepEqual(result.records.map(record => record.jobs[0].steps?.length ?? record.jobs.length), [30, 27, 18, 46, 10]);
  assert.equal(result.cutoff, Date.parse("2026-09-14T13:54:41Z"));
  assert.equal(result.predecessorStateDigest, INCIDENT.stateDigest);
  assert.equal(result.failedDeployPurpose, "historical-prelaunch-failure-not-successful-deploy");
  ...
  assert.equal(e.prelaunchRecoveryRunId, SELF);
  assert.deepEqual(decodeMaintenancePrelaunchRecoveryEvidence(encodeMaintenancePrelaunchRecoveryEvidence(e)), e);
  assert.equal(SPECS[0].run.id, 34852696974);
...
test("every fixed run event, head, identity, attempt, terminal result and timestamp is exact", async () => {
  for (const spec of SPECS) for (const patch of [
    ...
    { updated_at: "2026-09-14T15:20:00Z" }, { path: ".github/workflows/other.yml" },
  ]) {
  ...
  for (const spec of SPECS) {
    ...
    for (let index = 0; index < spec.jobs[0].steps.length; index++) {
      for (const change of [s => s.number++, s => s.name += " changed", s => s.status = "in_progress",
        ...
        s => s.started_at = "2026-09-14T15:20:00Z", s => s.completed_at = "2026-09-14T15:20:00Z"])
...
test("historical B9 inventory cannot gain or change an identity, digest, expiry or byte count", async () => {
  for (const change of [v => v.total_count++, v => v.artifacts.pop(),
    ...
    await assert.rejects(history((key, value) => { if (key === "artifacts:34852696974") change(value); return value; }));
});

test("failed D9 and prior renewal/CI must still have exactly zero artifacts; readiness bytes are also bound", async () => {
  for (const run of ["34855913697", "34852190872", "34850320013"]) {
  ...
  for (const change of [v => v.artifacts[0].digest = "sha256:" + "b".repeat(64),
    ...
    await assert.rejects(history((key, value) => { if (key === "artifacts:34855754738") change(value); return value; }));
  }
});

test("all five histories reject unknown same-second, later, old retried or nonterminal activity", async () => {
  for (const [index, file] of FILES.entries()) for (const time of ["2026-09-14T13:54:41Z", "2026-09-14T15:20:00Z"]) {
...
test("pagination counts, duplicate IDs, missing pages and the complete second observation fail closed", async () => {
  ...
  await assert.rejects(history((key, value, count) => {
    if (key === FILES[0] + ":1" && count === 2) { value.workflow_runs[0].updated_at = "2026-09-14T15:20:00Z"; } return value;
  }));
  for (const keyName of ["run:34852696974", "jobs:34852190872", "jobs:34850320013", "artifacts:34852696974", "artifacts:34855913697", "jobs:34855754738"])
    await assert.rejects(history((key, value, count) => {
  ...
  assert.equal((await inspectMaintenancePrelaunchRecoveryHistory(inspection(), f.api, NOW, SELF, () => NOW)).rows.length, 106);
  f.db.get(FILES[1] + ":2").total_count = 102;
  await assert.rejects(inspectMaintenancePrelaunchRecoveryHistory(inspection(), f.api, NOW, SELF, () => NOW));
});

test("only exact active prelaunch recovery workflow env may authorize the same unused attempt", async () => {
...
test("current main must be stable with all ten exact successful CI jobs and prior completed CI", async () => {
  ...
  for (const change of [v => v.total_count--, v => v.jobs.pop(), v => v.jobs[0].name = "fake",
    ...
    v => v.jobs[0].completed_at = "2026-09-14T15:10:00Z"])
...
test("actual authorization bounds and five-minute baseline never use an old clock", async () => {
  for (const now of [AUTH.authorizedAt - 1, AUTH.expiresAt, AUTH.expiresAt + 1])
    await assert.rejects(createMaintenancePrelaunchRecoveryWorkflowEvidence(inspection(), env(), fixture().api, now));
  await assert.rejects(createMaintenancePrelaunchRecoveryWorkflowEvidence(inspection(), env(), fixture().api, NOW + 300001));
  await assert.rejects(createMaintenancePrelaunchRecoveryWorkflowEvidence(inspection(), env(), fixture().api, NOW - 1001));
});
...
test("current-run updates use each actual observation clock without moving the original history time", async () => {
  ...
  const value = await createMaintenancePrelaunchRecoveryWorkflowEvidence(inspection(), env(), f.api, NOW, () => NOW + elapsed);
  ...
  await assert.rejects(createMaintenancePrelaunchRecoveryWorkflowEvidence(inspection(), env(), future.api, NOW, () => NOW + 1000));
});

test("CLI remains GET-only, bounded, masks before output, and never downloads backup payloads or raw logs", () => {
  const source = readFileSync(new URL("./production-maintenance-prelaunch-recovery-workflow.mjs", import.meta.url), "utf8");
