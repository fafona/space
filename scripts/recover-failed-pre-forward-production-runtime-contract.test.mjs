...
test("recovery is immutable to the one failed pre-forward incident", () => {
  assert.match(
    source,
    /EXPECTED_INCIDENT_DEPLOY_RUN_ID="32657417536"/,
  );
  assert.match(
    source,
    /EXPECTED_INCIDENT_SHA="f7ef0c0793be2c41d981fabae5ee44af35654904"/,
  );
  assert.match(
    source,
    /EXPECTED_INCIDENT_READINESS_RUN_ID="32657364781"/,
  );
  assert.match(source, /EXPECTED_INCIDENT_READINESS_RUN_ATTEMPT="1"/);
  assert.match(
    source,
    /EXPECTED_OLD_BUILD_ID="50a54b0bb04ca5c34b5ded2bd4419dae6701501a"/,
  );
  assert.match(
    source,
    /EXPECTED_CONFIRMATION="RECOVER_FAILED_PRE_FORWARD_DEPLOY_32657417536"/,
  );
  assert.doesNotMatch(
    source,
    /PRIOR_FAILED_RECOVERY|prior_failed_recovery|failedRecovery/,
  );
...
test("current release, environment, helper blobs, and deploy lock remain frozen", () => {
  ...
  assert.match(
    source,
    /EXPECTED_OLD_RELEASE_PREFIX="\$\{EXPECTED_OLD_BUILD_ID:0:12\}"/,
  );
  assert.match(
    source,
    /\^\(\[0-9a-f\]\{12\}\)-\(\[0-9\]\{14\}\)\$/,
  ...
  assert.match(
    source,
    /\[ "\$\{BASH_REMATCH\[1\]\}" = "\$EXPECTED_OLD_RELEASE_PREFIX" \]/,
  );
  assert.doesNotMatch(source, /\^2a121454a18a-/);
  ...
  const releaseNameBinding = sourceBetween(
    'PREVIOUS_RELEASE_NAME="$(basename -- "$PREVIOUS_RUNTIME_DIR")"',
    '\n[ -d "$PREVIOUS_RUNTIME_DIR/.next" ]',
  );
  const runReleaseNameBinding = (releaseName) => spawnSync(
    ...
    ["-s"],
    {
      ...
      env: {
        ...process.env,
        CONTRACT_RELEASE_NAME: releaseName,
        MSYS2_ENV_CONV_EXCL: "*",
        MSYS_NO_PATHCONV: "1",
      },
      input: [
        ...
        'EXPECTED_OLD_BUILD_ID="50a54b0bb04ca5c34b5ded2bd4419dae6701501a"',
        'EXPECTED_OLD_RELEASE_PREFIX="${EXPECTED_OLD_BUILD_ID:0:12}"',
        'PREVIOUS_RUNTIME_DIR="/srv/faolla/releases/$CONTRACT_RELEASE_NAME"',
        releaseNameBinding,
      ].join("\n"),
    },
  );
  const acceptedRelease = runReleaseNameBinding("50a54b0bb04c-20260823123456");
  assert.equal(acceptedRelease.status, 0, acceptedRelease.stderr);
  for (const rejectedRelease of [
    "2a121454a18a-20260823123456",
    "50a54b0bb04d-20260823123456",
    "50A54B0BB04C-20260823123456",
    "50a54b0bb04c-2026082312345",
    "50a54b0bb04c-20260823123456-extra",
  ]) {
    const rejected = runReleaseNameBinding(rejectedRelease);
    assert.notEqual(rejected.status, 0, rejectedRelease);
...
test("runtime mutation requires inactive web, a free port, and identity-bound rollback", () => {
  ...
  assert.ok(occurrences('verify_web_launch_contract "$web_pid" || exit 1') >= 5);
  assert.match(
    source,
    /pm2 start "\$NEXT_ENTRY" --name "\$APP_NAME"[\s\S]+--interpreter "\$NODE_ENTRY" --cwd "\$PREVIOUS_RUNTIME_DIR"[\s\S]+-- start -p "\$APP_PORT"/,
  );
  assert.doesNotMatch(source, /pm2 start npm/);
...
test("web launch contract accepts only the direct Next CLI and rejects npm wrappers", () => {
  const launchContract = `${sourceBetween(
    "verify_web_launch_contract() {",
    "\n}\n\nrevalidate_frozen_runtime || exit 1",
  )}\n}`;
  const webName = "faolla-web";
  const runtimeDirectory = "/srv/faolla/releases/50a54b0bb04c-20260823123456";
  const webPid = 419;
  const port = "3000";
  const nextEntry = `${runtimeDirectory}/node_modules/next/dist/bin/next`;
  const nodeEntry = "/usr/bin/node";
  const canonicalWeb = {
    name: webName,
    pid: webPid,
    pm_id: 2,
    pm2_env: {
      ...
      pm_id: 2,
      status: "online",
      pm_exec_path: nextEntry,
      pm_cwd: runtimeDirectory,
      exec_interpreter: nodeEntry,
      exec_mode: "fork_mode",
      args: ["start", "-p", port],
      node_args: [],
    },
  ...
  const unrelatedWorker = {
    name: "merchant-enterprise-automation-worker",
    pid: 731,
    pm_id: 7,
    pm2_env: {
      name: "merchant-enterprise-automation-worker",
  ...
  const parserEnvironment = {
    ...process.env,
    FAOLLA_EXPECTED_WEB_NAME: webName,
    FAOLLA_EXPECTED_WEB_PID: String(webPid),
    FAOLLA_EXPECTED_NEXT: nextEntry,
    FAOLLA_EXPECTED_NODE: nodeEntry,
    FAOLLA_EXPECTED_CWD: runtimeDirectory,
    FAOLLA_EXPECTED_PORT: port,
  };
  const parsed = spawnSync(process.execPath, ["-e", parser], {
    encoding: "utf8",
    env: parserEnvironment,
    input: JSON.stringify([unrelatedWorker, canonicalWeb]),
  });
  ...
  const runFixture = (
    ...
    input: [
      ...
      `APP_NAME=${shellSingleQuote(webName)}`,
      `APP_PORT=${shellSingleQuote(port)}`,
      `NEXT_ENTRY=${shellSingleQuote(nextEntry)}`,
      `NODE_ENTRY=${shellSingleQuote(nodeEntry)}`,
      `STARTED_WEB_PID=${shellSingleQuote(webPid)}`,
      'STARTED_WEB_START_TICKS="987654"',
      'STARTED_WEB_PROCESS_IDENTITY="8:15"',
      'STARTED_WEB_CWD_IDENTITY="8:21:1700000000"',
      "identity_calls=0",
      ...
      '      FAOLLA_EXPECTED_WEB_NAME="$FAOLLA_EXPECTED_WEB_NAME" \\',
      '      FAOLLA_EXPECTED_WEB_PID="$FAOLLA_EXPECTED_WEB_PID" \\',
      '      FAOLLA_EXPECTED_NEXT="$FAOLLA_EXPECTED_NEXT" \\',
      '      FAOLLA_EXPECTED_NODE="$FAOLLA_EXPECTED_NODE" \\',
      '      FAOLLA_EXPECTED_CWD="$FAOLLA_EXPECTED_CWD" \\',
      '      FAOLLA_EXPECTED_PORT="$FAOLLA_EXPECTED_PORT" "$@"',
      "  else",
      '    "$@"',
      "  fi",
      "}",
      'verify_web_launch_contract "$STARTED_WEB_PID"',
      '[ "$identity_calls" -eq 2 ]',
      "",
    ].join("\n"),
  });

  const accepted = runFixture([unrelatedWorker, canonicalWeb]);
  assert.equal(accepted.status, 0, `${accepted.stdout}\n${accepted.stderr}`);
  assert.equal(accepted.stdout, "");
  assert.equal(accepted.stderr, "");
  const literalNode = runFixture([
    unrelatedWorker,
    {
      ...canonicalWeb,
      pm2_env: { ...canonicalWeb.pm2_env, exec_interpreter: "node" },
    },
  ]);
  assert.equal(literalNode.status, 0, literalNode.stderr);

  const rejectedFixtures = [
    ["invalid JSON", "not-json"],
    ["non-array JSON", { web: canonicalWeb }],
    ["missing web", [unrelatedWorker]],
    ["duplicate web", [canonicalWeb, structuredClone(canonicalWeb)]],
    ["wrong top-level name", [{ ...canonicalWeb, name: "renamed-web" }]],
    ["wrong pid", [{ ...canonicalWeb, pid: webPid + 1 }]],
    [
      "npm wrapper",
      [{
        ...canonicalWeb,
        pm2_env: {
          ...canonicalWeb.pm2_env,
    ...
    [
      ...
      [{
        ...canonicalWeb,
        pm2_env: { ...canonicalWeb.pm2_env, pm_exec_path: "/tmp/next" },
      }],
    ],
    [
      "wrong cwd",
      [{ ...canonicalWeb, pm2_env: { ...canonicalWeb.pm2_env, pm_cwd: "/tmp" } }],
    ],
    [
      "wrong interpreter",
      [{
        ...canonicalWeb,
        pm2_env: { ...canonicalWeb.pm2_env, exec_interpreter: "/bin/bash" },
      }],
    ],
    [
      ...
      [{
        ...canonicalWeb,
        pm2_env: { ...canonicalWeb.pm2_env, exec_mode: "cluster_mode" },
      }],
    ],
    [
      "legacy npm arguments",
      [{
        ...canonicalWeb,
        pm2_env: {
          ...canonicalWeb.pm2_env,
    ...
    [
      ...
      [{
        ...canonicalWeb,
        pm2_env: { ...canonicalWeb.pm2_env, node_args: ["--inspect"] },
      }],
    ],
  ];
  ...
  for (const identityFailureAt of [1, 2]) {
    const rejected = runFixture([canonicalWeb], { identityFailureAt });
    assert.notEqual(rejected.status, 0, `identity check ${identityFailureAt}`);
    assert.equal(rejected.stdout, "");
    assert.equal(rejected.stderr, "");
  }
  const pm2Failure = runFixture([canonicalWeb], { pm2Status: 42 });
