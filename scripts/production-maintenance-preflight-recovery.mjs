...
import { assertMaintenancePrelaunchRecoveryProgress, validateMaintenancePrelaunchRecoveryState } from "./production-maintenance-prelaunch-recovery.mjs";
...
/** Pure recovery of ONE fixed preflight failure, carrying the SAME unused
 ...
 * The complete failed v10 is reconstructed byte-for-byte without duplicating
 * its history. Every v10 audit (including its 20:00 deadline) remains untouched.
 * Historical validation never substitutes a clock for live host observations.
 */
export const MAINTENANCE_PREFLIGHT_RECOVERY_INCIDENT = Object.freeze({
  operationId: BUDGET.operationId, bootId: BUDGET.bootId,
  previousTargetSha: "5c1130adf37308e30e5c656f3d894e096cd65b7f",
  expectedOldSha: BUDGET.expectedOldSha, revision: 38, createdAt: BUDGET.createdAt,
  stateBytes: 1923312, stateDigest: "06654f0d57a38ac823983762308a6dba2042d3b6a6b0c701116781bed4b6840c",
  backupRunId: "34868191767", backupRunAttempt: 1,
  migrationRunId: BUDGET.migrationRunId, migrationRunAttempt: 1,
  readinessRunId: "34873107909", readinessRunAttempt: 1,
  failedDeployRunId: "34873244708", failedDeployRunAttempt: 1,
  priorRecoveryRunId: "34867675835", priorRecoveryRunAttempt: 1,
  priorMainCIrunId: "34865334166", priorMainCIrunAttempt: 1,
});
const INCIDENT = MAINTENANCE_PREFLIGHT_RECOVERY_INCIDENT;
export const MAINTENANCE_PREFLIGHT_RECOVERY_AUTHORIZATION = Object.freeze({
  ...
  authorizedAt: Date.parse("2026-09-14T17:48:01.000Z"),
  previousExpiresAt: Date.parse("2026-09-14T20:00:00.000Z"),
...
const EVIDENCE_KEYS = [...INSPECTION_KEYS, "toolsSha", "preflightRecoveryRunId", "preflightRecoveryRunAttempt", "mainCIrunId", "historyDigest", "historyCheckedAt"];
...
function predecessorAudit(state) {
  if (!exact(state, STATE_KEYS) || state.version !== 10 || state.activeAttempt !== 3 ||
  ...
  // A FIXED historical audit clock for only this byte-pinned v10. It is inside
  // the immutable original 20:00 window and cannot be supplied by the caller.
  // Actual validation/recovery calls separately enforce the real 17:48..20:00
  ...
  if (historicalAuditClock >= AUTHORIZATION.previousExpiresAt ||
      state.prelaunchRecovery?.authorization?.expiresAt !== AUTHORIZATION.previousExpiresAt ||
      state.prelaunchRecovery?.evidence?.prelaunchRecoveryRunId !== INCIDENT.priorRecoveryRunId ||
      state.prelaunchRecovery?.evidence?.prelaunchRecoveryRunAttempt !== INCIDENT.priorRecoveryRunAttempt ||
      state.prelaunchRecovery?.evidence?.mainCIrunId !== INCIDENT.priorMainCIrunId) fail();
  try { validateMaintenancePrelaunchRecoveryState(state, { bootId: INCIDENT.bootId, now: historicalAuditClock }); } catch { fail(); }
  return state;
}
function compactPredecessor(state) {
  return { version: 10, revision: INCIDENT.revision, targetSha: INCIDENT.previousTargetSha, phase: "failed-held",
    stateDigest: INCIDENT.stateDigest, stateBytes: INCIDENT.stateBytes, ingress: state.ingress };
}
function projectOriginal(state) {
  const compact = state.preflightRecovery?.predecessor;
  if (!exact(compact, PREDECESSOR_KEYS) || compact.version !== 10 || compact.revision !== INCIDENT.revision ||
  ...
  // New live launch evidence is NOT discarded: it remains on the actual v11.
  const original = { ...project(state, STATE_KEYS), version: 10, revision: INCIDENT.revision,
...
function baselineFor(value, predecessor, now) {
  ...
  baselineShape(value);
  // Preflight recovery is not permission to re-anchor the unused attempt. Only
  ...
  if (!isDeepStrictEqual(project(value, stableKeys), project(predecessor.prelaunchRecovery.stoppedBaseline, stableKeys))) fail();
  if (
      ...
      value.observedAt > now || now - value.observedAt > MAINTENANCE_PREFLIGHT_RECOVERY_HISTORY_MAX_AGE_MS ||
...
function checkInspection(value) {
  if (!exact(value, INSPECTION_KEYS) || value.version !== 1 || value.state !== "preflight-recovery-inspected" ||
      ...
      value.authorizationDigest !== MAINTENANCE_PREFLIGHT_RECOVERY_AUTHORIZATION_DIGEST ||
...
function checkEvidence(value) {
  ...
  if (value.toolsSha !== value.targetSha || !matches(RUN, value.preflightRecoveryRunId) || value.preflightRecoveryRunAttempt !== 1 ||
      !matches(RUN, value.mainCIrunId) || value.preflightRecoveryRunId === value.mainCIrunId ||
      PRIOR_RUNS.includes(value.preflightRecoveryRunId) || PRIOR_RUNS.includes(value.mainCIrunId) || !matches(DIGEST, value.historyDigest) ||
...
function inspectionFor(state, context) {
  ...
  return checkInspection({ version: 1, state: "preflight-recovery-inspected", operationId: INCIDENT.operationId, targetSha: context.targetSha,
    ...
    authorizationDigest: MAINTENANCE_PREFLIGHT_RECOVERY_AUTHORIZATION_DIGEST, ...project(INCIDENT, RUN_KEYS) });
}

export function validateMaintenancePreflightRecoveryPredecessor(rawState, rawClock) {
  const state = bounded(rawState), clock = bounded(rawClock); actualClock(clock); predecessorAudit(state); return freeze(state);
}
export function createMaintenancePreflightRecoveryInspection(rawState, rawContext) {
  return freeze(inspectionFor(bounded(rawState), bounded(rawContext)));
}
export function validateMaintenancePreflightRecoveryInspection(value) {
  return freeze(checkInspection(bounded(value, MAINTENANCE_PREFLIGHT_RECOVERY_MAX_EVIDENCE_BYTES)));
}
export function validateMaintenancePreflightRecoveryEvidence(value) {
  return freeze(checkEvidence(bounded(value, MAINTENANCE_PREFLIGHT_RECOVERY_MAX_EVIDENCE_BYTES)));
}
export function encodeMaintenancePreflightRecoveryEvidence(value) {
  const encoded = Buffer.from(JSON.stringify(validateMaintenancePreflightRecoveryEvidence(value))).toString("base64url");
  if (encoded.length > MAINTENANCE_PREFLIGHT_RECOVERY_MAX_EVIDENCE_BYTES) fail(); return encoded;
}
export function decodeMaintenancePreflightRecoveryEvidence(value) {
  if (typeof value !== "string" || !value || value.length > MAINTENANCE_PREFLIGHT_RECOVERY_MAX_EVIDENCE_BYTES || !/^[A-Za-z0-9_-]+$/.test(value)) fail();
  try {
    const decoded = new TextDecoder("utf-8", { fatal: true }).decode(Buffer.from(value, "base64url"));
    const result = validateMaintenancePreflightRecoveryEvidence(JSON.parse(decoded));
    if (encodeMaintenancePreflightRecoveryEvidence(result) !== value) fail(); return result;
  } catch { fail(); }
}
export function buildMaintenancePreflightRecoveredState(rawState, rawEvidence, rawContext) {
  const state = bounded(rawState), context = bounded(rawContext), evidence = checkEvidence(bounded(rawEvidence, MAINTENANCE_PREFLIGHT_RECOVERY_MAX_EVIDENCE_BYTES));
  ...
  if (!isDeepStrictEqual(inspection, project(evidence, INSPECTION_KEYS)) || evidence.historyCheckedAt > context.now ||
      context.now - evidence.historyCheckedAt > MAINTENANCE_PREFLIGHT_RECOVERY_HISTORY_MAX_AGE_MS) fail();
  ...
  const next = { ...state, version: 11, revision: state.revision + 1, targetSha: context.targetSha, phase: "held",
    preflightRecovery: { version: 1, predecessor: compactPredecessor(state), evidence,
      recoveredAt: context.now, stoppedBaseline: context.stoppedBaseline, authorization: { ...AUTHORIZATION } } };
  return validateMaintenancePreflightRecoveryState(next, { bootId: context.bootId, now: context.now });
}
export function validateMaintenancePreflightRecoveryState(rawState, rawClock) {
  const state = bounded(rawState), clock = bounded(rawClock); actualClock(clock);
  if (!exact(state, [...STATE_KEYS, "preflightRecovery"]) || state.version !== 11 || state.activeAttempt !== 3 ||
      !integer(state.revision) || state.revision <= INCIDENT.revision || !PHASES.includes(state.phase) ||
      !exact(state.preflightRecovery, AUDIT_KEYS)) fail();
  ...
  if (!record(state.ingress) || state.targetSha !== evidence.targetSha ||
      ...
      evidence.historyCheckedAt > audit.recoveredAt || audit.recoveredAt - evidence.historyCheckedAt > MAINTENANCE_PREFLIGHT_RECOVERY_HISTORY_MAX_AGE_MS ||
...
export function reconstructMaintenancePreflightRecoveryPredecessor(rawState, rawClock) {
  ...
  return freeze(projectOriginal(validateMaintenancePreflightRecoveryState(rawState, rawClock)));
}
...
export function assertMaintenancePreflightRecoveryProgress(rawPrevious, rawNext) {
  const previous = bounded(rawPrevious), next = bounded(rawNext);
  if (!record(previous) || !record(next)) fail();
  if (previous.version !== 11 && next.version !== 11) {
    if ([previous, next].some(state => Object.hasOwn(state, "preflightRecovery"))) fail();
    return assertMaintenancePrelaunchRecoveryProgress(previous, next);
  }
  if (previous.version === 10 && next.version === 11) {
    if (!exact(next.preflightRecovery, AUDIT_KEYS)) fail();
    const audit = next.preflightRecovery, evidence = checkEvidence(audit.evidence);
    const expected = buildMaintenancePreflightRecoveredState(previous, evidence, { operationId: evidence.operationId,
  ...
  if (previous.version !== 11 || next.version !== 11 || !integer(previous.revision) || previous.revision === Number.MAX_SAFE_INTEGER ||
      next.revision !== previous.revision + 1 || [...FIXED_KEYS, "targetSha", "activeAttempt", "preflightRecovery"].some(key => !isDeepStrictEqual(previous[key], next[key])) ||
  ...
  const auditClock = { bootId: INCIDENT.bootId, now: previous.preflightRecovery?.recoveredAt };
  validateMaintenancePreflightRecoveryState(previous, auditClock); validateMaintenancePreflightRecoveryState(next, auditClock);
