...
import { assertWindowRenewalGenerationsStopped, assertWindowRenewalStopped, captureWindowRenewalBaseline, validateWindowRenewalBaseline, verifyWindowRenewalBaseline } from "./production-maintenance-window-inspection.mjs";
...
const NOW = Date.parse("2026-09-14T13:00:00Z"), copy = value => structuredClone(value);
...
import { MAINTENANCE_WINDOW_RENEWAL_INCIDENT as WINDOW, MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION as WINDOW_AUTH,
  createMaintenanceWindowRenewalInspection, buildMaintenanceWindowRenewedState,
  validateMaintenanceWindowRenewalPredecessor } from "./production-maintenance-window-renewal.mjs";
...
async function renew(f) {
  const stoppedBaseline = await captureWindowRenewalBaseline(f.state, f.io);
  const context = { operationId: U, previousTargetSha: WINDOW.previousTargetSha, targetSha: "a".repeat(40), expectedOldSha: O,
    expectedRevision: WINDOW.revision, expectedDigest: WINDOW.stateDigest, bootId: BOOT, now: NOW,
    sourceDiffDigest: "b".repeat(64), migrationDigest: "c".repeat(64), stoppedBaseline };
  f.state = copy(buildMaintenanceWindowRenewedState(f.state, { ...createMaintenanceWindowRenewalInspection(f.state, context),
    toolsSha: context.targetSha, windowRenewalRunId: "34900000001", windowRenewalRunAttempt: 1, mainCIrunId: "34900000002",
    historyDigest: "d".repeat(64), historyCheckedAt: NOW - 1000 }, context));
  return f;
}
test("unused expired v8 and flat renewed v9 compose actual O/T5/T6/T7 read-only proofs", { concurrency: false }, async t => {
  ...
  const seed = assembleV8(seed7); mappings.set(JSON.stringify(seed.state), WINDOW.stateDigest);
  const fixture = () => assembleV8(assembleV7(assembleV6(fixtureV5())));
  const reject = promise => assert.rejects(promise, /window_(?:inspection_unverified|handoff_unverified)/);
  await t.test("only four exact synthetic states map, real private state is never used", () => {
    assert.equal(crypto.createHash("sha256").update(JSON.stringify(seed.state)).digest("hex"), WINDOW.stateDigest);
  ...
  await t.test("capture observes all four real frozen directories/generations and port; signed baseline timestamp retained", async () => {
    const f = fixture(), bytes = JSON.stringify(f.state), baseline = await captureWindowRenewalBaseline(f.state, f.io);
    ...
    f.io.now = () => NOW + 1; assert.equal(await verifyWindowRenewalBaseline(f.state, baseline, f.io), true); assert.equal(baseline.observedAt, NOW);
  });
  await t.test("O web/worker, T5, T6 and T7 generations or descendants cannot revive", async () => {
    for (const select of [f => f.state.runtime.web.processes[0], f => f.state.runtime.worker.managed.processes[0],
      ...
      const f = fixture(), fact = select(f); f.facts.set(fact.pid, fact); await reject(captureWindowRenewalBaseline(f.state, f.io));
    }
  });
  await t.test("later full-stop assertion does not consult current; it still requires empty port", async () => {
    ...
    assert.equal(await assertWindowRenewalStopped(f.state, f.io), true);
    f.io.runtime.portEmpty = () => false; await reject(assertWindowRenewalStopped(f.state, f.io));
  });
  await t.test("generation-only checks allow T8 listener and same app, but exclude all histories and registry drift", async () => {
    ...
    assert.equal(await assertWindowRenewalGenerationsStopped(good.f.state, good.f.io), true);
    ...
    ]) { const changed = make(); mutate(changed); await reject(assertWindowRenewalGenerationsStopped(changed.f.state, changed.f.io)); }
    for (const sha of [O, T5, T6, T7]) {
      const changed = make(); changed.row.pm2_env.pm_exec_path = changed.f.disks.get(sha).nextEntryPath;
      await reject(assertWindowRenewalGenerationsStopped(changed.f.state, changed.f.io));
      ...
      await reject(assertWindowRenewalGenerationsStopped(descendant.f.state, descendant.f.io));
    }
  });
  await t.test("each original/candidate disk and environment is checked in full-stop and generation-only paths", async () => {
    for (const observe of [captureWindowRenewalBaseline, assertWindowRenewalGenerationsStopped]) {
      for (const sha of [O, T5, T6, T7]) {
  ...
  await t.test("fresh daemon/boot, registry and current observations cannot drift within a check", async () => {
    ...
    await reject(captureWindowRenewalBaseline(current.state, current.io));
    const registry = fixture(); let registryReads = 0;
    registry.io.runtime.pm2Registry = async () => ++registryReads === 4 ? [{ malformed: true }] : [];
    await reject(captureWindowRenewalBaseline(registry.state, registry.io));
  ...
  await t.test("T7 handoff returns only 27 physical/configuration fields and original O worker policy", async () => {
    const f = fixture(), baseline = await captureWindowRenewalBaseline(f.state, f.io), bytes = JSON.stringify(f.state);
    const fields = await readWindowRenewalHandoffFields(f.state.runtime, f.state, baseline, f.io);
    ...
    await reject(readWindowRenewalHandoffFields({ ...f.state.runtime, bootId: "bad" }, f.state, baseline, f.io));
    let n = 0; f.io.readCurrent = () => ({ ...f.current(), linkIdentity: id(++n < 3 ? 92 : 93, "link") });
    await reject(readWindowRenewalHandoffFields(f.state.runtime, f.state, baseline, f.io));
  });
  await t.test("wrong current, file/env drift, boot, journal, audit, getters and mutations reject", async () => {
    ...
    ]) { const f = fixture(); f.state = copy(f.state); mutate(f); await reject(captureWindowRenewalBaseline(f.state, f.io)); }
    ...
    await reject(captureWindowRenewalBaseline(hostile, f.io)); assert.equal(touched, 0);
    await reject(captureWindowRenewalBaseline(new Proxy(f.state, {}), f.io));
    await reject(captureWindowRenewalBaseline(f.state, { ...f.io, runtime: { ...f.io.runtime, pm2Control() { assert.fail(); } } }));
  });
  await t.test("actual deadline is enforced before and after observations; shape never authorizes extension", async () => {
    const f = fixture(), baseline = await captureWindowRenewalBaseline(f.state, f.io);
    ...
    assert.throws(() => validateWindowRenewalBaseline({ ...baseline, version: 1 }), /window_inspection_unverified/);
    for (const tooEarly of ["2026-09-14T10:00:00Z", "2026-09-14T12:31:03.999Z"]) {
      f.io.now = () => Date.parse(tooEarly); await reject(captureWindowRenewalBaseline(f.state, f.io));
    }
    f.io.now = () => Date.parse("2026-09-14T16:00:00Z"); await reject(captureWindowRenewalBaseline(f.state, f.io));
    let n = 0; f.io.now = () => ++n === 1 ? NOW : Date.parse("2026-09-14T16:00:00Z");
    await reject(captureWindowRenewalBaseline(f.state, f.io));
    n = 0; await reject(assertWindowRenewalGenerationsStopped(f.state, f.io));
    f.io.now = () => NOW - 1; await reject(verifyWindowRenewalBaseline(f.state, baseline, f.io));
  });

  await t.test("v9 reconstructs the exact v8 without changing audits and all real checks use the new clock", async () => {
    const f = fixture(), previous = JSON.stringify(f.state), oldAudit = JSON.stringify(f.state.budgetRecovery);
    await renew(f);
    assert.equal(f.state.version, 9); assert.equal(f.state.activeAttempt, 3);
    assert.equal(JSON.stringify(f.state.budgetRecovery), oldAudit);
    assert.equal(f.state.windowRenewal.predecessor.state, undefined);
    assert.equal(f.state.windowRenewal.predecessor.stateDigest, WINDOW.stateDigest);
    assert.equal(f.state.windowRenewal.stoppedBaseline.observedAt, NOW);
    const baseline = await captureWindowRenewalBaseline(f.state, f.io);
    assert.equal(await verifyWindowRenewalBaseline(f.state, baseline, f.io), true);
    assert.equal(await assertWindowRenewalStopped(f.state, f.io), true);
    assert.equal(await assertWindowRenewalGenerationsStopped(f.state, f.io), true);
    assert.equal(Object.keys(await readWindowRenewalHandoffFields(f.state.runtime, f.state, baseline, f.io)).length, 27);
    assert.equal(JSON.stringify(f.state.budgetRecovery), oldAudit);
    assert.notEqual(JSON.stringify(f.state), previous);
    f.io.now = () => WINDOW_AUTH.expiresAt;
    await reject(assertWindowRenewalStopped(f.state, f.io));
    await reject(assertWindowRenewalGenerationsStopped(f.state, f.io));
  });
  await t.test("old baseline cannot be reused and all boundary clocks are real, including post-current read", async () => {
    const f = fixture();
    await reject(verifyWindowRenewalBaseline(f.state, f.state.budgetRecovery.stoppedBaseline, f.io));
    f.io.now = () => WINDOW_AUTH.authorizedAt;
    assert.equal((await captureWindowRenewalBaseline(f.state, f.io)).observedAt, WINDOW_AUTH.authorizedAt);
    f.io.now = () => WINDOW_AUTH.expiresAt - 1;
    assert.equal((await captureWindowRenewalBaseline(f.state, f.io)).observedAt, WINDOW_AUTH.expiresAt - 1);
    let now = NOW, currentReads = 0; f.io.now = () => now;
    f.io.readCurrent = () => { if (++currentReads === 2) now = WINDOW_AUTH.expiresAt; return copy(f.current()); };
    await reject(captureWindowRenewalBaseline(f.state, f.io));
    assert.equal(currentReads, 2);
    assert.throws(() => validateMaintenanceWindowRenewalPredecessor(f.state, { bootId: BOOT, now }));
  });
  await t.test("renewal may refresh only time, not re-anchor a same-target link or a different stable registry", async () => {
    for (const renewed of [false, true]) {
      const link = fixture(); if (renewed) await renew(link);
      link.io.readCurrent = () => ({ ...link.current(), linkIdentity: id(999, "link") });
      await reject(captureWindowRenewalBaseline(link.state, link.io));
      const registry = fixture(); if (renewed) await renew(registry);
  ...
  await t.test("read-only source cannot acquire locks or actuate and covers all historical candidates", () => {
    const source = readFileSync(new URL("./production-maintenance-window-inspection.mjs", import.meta.url), "utf8");
