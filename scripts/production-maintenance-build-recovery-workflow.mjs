...
import { validateMaintenanceBuildRecoveryInspection, validateMaintenanceBuildRecoveryEvidence, encodeMaintenanceBuildRecoveryEvidence,
  MAINTENANCE_BUILD_RECOVERY_DEADLINE_EXTENSION, MAINTENANCE_BUILD_RECOVERY_HISTORY_MAX_AGE_MS,
...
function checkWorkflowClock(now) {
  if (!Number.isSafeInteger(now) || now < MAINTENANCE_BUILD_RECOVERY_DEADLINE_EXTENSION.authorizedAt ||
      now >= MAINTENANCE_BUILD_RECOVERY_DEADLINE_EXTENSION.expiresAt || now < ADDITIONAL.authorizedAt) fail();
}
...
function assertAdditionalProvenance(results, bytes, name) {
  ...
  if (statement?._type !== "https://in-toto.io/Statement/v1" || statement.predicateType !== "https://slsa.dev/provenance/v1" ||
      ...
      hash(predicate.buildDefinition.externalParameters?.workflow) !== hash({ path: ADDITIONAL.workflowPath, ref: "refs/heads/main", repository: "https://github.com/fafona/space" }) ||
      predicate.buildDefinition.internalParameters?.github?.event_name !== "schedule" ||
      ...
      predicate.buildDefinition.resolvedDependencies[0].digest.gitCommit !== ADDITIONAL.sourceSha ||
      predicate.runDetails?.builder?.id !== "https://github.com/fafona/space/.github/workflows/database-backup.yml@refs/heads/main" ||
      predicate.runDetails.metadata?.invocationId !== "https://github.com/fafona/space/actions/runs/34745334237/attempts/1") fail();
}
 ...
 * This backup never replaces the original maintenance B3/R3 subjects. */
export function validateMaintenanceBuildRecoveryAdditionalBackup(records, now) {
...
export function readMaintenanceBuildRecoveryAdditionalBackup(directory) {
...
export async function inspectMaintenanceBuildRecoveryHistory(inspection, api, now) {
  ...
  if (!additionalBackup || additionalBackup.created < incidents.at(-1).updated) fail();
  return { all, incidents, additionalBackup };
}

export async function createMaintenanceBuildRecoveryWorkflowEvidence(inspection, env, api, priorBindings, now = Date.now(), additionalRecords) {
  const checked = validateMaintenanceBuildRecoveryInspection(inspection);
  checkWorkflowClock(now);
  if (env.GITHUB_REPOSITORY !== REPOSITORY || env.GITHUB_EVENT_NAME !== "workflow_dispatch" || env.GITHUB_REF !== "refs/heads/main" ||
      ...
      env.MAINTENANCE_OPERATION_ID !== checked.operationId || env.ACTION !== "recover-build" || env.CONFIRMATION !== "RECOVER_BUILD_PRODUCTION_MAINTENANCE_UNTIL_20260913T220000Z") fail();
  const bindings = validateMaintenanceBuildRecoveryPriorBindings(checked, priorBindings);
  const additionalBackup = validateMaintenanceBuildRecoveryAdditionalBackup(additionalRecords, now);
  ...
  return validateMaintenanceBuildRecoveryEvidence({ ...checked, toolsSha: checked.targetSha,
    ...
    historyDigest: hash({ history, bindings, additionalBackup, artifacts }), historyCheckedAt: now,
    additionalBackupEvidenceDigest: hash({ spec: ADDITIONAL, history: history.additionalBackup, artifacts, verified: additionalBackup }) });
}
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  try {
    ...
    const additionalBackup = readMaintenanceBuildRecoveryAdditionalBackup(process.argv[7]);
    const evidence = await createMaintenanceBuildRecoveryWorkflowEvidence(inspection, process.env, api, priorBindings, Date.now(), additionalBackup);
