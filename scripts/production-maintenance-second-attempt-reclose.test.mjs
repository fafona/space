...
import { MAINTENANCE_SECOND_ATTEMPT_RECOVERY_AUTHORIZATION as AUTH } from "./production-maintenance-second-attempt-recovery.mjs";
...
function fixture() {
  ...
  const baseline = { version: 7, revision: 35, activeAttempt: 2, operationId: AUTH.operationId, targetSha: "e".repeat(40),
  ...
  const store = {
    async readOperationUnderExistingOperationLock() {
      ...
      // Full actual pure validation is covered by second-attempt-control.test.mjs.
...
test("every fixed binding and unsupported request action rejects before host I/O", async () => {
  for (const key of ["version", "operationId", "targetSha", "expectedOldSha", "appDir", "appName", "appPort", "bootId", "createdAt", "runtime", "tokenHash", "activeAttempt", "attemptRecovery", "secondAttemptRecovery"]) {
  ...
  for (const action of ["prepare", "recover-attempt", "recover-second-attempt", "check-held", "runtime-handoff", "snapshot-web", ""] ) {
