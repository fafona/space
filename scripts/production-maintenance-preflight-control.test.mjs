...
import { MAINTENANCE_PREFLIGHT_RECOVERY_INCIDENT as BUDGET, MAINTENANCE_PREFLIGHT_RECOVERY_AUTHORIZATION as AUTH,
 buildMaintenancePreflightRecoveredState, encodeMaintenancePreflightRecoveryEvidence, reconstructMaintenancePreflightRecoveryPredecessor } from "./production-maintenance-preflight-recovery.mjs";
...
const NOW = Date.parse("2026-09-14T18:10:00Z"), TARGET = "f".repeat(40), ID = "1:2:3:4:5:1:0:33188";
...
function request(action, f) {
  ...
  if (["inspect-preflight-recovery", "recover-preflight"].includes(action)) args.push("--previous-target-sha", BUDGET.previousTargetSha);
  if (action === "recover-preflight") args.push("--preflight-recovery-evidence", encodeMaintenancePreflightRecoveryEvidence(f.evidence));
...
function proofs(state) {
  ...
  if ([8, 9, 10, 11].includes(state.version)) proofs(state.budgetRecovery.predecessor.state);
...
function rehearsal(f, initial = f.state) {
  ...
  const validateState = state => {
    validateMaintenanceState(state, { ...state, action: state.version === 10 ? "inspect-preflight-recovery" : "check-held",
  ...
  const ops = {
    ...
    assertPreflightDiskHeadroom: record("diskHeadroom"),
    verifyIngress: record("ingress"), assertRuntimeStopped: record("originalStopped"), async assertPreflightRecoveryStopped(state) {
    ...
    async assertPreflightRecoveryGenerationsStopped(state) {
    ...
    async readPreflightRecoverySnapshot() { const state = await load(), snapshot = baselines.get(state); return { ...snapshot, state }; },
    capturePreflightRecoveryBaseline: record("captureBaseline", f.baseline),
    async verifyPreflightRecoveryBaseline(state, baseline) { events.push("verifyBaseline"); assert.deepEqual(state.budgetRecovery, f.state.budgetRecovery); assert.deepEqual(baseline, f.baseline); return true; },
    readPreflightRecoverySourceProof: record("source", { sourceDiffDigest: f.context.sourceDiffDigest }),
    readPreflightRecoveryMigrationProof: record("migration", f.context.migrationDigest),
...
test("isolated unused v9 failed-preflight to v10 recovery, controller, typed runtime and real journal storage composition", { concurrency: false }, async t => {
 await withPreflightRecoveryFixture(t, async ({ predecessor: seed, fixture: rawFixture, hash }) => {
 const fixture = () => { const f = rawFixture(); return { ...f, baseline: f.context.stoppedBaseline }; };
  await t.test("new authority is exact while ordinary checks preserve the failed v10 phase", async () => {
    ...
    assert.equal(Buffer.byteLength(JSON.stringify(seed)), 1923312);
    ...
    for (const action of ["check-held", "check-runtime-held", "runtime-handoff", "start-candidate", "end", "fail-held"]) {
      ...
      assert.equal(validateMaintenanceState(seed, ordinary, BUDGET.bootId, NOW).phase, "failed-held");
    }
    for (const now of [AUTH.authorizedAt - 1, AUTH.expiresAt]) {
      const f = fixture(seed), r = rehearsal(f); r.setNow(now);
      await assert.rejects(runMaintenanceAction(request("inspect-preflight-recovery", f), r.ops));
  ...
  await t.test("inspection has zero writes/sends and returns the actual fixture baseline binding", async () => {
    const f = fixture(seed), r = rehearsal(f), report = await runMaintenanceAction(request("inspect-preflight-recovery", f), r.ops);
  ...
  await t.test("recovery uses one durable CAS and retains the whole consumed journal and all prior audits", async () => {
    const f = fixture(seed), r = rehearsal(f), next = buildMaintenancePreflightRecoveredState(f.state, f.evidence, f.context);
    assert.equal((await runMaintenanceAction(request("recover-preflight", f), r.ops)).state, "held");
    assert.deepEqual(r.saved(), next); assert.deepEqual(reconstructMaintenancePreflightRecoveryPredecessor(next, { bootId: BUDGET.bootId, now: NOW }), f.state); assert.equal(r.writes(), 1); assert.equal(r.events.filter(event => event === "recoveryCAS").length, 1);
    ...
    assert.equal(r.saved().version, 11); assert.equal(r.saved().revision, 39); assert.equal(r.saved().activeAttempt, 3);
    ...
    await assert.rejects(runMaintenanceAction(request("recover-preflight", f), r.ops));
  ...
  await t.test("wrong target, invalid evidence, stale history or failed host proof never writes", async () => {
    for (const alter of [
      ...
      (r, q) => { q.preflightRecoveryEvidence.historyDigest = "invalid"; },
      (r, q) => { q.preflightRecoveryEvidence.historyCheckedAt = NOW - 300001; },
      ...
      r => { r.ops.assertPreflightDiskHeadroom = async () => { throw new Error("synthetic insufficient disk"); }; },
    ]) {
      const f = fixture(seed), r = rehearsal(f), q = copy(request("recover-preflight", f)); alter(r, q);
  ...
  await t.test("second full proof round rechecks source, migration and real final deadline before CAS", async () => {
    for (const kind of ["source", "migration", "deadline"]) {
      ...
      if (kind === "source") r.ops.readPreflightRecoverySourceProof = async () => ({ sourceDiffDigest: ++calls === 1 ? f.context.sourceDiffDigest : "e".repeat(64) });
      if (kind === "migration") r.ops.readPreflightRecoveryMigrationProof = async () => ++calls === 1 ? f.context.migrationDigest : "e".repeat(64);
      if (kind === "deadline") r.ops.assertDatabaseQuiet = async () => { if (++calls === 2) r.setNow(AUTH.expiresAt); };
      await assert.rejects(runMaintenanceAction(request("recover-preflight", f), r.ops)); assert.equal(r.writes(), 0);
    }
  });
  await t.test("file fsync and post-rename uncertainty produce no recovery success and no send/retry", async () => {
    for (const stage of ["fileSync", "renameAfter", "directorySync"]) {
      ...
      await assert.rejects(runMaintenanceAction(request("recover-preflight", f), r.ops), /storage_unconfirmed/);
      ...
      assert.equal(r.saved().version, stage === "fileSync" ? 10 : 11); assert.equal(r.fds.size, 0);
    }
  });
  await t.test("later v9 start persists its own attempt before one send while all three predecessors stay immutable", async () => {
    const f = fixture(seed), initial = buildMaintenancePreflightRecoveredState(f.state, f.evidence, f.context), r = rehearsal(f, initial);
  ...
  await t.test("any O/T5/T6/T7 revival blocks inspect, recovery and launch before writes or sends", async () => {
    for (const name of ["O", "T5", "T6", "T7"]) {
      for (const action of ["inspect-preflight-recovery", "recover-preflight", "start-candidate"]) {
        const f = fixture(seed), initial = action === "start-candidate" ? buildMaintenancePreflightRecoveredState(f.state, f.evidence, f.context) : f.state;
      ...
      const f = fixture(seed), r = rehearsal(f, buildMaintenancePreflightRecoveredState(f.state, f.evidence, f.context));
  ...
  await t.test("every candidate snapshot, check, registration and handoff excludes O/T5/T6/T7 before reading renewed target", async () => {
    ...
    for (const action of ["snapshot-web", "snapshot-worker", "check-candidate", "register-candidate", "candidate-handoff", "end"]) {
      r.clearEvents(); const writes = r.writes();
      r.ops.assertPreflightRecoveryGenerationsStopped = async () => { r.events.push("historicalGenerationStopped"); throw new Error("synthetic historical revival"); };
  ...
  await t.test("historical revival after resume or before reopen blocks restore and stops only the new exact resumed proof", async () => {
    for (const rejectAt of [2, 3]) {
      ...
      r.ops.assertPreflightRecoveryGenerationsStopped = async () => { r.events.push("historicalGenerationStopped");
      ...
      r.ops.assertPreflightRecoveryStopped = async () => { r.events.push("bothStopped"); throw new Error("synthetic historical remains alive"); };
  ...
  await t.test("unknown ACK is reconciled by an observation and stopped, never launched again", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenancePreflightRecoveredState(f.state, f.evidence, f.context)); r.setOutcome("unknown");
  ...
  await t.test("real storage refuses v10 journal reset, audit deletion and predecessor reuse", async () => {
    ...
    for (const alter of [s => { s.launchJournal = null; s.launchDisk = null; s.candidate = null; s.phase = "held"; },
      s => { delete s.budgetRecovery; }, s => { s.version = 9; delete s.preflightRecovery; }, s => { delete s.preflightRecovery; }, s => { s.preflightRecovery.authorization.expiresAt++; }, s => { s.activeAttempt++; },
