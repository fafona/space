...
import { validateLeaseBaseline, verifyLeaseBaseline } from "./production-maintenance-lease-inspection.mjs";
import { validateMaintenanceLeasePredecessor,
  reconstructMaintenanceLeasePredecessor } from "./production-maintenance-lease.mjs";
...
const UNUSED_TARGETS = ["3614f5bfc85cf72d064732141a9998a0bbaec513", "5c1130adf37308e30e5c656f3d894e096cd65b7f", "67ddb91bf618e9716b79df6bf97f08d3865919b7", "78ca8104442172baf046b62f9184cf9c7f6a9279", "3b6c55ea4397c4505e75c59bb3bb56d7dd7d1cc0"];
const fail = () => { throw new Error("maintenance_lease_handoff_unverified"); };
export function validateLeaseHandoffReport(raw, request) {
  try {
    const report = validateBudgetRecoveryHandoffReport(raw, request);
    validateLeaseBaseline(report.budgetBaseline.stoppedBaseline);
    if (UNUSED_TARGETS.includes(report.targetSha)) fail();
    return report;
  } catch { fail(); }
}

export async function readLeaseHandoffFields(originalRuntime, rawState, rawBaseline, observations = {}) {
  try {
    const baseline = validateLeaseBaseline(rawBaseline);
    ...
    await verifyLeaseBaseline(rawState, baseline, observations);
    const clock = { bootId: baseline.bootId, now: (observations.now ?? Date.now)() };
    const predecessor = Object.getOwnPropertyDescriptor(rawState, "version")?.value === 11 ?
      validateMaintenanceLeasePredecessor(rawState, clock) :
      reconstructMaintenanceLeasePredecessor(rawState, clock);
