...
import { assertPreflightRecoveryGenerationsStopped, assertPreflightRecoveryStopped, capturePreflightRecoveryBaseline, validatePreflightRecoveryBaseline, verifyPreflightRecoveryBaseline } from "./production-maintenance-preflight-inspection.mjs";
...
const NOW = Date.parse("2026-09-14T18:10:00Z"), copy = value => structuredClone(value);
...
import { MAINTENANCE_PREFLIGHT_RECOVERY_INCIDENT as PREFLIGHT, MAINTENANCE_PREFLIGHT_RECOVERY_AUTHORIZATION as PREFLIGHT_AUTH,
  createMaintenancePreflightRecoveryInspection, buildMaintenancePreflightRecoveredState,
  validateMaintenancePreflightRecoveryPredecessor } from "./production-maintenance-preflight-recovery.mjs";
import { MAINTENANCE_PRELAUNCH_RECOVERY_INCIDENT as PRELAUNCH, createMaintenancePrelaunchRecoveryInspection, buildMaintenancePrelaunchRecoveredState } from "./production-maintenance-prelaunch-recovery.mjs";
...
function assembleV10(f) {
 const now=Date.parse("2026-09-14T16:21:11Z"), state=f.state;
 const context={operationId:U,previousTargetSha:PRELAUNCH.previousTargetSha,targetSha:PREFLIGHT.previousTargetSha,
  expectedOldSha:O,expectedRevision:PRELAUNCH.revision,expectedDigest:PRELAUNCH.stateDigest,bootId:BOOT,now,
 ...
 const evidence={...createMaintenancePrelaunchRecoveryInspection(state,context),toolsSha:context.targetSha,
  prelaunchRecoveryRunId:PREFLIGHT.priorRecoveryRunId,prelaunchRecoveryRunAttempt:1,mainCIrunId:PREFLIGHT.priorMainCIrunId,
  historyDigest:"d".repeat(64),historyCheckedAt:now-1000};
 f.state=copy(buildMaintenancePrelaunchRecoveredState(state,evidence,context));
 f.state.phase="failed-held";f.state.revision=38;f.state.ingress.padding="";
 const remaining=PREFLIGHT.stateBytes-Buffer.byteLength(JSON.stringify(f.state));assert(remaining>0);
 f.state.ingress.padding="q".repeat(remaining);f.io.now=()=>NOW;return f;
}
async function recover(f) {
  const stoppedBaseline = await capturePreflightRecoveryBaseline(f.state, f.io);
  const context = { operationId: U, previousTargetSha: PREFLIGHT.previousTargetSha, targetSha: "f".repeat(40), expectedOldSha: O,
    expectedRevision: PREFLIGHT.revision, expectedDigest: PREFLIGHT.stateDigest, bootId: BOOT, now: NOW,
    sourceDiffDigest: "b".repeat(64), migrationDigest: "c".repeat(64), stoppedBaseline };
  f.state = copy(buildMaintenancePreflightRecoveredState(f.state, { ...createMaintenancePreflightRecoveryInspection(f.state, context),
    toolsSha: context.targetSha, preflightRecoveryRunId: "34900000001", preflightRecoveryRunAttempt: 1, mainCIrunId: "34900000002",
...
test("exact failed v9 and flat recovered v10 compose actual O/T5/T6/T7 read-only proofs", { concurrency: false }, async t => {
  ...
  const seed9 = assembleV9(seed8); mappings.set(JSON.stringify(seed9.state), PRELAUNCH.stateDigest);
  const seed = assembleV10(seed9); mappings.set(JSON.stringify(seed.state), PREFLIGHT.stateDigest);
  const fixture = () => assembleV10(assembleV9(assembleV8(assembleV7(assembleV6(fixtureV5())))));
  const reject = promise => assert.rejects(promise, /preflight_(?:inspection_unverified|handoff_unverified)/);
  await t.test("only five exact synthetic states map, real private state is never used", () => {
    assert.equal(crypto.createHash("sha256").update(JSON.stringify(seed.state)).digest("hex"), PREFLIGHT.stateDigest);
  ...
  await t.test("capture observes all four real frozen directories/generations and port; signed baseline timestamp retained", async () => {
    const f = fixture(), bytes = JSON.stringify(f.state), baseline = await capturePreflightRecoveryBaseline(f.state, f.io);
    ...
    f.io.now = () => NOW + 1; assert.equal(await verifyPreflightRecoveryBaseline(f.state, baseline, f.io), true); assert.equal(baseline.observedAt, NOW);
  });
  await t.test("O web/worker, T5, T6 and T7 generations or descendants cannot revive", async () => {
    for (const select of [f => f.state.runtime.web.processes[0], f => f.state.runtime.worker.managed.processes[0],
      ...
      const f = fixture(), fact = select(f); f.facts.set(fact.pid, fact); await reject(capturePreflightRecoveryBaseline(f.state, f.io));
    }
  });
  await t.test("later full-stop assertion does not consult current; it still requires empty port", async () => {
    ...
    assert.equal(await assertPreflightRecoveryStopped(f.state, f.io), true);
    f.io.runtime.portEmpty = () => false; await reject(assertPreflightRecoveryStopped(f.state, f.io));
  });
  await t.test("generation-only checks allow T8 listener and same app, but exclude all histories and registry drift", async () => {
    ...
    assert.equal(await assertPreflightRecoveryGenerationsStopped(good.f.state, good.f.io), true);
    ...
    ]) { const changed = make(); mutate(changed); await reject(assertPreflightRecoveryGenerationsStopped(changed.f.state, changed.f.io)); }
    for (const sha of [O, T5, T6, T7]) {
      const changed = make(); changed.row.pm2_env.pm_exec_path = changed.f.disks.get(sha).nextEntryPath;
      await reject(assertPreflightRecoveryGenerationsStopped(changed.f.state, changed.f.io));
      ...
      await reject(assertPreflightRecoveryGenerationsStopped(descendant.f.state, descendant.f.io));
    }
  });
  await t.test("each original/candidate disk and environment is checked in full-stop and generation-only paths", async () => {
    for (const observe of [capturePreflightRecoveryBaseline, assertPreflightRecoveryGenerationsStopped]) {
      for (const sha of [O, T5, T6, T7]) {
  ...
  await t.test("fresh daemon/boot, registry and current observations cannot drift within a check", async () => {
    ...
    await reject(capturePreflightRecoveryBaseline(current.state, current.io));
    const registry = fixture(); let registryReads = 0;
    registry.io.runtime.pm2Registry = async () => ++registryReads === 4 ? [{ malformed: true }] : [];
    await reject(capturePreflightRecoveryBaseline(registry.state, registry.io));
  ...
  await t.test("T7 handoff returns only 27 physical/configuration fields and original O worker policy", async () => {
    const f = fixture(), baseline = await capturePreflightRecoveryBaseline(f.state, f.io), bytes = JSON.stringify(f.state);
    const fields = await readPreflightRecoveryHandoffFields(f.state.runtime, f.state, baseline, f.io);
    ...
    await reject(readPreflightRecoveryHandoffFields({ ...f.state.runtime, bootId: "bad" }, f.state, baseline, f.io));
    let n = 0; f.io.readCurrent = () => ({ ...f.current(), linkIdentity: id(++n < 3 ? 92 : 93, "link") });
    await reject(readPreflightRecoveryHandoffFields(f.state.runtime, f.state, baseline, f.io));
  });
  await t.test("wrong current, file/env drift, boot, journal, audit, getters and mutations reject", async () => {
    ...
    ]) { const f = fixture(); f.state = copy(f.state); mutate(f); await reject(capturePreflightRecoveryBaseline(f.state, f.io)); }
    ...
    await reject(capturePreflightRecoveryBaseline(hostile, f.io)); assert.equal(touched, 0);
    await reject(capturePreflightRecoveryBaseline(new Proxy(f.state, {}), f.io));
    await reject(capturePreflightRecoveryBaseline(f.state, { ...f.io, runtime: { ...f.io.runtime, pm2Control() { assert.fail(); } } }));
  });
  await t.test("actual deadline is enforced before and after observations; shape never authorizes extension", async () => {
    const f = fixture(), baseline = await capturePreflightRecoveryBaseline(f.state, f.io);
    ...
    assert.throws(() => validatePreflightRecoveryBaseline({ ...baseline, version: 1 }), /preflight_inspection_unverified/);
    for (const tooEarly of ["2026-09-14T10:00:00Z", "2026-09-14T15:00:31.999Z"]) {
      f.io.now = () => Date.parse(tooEarly); await reject(capturePreflightRecoveryBaseline(f.state, f.io));
    }
    f.io.now = () => Date.parse("2026-09-14T20:00:00Z"); await reject(capturePreflightRecoveryBaseline(f.state, f.io));
    let n = 0; f.io.now = () => ++n === 1 ? NOW : Date.parse("2026-09-14T20:00:00Z");
    await reject(capturePreflightRecoveryBaseline(f.state, f.io));
    n = 0; await reject(assertPreflightRecoveryGenerationsStopped(f.state, f.io));
    f.io.now = () => NOW - 1; await reject(verifyPreflightRecoveryBaseline(f.state, baseline, f.io));
  });
  ...
  await t.test("v10 reconstructs the exact failed v9 without changing audits and all real checks use the new clock", async () => {
    ...
    await recover(f);
    assert.equal(f.state.version, 11); assert.equal(f.state.activeAttempt, 3);
    assert.equal(JSON.stringify(f.state.budgetRecovery), oldAudit);
    assert.equal(f.state.preflightRecovery.predecessor.state, undefined);
    assert.equal(f.state.preflightRecovery.predecessor.stateDigest, PREFLIGHT.stateDigest);
    assert.equal(f.state.preflightRecovery.stoppedBaseline.observedAt, NOW);
    const baseline = await capturePreflightRecoveryBaseline(f.state, f.io);
    assert.equal(await verifyPreflightRecoveryBaseline(f.state, baseline, f.io), true);
    assert.equal(await assertPreflightRecoveryStopped(f.state, f.io), true);
    assert.equal(await assertPreflightRecoveryGenerationsStopped(f.state, f.io), true);
    assert.equal(Object.keys(await readPreflightRecoveryHandoffFields(f.state.runtime, f.state, baseline, f.io)).length, 27);
    assert.equal(JSON.stringify(f.state.budgetRecovery), oldAudit);
    assert.notEqual(JSON.stringify(f.state), previous);
    f.io.now = () => PREFLIGHT_AUTH.expiresAt;
    await reject(assertPreflightRecoveryStopped(f.state, f.io));
    await reject(assertPreflightRecoveryGenerationsStopped(f.state, f.io));
  });
  await t.test("old baseline cannot be reused and all boundary clocks are real, including post-current read", async () => {
    const f = fixture();
    await reject(verifyPreflightRecoveryBaseline(f.state, f.state.budgetRecovery.stoppedBaseline, f.io));
    f.io.now = () => PREFLIGHT_AUTH.authorizedAt;
    assert.equal((await capturePreflightRecoveryBaseline(f.state, f.io)).observedAt, PREFLIGHT_AUTH.authorizedAt);
    f.io.now = () => PREFLIGHT_AUTH.expiresAt - 1;
    assert.equal((await capturePreflightRecoveryBaseline(f.state, f.io)).observedAt, PREFLIGHT_AUTH.expiresAt - 1);
    let now = NOW, currentReads = 0; f.io.now = () => now;
    f.io.readCurrent = () => { if (++currentReads === 2) now = PREFLIGHT_AUTH.expiresAt; return copy(f.current()); };
    await reject(capturePreflightRecoveryBaseline(f.state, f.io));
    assert.equal(currentReads, 2);
    assert.throws(() => validateMaintenancePreflightRecoveryPredecessor(f.state, { bootId: BOOT, now }));
  });
  await t.test("renewal may refresh only time, not re-anchor a same-target link or a different stable registry", async () => {
    for (const renewed of [false, true]) {
      ...
      await reject(capturePreflightRecoveryBaseline(link.state, link.io));
  ...
  await t.test("read-only source cannot acquire locks or actuate and covers all historical candidates", () => {
    const source = readFileSync(new URL("./production-maintenance-preflight-inspection.mjs", import.meta.url), "utf8");
