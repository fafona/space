...
// Startup recovery and bounded diagnostic changes only. Business source,
// dependencies, migrations and prior immutable source/SQL audits stay unchanged.
export const MAINTENANCE_STARTUP_SOURCE_PATHS = Object.freeze([
  ...
  "scripts/production-maintenance-startup-recovery.mjs",
  ...
  "scripts/production-maintenance-startup-inspection.mjs",
  ...
  "scripts/production-maintenance-startup-control.test.mjs",
...
export const MAINTENANCE_STARTUP_MIGRATION_WINDOW = Object.freeze({
  runId: "34721155156", attempt: 1, startedAt: "2026-09-12T21:52:38.000Z", endedBefore: "2026-09-12T21:52:46.000Z",
});
const PREVIOUS = "e83c91abbf8e0d327708bd0b04c3a33ed423ab91", CREATED_AT = 1789236034129;
...
const fail = () => { throw new Error("maintenance_startup_evidence_unverified"); };
...
export function readMaintenanceStartupSourceProof(request, executeFixed = runGit) {
  try {
    ...
    if (!fields.length || fields.length % 2 || fields.length > MAINTENANCE_STARTUP_SOURCE_PATHS.length * 2) fail();
    for (let index = 0; index < fields.length; index += 2) {
      const match = /^:(000000|100644|100755) (100644|100755) ([0-9a-f]{40}) ([0-9a-f]{40}) ([AM])$/.exec(fields[index]), file = fields[index + 1];
      if (!match || !MAINTENANCE_STARTUP_SOURCE_PATHS.includes(file) || changes.some(value => value.path === file)) fail();
...
export const MAINTENANCE_STARTUP_MIGRATION_SQL = "BEGIN READ ONLY; SET LOCAL search_path=pg_catalog; SET LOCAL timezone='UTC'; " +
...
export function validateMaintenanceStartupMigrationProof(value, databaseOid, createdAt, request, executeFixed = runGit) {
  try {
    ...
    const start = BigInt(Date.parse(MAINTENANCE_STARTUP_MIGRATION_WINDOW.startedAt)) * 1000n;
    const end = BigInt(Date.parse(MAINTENANCE_STARTUP_MIGRATION_WINDOW.endedBefore)) * 1000n;
