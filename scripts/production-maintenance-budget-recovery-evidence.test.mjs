...
import { MAINTENANCE_BUDGET_RECOVERY_SOURCE_PATHS, MAINTENANCE_BUDGET_RECOVERY_MIGRATION_WINDOW,
  MAINTENANCE_BUDGET_RECOVERY_MIGRATION_SQL, readMaintenanceBudgetRecoverySourceProof,
  validateMaintenanceBudgetRecoveryMigrationProof } from "./production-maintenance-budget-recovery-evidence.mjs";
const previousTargetSha = "d9de5fe689226fcdd13a1e95039901b5d0f39167", targetSha = "f".repeat(40);
...
const safeError = /^Error: maintenance_budget_recovery_evidence_unverified$/;
...
const validate = (value, execute = git().run) => validateMaintenanceBudgetRecoveryMigrationProof(value, 5, createdAt, request(), execute);

test("source allowlist is fixed to this post-launch budget/attempt delta and both executing target checks are real", () => {
  const f = git(), result = readMaintenanceBudgetRecoverySourceProof(request(), f.run);
  assert.match(result.sourceDiffDigest, /^[a-f0-9]{64}$/); assert.deepEqual(result.sourceChangedPaths, [file]);
  assert.equal(MAINTENANCE_BUDGET_RECOVERY_SOURCE_PATHS.length, 27); assert.ok(Object.isFrozen(MAINTENANCE_BUDGET_RECOVERY_SOURCE_PATHS));
...
test("source hash binds every blob but canonicalizes order; app, SQL, deps, old recovery and CI changes are refused", () => {
  const second = "scripts/production-maintenance-budget-recovery.mjs";
  const proof = raw => readMaintenanceBudgetRecoverySourceProof(request(), git(args => args[0] === "diff" ? raw : undefined).run);
...
test("source drift, dirty tree, unknown base and hostile descriptor errors remain fixed and private", () => {
  for (const change of [(args, calls) => args[0] === "rev-parse" && calls.length > 4 ? previousTargetSha + "\n" : undefined,
    ...
    assert.throws(() => readMaintenanceBudgetRecoverySourceProof(request(), git(change).run), safeError);
  }
  let accessed = 0;
  for (const value of [{ ...request(), previousTargetSha: "e".repeat(40) }, { ...request(), directory: "/arbitrary" }, new Proxy(request(), {}),
    { get targetSha() { accessed++; return targetSha; }, previousTargetSha }]) assert.throws(() => readMaintenanceBudgetRecoverySourceProof(value, git().run), safeError);
  assert.equal(accessed, 0);
});
test("fixed complete readonly SQL and exact T7/new-target migration trees verify all 56 registrations through 048", () => {
  assert.match(MAINTENANCE_BUDGET_RECOVERY_MIGRATION_SQL, /^BEGIN READ ONLY; SET LOCAL search_path=pg_catalog;/);
  assert.match(MAINTENANCE_BUDGET_RECOVERY_MIGRATION_SQL, /SET LOCAL row_security=off/);
  assert.match(MAINTENANCE_BUDGET_RECOVERY_MIGRATION_SQL, /ORDER BY version/);
  assert.match(MAINTENANCE_BUDGET_RECOVERY_MIGRATION_SQL, /FROM public\.faolla_schema_migrations/);
  assert.match(MAINTENANCE_BUDGET_RECOVERY_MIGRATION_SQL, /ROLLBACK;$/);
  assert.doesNotMatch(MAINTENANCE_BUDGET_RECOVERY_MIGRATION_SQL, /\b(?:UPDATE|INSERT|DELETE|CREATE|ALTER|DROP|LIMIT)\b/);
  ...
  assert.equal(MAINTENANCE_BUDGET_RECOVERY_MIGRATION_WINDOW.runId, "34721155156");
});
test("registry rejects missing/extra/non-prefix/renamed rows, changed source tree, invalid identity and any unknown columns", () => {
  ...
  assert.throws(() => validateMaintenanceBudgetRecoveryMigrationProof(ledger(), 5, createdAt + 1, request(), git().run), safeError);
});
...
test("every explicit allowed file is accepted individually, no broad source prefix or permission change", () => {
  for (const path of MAINTENANCE_BUDGET_RECOVERY_SOURCE_PATHS) {
    const f = git(args => args[0] === "diff" ? diff(path) : undefined);
    assert.deepEqual(readMaintenanceBudgetRecoverySourceProof(request(), f.run).sourceChangedPaths, [path]);
  }
  for (const path of ["package.json", "scripts/production-maintenance-runtime.test.mjs", "scripts/production-maintenance-ingress.mjs",
    ...
    assert.throws(() => readMaintenanceBudgetRecoverySourceProof(request(), git(args => args[0] === "diff" ? diff(path) : undefined).run), safeError);
  }
  assert.throws(() => readMaintenanceBudgetRecoverySourceProof(request(), git(args => args[0] === "diff" ? diff().replace("100644 100644", "100644 100755") : undefined).run), safeError);
});
