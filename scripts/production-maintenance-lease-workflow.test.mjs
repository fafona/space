...
import { MAINTENANCE_LEASE_PRIOR_RUNS as SPECS, createMaintenanceLeaseWorkflowEvidence } from "./production-maintenance-lease-workflow.mjs";
import { MAINTENANCE_LEASE_AUTHORIZATION as AUTH, MAINTENANCE_LEASE_PREDECESSOR as PIN, validateMaintenanceLeaseInspection } from "./production-maintenance-lease.mjs";
const NOW = Date.parse("2026-09-14T20:30:00Z"), TARGET = "a".repeat(40), SELF = "99999999999", CI = "99999999998";
...
function inspected() {
  ...
  return validateMaintenanceLeaseInspection({ version: 1, state: "lease-renewal-inspected", operationId: AUTH.operationId, targetSha: TARGET,
    previousTargetSha: PIN.targetSha, expectedOldSha: PIN.expectedOldSha, revision: 39, stateDigest: PIN.stateDigest, stateBytes: PIN.stateBytes,
...
const env = () => ({ GITHUB_REPOSITORY: "fafona/space", GITHUB_EVENT_NAME: "workflow_dispatch", GITHUB_REF: "refs/heads/main",
  ...
  EXPECTED_OLD_SHA: PIN.expectedOldSha, MAINTENANCE_OPERATION_ID: AUTH.operationId, ACTION: "renew-lease", CONFIRMATION: "RENEW_PRODUCTION_MAINTENANCE_LEASE" });
function fixture(mutate = (_key, value) => value) {
  ...
  const self = repo({ ...SPECS[1].run, id: Number(SELF), head_sha: TARGET, status: "in_progress", conclusion: null,
    created_at: "2026-09-14T20:29:00Z", run_started_at: "2026-09-14T20:29:00Z", updated_at: "2026-09-14T20:29:01Z" });
  const ci = repo({ ...SPECS[2].run, id: Number(CI), head_sha: TARGET,
    created_at: "2026-09-14T20:10:00Z", run_started_at: "2026-09-14T20:10:00Z", updated_at: "2026-09-14T20:28:00Z" });
  db.set("run:" + SELF, self); db.set("ci", { total_count: 1, workflow_runs: [ci] }); db.set("main", { sha: TARGET });
  db.set("jobs:" + CI, { total_count: 10, jobs: SPECS[2].jobs.map((j, i) => ({ ...j, id: 10000 + i, run_id: Number(CI), head_sha: TARGET,
    started_at: "2026-09-14T20:10:05Z", completed_at: "2026-09-14T20:27:59Z" })) });
...
const check = (mutate, patch = {}, now = NOW, clock = () => NOW) => createMaintenanceLeaseWorkflowEvidence(inspected(), { ...env(), ...patch }, fixture(mutate), now, clock);
test("lease workflow binds cancelled backup as history only and exact new main CI", async () => {
  const result = await check(); assert.equal(result.leaseRunId, SELF); assert.equal(result.activeAttempt, 3);
  assert.equal(SPECS[0].run.conclusion, "cancelled"); assert.equal(SPECS[0].run.id, 34886165604);
