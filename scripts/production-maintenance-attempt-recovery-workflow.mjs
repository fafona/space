...
import { validateMaintenanceAttemptRecoveryInspection, validateMaintenanceAttemptRecoveryEvidence, encodeMaintenanceAttemptRecoveryEvidence,
  MAINTENANCE_ATTEMPT_RECOVERY_DEADLINE_AUTHORIZATION, MAINTENANCE_ATTEMPT_RECOVERY_HISTORY_MAX_AGE_MS } from "./production-maintenance-attempt-recovery.mjs";
...
const WORKFLOWS = Object.freeze([
  ["database-backup.yml", "Encrypted Database Backup", "workflow_dispatch",
    [["34715932102", OLD_TARGET, "success"], ["34724943157", PREVIOUS_TARGET, "success"], ["34778579797", LAUNCHED_TARGET, "success"]]],
  ...
  ["ordinary-account-cutover-readiness.yml", "Ordinary Account Cutover Readiness", "workflow_dispatch",
    [["34721256683", OLD_TARGET, "success"], ["34728212357", PREVIOUS_TARGET, "success"], ["34781336277", LAUNCHED_TARGET, "success"]]],
  ["deploy.yml", "Deploy Production", "workflow_run",
    [["34721317710", OLD_TARGET, "failure"], ["34728263285", PREVIOUS_TARGET, "failure"], ["34781392661", LAUNCHED_TARGET, "failure"]]],
]);
const INCIDENT_ORDER = Object.freeze(["34715932102", "34721155156", "34721256683", "34721317710",
  "34724943157", "34728212357", "34728263285", "34778579797", "34781336277", "34781392661"]);
const fail = () => { throw new Error("maintenance_attempt_recovery_workflow_unverified"); };
...
function checkWorkflowClock(now) {
  if (!Number.isSafeInteger(now) || now < MAINTENANCE_ATTEMPT_RECOVERY_DEADLINE_AUTHORIZATION.authorizedAt ||
      now >= MAINTENANCE_ATTEMPT_RECOVERY_DEADLINE_AUTHORIZATION.expiresAt || now < ADDITIONAL.authorizedAt) fail();
}
 ...
 * This backup never replaces the original maintenance B5/R5 subjects. */
export function validateMaintenanceAttemptRecoveryAdditionalBackup(records, now) {
...
export function readMaintenanceAttemptRecoveryAdditionalBackup(directory) {
 ...
 * This function binds the verified subjects to the original operation and the fixed T5 backup/readiness runs;
 * it never relabels old backup/readiness evidence for the new target.
 */
export function validateMaintenanceAttemptRecoveryPriorBindings(inspection, records) {
  const checked = validateMaintenanceAttemptRecoveryInspection(inspection);
  ...
  for (const phase of ["backup", "readiness"]) {
    ...
    assertAdditionalProvenance(record.provenance, record.bytes, "production-maintenance-binding.json", {
      ...
      sourceSha: LAUNCHED_TARGET, event: "workflow_dispatch", runId,
    });
...
export async function inspectMaintenanceAttemptRecoveryHistory(inspection, api, now) {
  const checked = validateMaintenanceAttemptRecoveryInspection(inspection);
  checkWorkflowClock(now);
...
export async function createMaintenanceAttemptRecoveryWorkflowEvidence(inspection, env, api, priorBindings, now = Date.now(), additionalRecords) {
  const checked = validateMaintenanceAttemptRecoveryInspection(inspection);
  checkWorkflowClock(now);
  if (env.GITHUB_REPOSITORY !== REPOSITORY || env.GITHUB_EVENT_NAME !== "workflow_dispatch" || env.GITHUB_REF !== "refs/heads/main" ||
      ...
      env.MAINTENANCE_OPERATION_ID !== checked.operationId || env.ACTION !== "recover-attempt" || env.CONFIRMATION !== "RECOVER_ATTEMPT_PRODUCTION_MAINTENANCE_UNTIL_20260914T040000Z") fail();
  if (checked.stoppedBaseline.observedAt > now || now - checked.stoppedBaseline.observedAt > MAINTENANCE_ATTEMPT_RECOVERY_HISTORY_MAX_AGE_MS) fail();
  const bindings = validateMaintenanceAttemptRecoveryPriorBindings(checked, priorBindings);
  const additionalBackup = validateMaintenanceAttemptRecoveryAdditionalBackup(additionalRecords, now);
  ...
  const history = await inspectMaintenanceAttemptRecoveryHistory(checked, api, now);
  ...
  if ((await api(`repos/${REPOSITORY}/commits/main`))?.sha !== checked.targetSha) fail();
  return validateMaintenanceAttemptRecoveryEvidence({ ...checked, toolsSha: checked.targetSha,
    attemptRecoveryRunId: env.GITHUB_RUN_ID, attemptRecoveryRunAttempt: 1, mainCIrunId,
    historyDigest: hash({ history, bindings, additionalBackup, artifacts, priorRecovery }), historyCheckedAt: now });
}
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  try {
    ...
    const additionalBackup = readMaintenanceAttemptRecoveryAdditionalBackup(process.argv[7]);
    const evidence = await createMaintenanceAttemptRecoveryWorkflowEvidence(inspection, process.env, api, priorBindings, Date.now(), additionalBackup);
    const completedAt = Date.now(); checkWorkflowClock(completedAt);
    if (completedAt < evidence.historyCheckedAt || completedAt - evidence.historyCheckedAt > MAINTENANCE_ATTEMPT_RECOVERY_HISTORY_MAX_AGE_MS) fail();
    if (completedAt < evidence.stoppedBaseline.observedAt || completedAt - evidence.stoppedBaseline.observedAt > MAINTENANCE_ATTEMPT_RECOVERY_HISTORY_MAX_AGE_MS) fail();
    const encoded = encodeMaintenanceAttemptRecoveryEvidence(evidence);
    process.stdout.write(`::add-mask::${encoded}\n`);
    appendFileSync(process.env.GITHUB_OUTPUT, `attempt_recovery_evidence=${encoded}\n`);
    process.stdout.write("maintenance_attempt_recovery_history_verified\n");
  } catch { process.stderr.write("maintenance_attempt_recovery_workflow_unverified\n"); process.exitCode = 1; }
}
