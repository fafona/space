...
import { MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION as AUTHORIZATION,
  validateMaintenanceWindowRenewalPredecessor, reconstructMaintenanceWindowRenewalPredecessor } from "./production-maintenance-window-renewal.mjs";
 ...
 * The pure window validator alone verifies prior audits at their recorded times;
 ...
 */
const ERROR = "production_maintenance_window_inspection_unverified";
...
export function validateWindowRenewalBaseline(raw) {
...
function authorizedPredecessor(raw, now) {
  ...
  if (value.version === 8) return validateMaintenanceWindowRenewalPredecessor(value, clock);
  if (value.version === 9) return reconstructMaintenanceWindowRenewalPredecessor(value, clock);
  fail();
}
...
export async function assertWindowRenewalStopped(rawState, overrides = {}) {
...
export async function assertWindowRenewalGenerationsStopped(rawState, overrides = {}) {
  try {
...
export async function captureWindowRenewalBaseline(rawState, overrides = {}) {
  try {
    ...
    const baseline = validateWindowRenewalBaseline({ version: 3, stateDigest: PIN, candidateDigest: hash(state.candidate),
    ...
    const previous = predecessor.budgetRecovery.stoppedBaseline;
    if (!isDeepStrictEqual({ ...baseline, observedAt: previous.observedAt }, previous)) fail();
    return baseline;
  } catch { fail(); }
}
export async function verifyWindowRenewalBaseline(state, rawBaseline, overrides = {}) {
  try {
    const baseline = validateWindowRenewalBaseline(rawBaseline);
    const fresh = await captureWindowRenewalBaseline(state, overrides);
