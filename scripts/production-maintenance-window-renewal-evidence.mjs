...
export const MAINTENANCE_WINDOW_RENEWAL_SOURCE_PATHS = Object.freeze([
  ...
  "scripts/production-maintenance-workflow-contract.mjs",
  "scripts/production-maintenance-workflow-contract.test.mjs",
  "scripts/production-maintenance-window-renewal.mjs",
  "scripts/production-maintenance-window-renewal.test.mjs",
  "scripts/production-maintenance-window-renewal-evidence.mjs",
  "scripts/production-maintenance-window-renewal-evidence.test.mjs",
  "scripts/production-maintenance-window-renewal-workflow.mjs",
  "scripts/production-maintenance-window-renewal-workflow.test.mjs",
  "scripts/production-maintenance-window-inspection.mjs",
  "scripts/production-maintenance-window-inspection.test.mjs",
  "scripts/production-maintenance-window-handoff.mjs",
  "scripts/production-maintenance-window-handoff.test.mjs",
  "scripts/production-maintenance-window-control.test.mjs",
...
const PREVIOUS = "3b6c55ea4397c4505e75c59bb3bb56d7dd7d1cc0", CREATED_AT = 1789236034129;
...
const fail = () => { throw new Error("maintenance_window_renewal_evidence_unverified"); };
...
export function readMaintenanceWindowRenewalSourceProof(request, executeFixed = runGit) {
  try {
    ...
    if (!fields.length || fields.length % 2 || fields.length > MAINTENANCE_WINDOW_RENEWAL_SOURCE_PATHS.length * 2) fail();
    for (let index = 0; index < fields.length; index += 2) {
      const match = /^:(000000|100644|100755) (100644|100755) ([0-9a-f]{40}) ([0-9a-f]{40}) ([AM])$/.exec(fields[index]), file = fields[index + 1];
      if (!match || !MAINTENANCE_WINDOW_RENEWAL_SOURCE_PATHS.includes(file) || changes.some(value => value.path === file)) fail();
...
export const MAINTENANCE_WINDOW_RENEWAL_MIGRATION_SQL = "BEGIN READ ONLY; SET LOCAL search_path=pg_catalog; SET LOCAL timezone='UTC'; " +
...
export function validateMaintenanceWindowRenewalMigrationProof(value, databaseOid, createdAt, request, executeFixed = runGit) {
  try {
    ...
    const start = BigInt(Date.parse(MAINTENANCE_WINDOW_RENEWAL_MIGRATION_WINDOW.startedAt)) * 1000n;
    const end = BigInt(Date.parse(MAINTENANCE_WINDOW_RENEWAL_MIGRATION_WINDOW.endedBefore)) * 1000n;
