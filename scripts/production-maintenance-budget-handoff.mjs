...
import { validateBudgetRecoveryBaseline, verifyBudgetRecoveryBaseline } from "./production-maintenance-budget-inspection.mjs";

// Only a private disk handoff for the exact stopped T7. No original runtime
...
const HISTORICAL_TARGETS = [T7, "3af8fa6ba6644593e10bef0a391389b2b34e926a", OLD, "f3104de19aa59e527c7b94a99850d151448da8cd",
...
const fail = () => { throw new Error("maintenance_budget_handoff_unverified"); };
...
function fields(raw, baseline) {
  ...
  if (value.PREVIOUS_LINK_TARGET !== baseline.current.target || value.PREVIOUS_RUNTIME_DIR !== baseline.current.target ||
      ...
      value.PREVIOUS_BUILD_ID !== T7 || value.PREVIOUS_BUILD_PREFIX !== T7.slice(0, 12) ||
...
export function validateBudgetRecoveryHandoffReport(raw, request) {
  try {
    ...
    if (value.version !== 3 || value.state !== "held" || value.operationId !== OPERATION || value.expectedOldSha !== OLD ||
    ...
    if (reference.version !== 3 || reference.predecessorStateDigest !== PIN || reference.previousTargetSha !== T7) fail();
...
export async function readBudgetRecoveryHandoffFields(originalRuntime, rawPredecessor, rawBaseline, observations = {}) {
  try {
    const baseline = validateBudgetRecoveryBaseline(rawBaseline);
    await verifyBudgetRecoveryBaseline(rawPredecessor, baseline, observations);
    ...
    // O is checked against O, not T7's legitimately changed environment.
