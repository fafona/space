...
import { MAINTENANCE_PRELAUNCH_RECOVERY_AUTHORIZATION as AUTHORIZATION,
  validateMaintenancePrelaunchRecoveryPredecessor, reconstructMaintenancePrelaunchRecoveryPredecessor } from "./production-maintenance-prelaunch-recovery.mjs";
 ...
 * The pure prelaunch validator alone verifies prior audits at their recorded times;
 ...
 */
const ERROR = "production_maintenance_prelaunch_inspection_unverified";
...
export function validatePrelaunchRecoveryBaseline(raw) {
...
function authorizedPredecessor(raw, now) {
  ...
  if (value.version === 9) return validateMaintenancePrelaunchRecoveryPredecessor(value, clock);
  if (value.version === 10) return reconstructMaintenancePrelaunchRecoveryPredecessor(value, clock);
  fail();
}
...
export async function assertPrelaunchRecoveryStopped(rawState, overrides = {}) {
...
export async function assertPrelaunchRecoveryGenerationsStopped(rawState, overrides = {}) {
  try {
...
export async function capturePrelaunchRecoveryBaseline(rawState, overrides = {}) {
  try {
    ...
    const baseline = validatePrelaunchRecoveryBaseline({ version: 3, stateDigest: PIN, candidateDigest: hash(state.candidate),
    ...
    const previous = predecessor.windowRenewal.stoppedBaseline;
    if (!isDeepStrictEqual({ ...baseline, observedAt: previous.observedAt }, previous)) fail();
    return baseline;
  } catch { fail(); }
}
export async function verifyPrelaunchRecoveryBaseline(state, rawBaseline, overrides = {}) {
  try {
    const baseline = validatePrelaunchRecoveryBaseline(rawBaseline);
    const fresh = await capturePrelaunchRecoveryBaseline(state, overrides);
