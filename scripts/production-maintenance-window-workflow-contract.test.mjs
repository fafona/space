...
const env = { GITHUB_REPOSITORY: "fafona/space", GITHUB_EVENT_NAME: "workflow_dispatch", GITHUB_REF: "refs/heads/main", GITHUB_RUN_ATTEMPT: "1",
  ACTION: "renew-window", CONFIRMATION: "RENEW_PRODUCTION_MAINTENANCE_UNTIL_20260914T160000Z", TARGET_SHA: "a".repeat(40), GITHUB_SHA: "a".repeat(40),
  MAINTENANCE_OPERATION_ID: "eb81284a-09c4-4514-8f16-38eaf6acc1e4", PREVIOUS_TARGET_SHA: "3b6c55ea4397c4505e75c59bb3bb56d7dd7d1cc0",
...
test("fixed renewal confirmation retains one current-main attempt and rejects alternate authority", () => {
...
test("unused-state inspection precedes fixed whole-history verification and only then the transition", () => {
  const names = ["Inspect Unused Window Renewal State", "Verify Exact Window Renewal History Under Production Lock", "Execute Fixed Maintenance Transition"];
  const positions = names.map(name => steps.indexOf(step(name))); assert.deepEqual([...positions].sort((a,b) => a-b), positions);
  for (const name of names.slice(0,2)) assert.equal(step(name).if, "inputs.action == 'renew-window'");
  const inspection = step(names[0]).run;
  assert.match(inspection, /umask 077/); assert.match(inspection, /inspect-window-renewal .*--target-sha .*--previous-target-sha .*--expected-old-sha .*--expected-operation-id/);
  assert.match(inspection, /> "\$capture_dir\/out" 2> "\$capture_dir\/err"/); assert.doesNotMatch(inspection, /cat |tee |renew-window --|fail-held/);
  assert.equal(step(names[1]).env.INSPECTION_DIR, "${{ steps.window-renewal-inspection.outputs.capture_dir }}");
  assert.match(step(names[1]).run, /node scripts\/production-maintenance-window-renewal-workflow\.mjs/);
});
test("renewal evidence is isolated; end still requires signed successful deployment and public verification", () => {
  ...
  assert.equal(transition.env.WINDOW_RENEWAL_EVIDENCE, "${{ steps.window-renewal-evidence.outputs.window_renewal_evidence }}");
  assert.match(transition.run, /renew-window\) command=renew-window; expected_state=held/);
  assert.match(transition.run, /test "\$\{#WINDOW_RENEWAL_EVIDENCE\}" -le 16384/);
  assert.match(transition.run, /--previous-target-sha "\$PREVIOUS_TARGET_SHA" --window-renewal-evidence "\$WINDOW_RENEWAL_EVIDENCE"/);
  assert.match(transition.run, /else\n\s+test -z "\$WINDOW_RENEWAL_EVIDENCE"/);
...
test("private cleanup stays scoped and every new shell block parses without any deployment", () => {
  const cleanup = step("Remove Runner Window Renewal Evidence");
  assert.equal(cleanup.if, "always() && inputs.action == 'renew-window'"); assert.deepEqual(Object.keys(cleanup.env), ["INSPECTION_DIR"]);
  assert.ok(cleanup.run.includes('"$RUNNER_TEMP"/maintenance-window-renewal.????????'));
  assert.doesNotMatch(cleanup.run, /rm -rf|sudo|ssh |state\.json|fail-held|renew-window --/);
  ...
  for (const item of steps.filter(value => value.if?.includes("renew-window") || ["Execute Fixed Maintenance Transition", "Validate Fixed Manual Transition"].includes(value.name)))
