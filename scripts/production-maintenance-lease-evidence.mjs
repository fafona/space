...
export const MAINTENANCE_LEASE_SOURCE_PATHS = Object.freeze([
  ...
  "scripts/production-maintenance-lease.mjs",
  "scripts/production-maintenance-lease.test.mjs",
  "scripts/production-maintenance-lease-inspection.mjs",
  "scripts/production-maintenance-lease-handoff.mjs",
  "scripts/production-maintenance-lease-evidence.mjs",
  "scripts/production-maintenance-lease-workflow.mjs",
  "scripts/production-maintenance-lease-workflow.test.mjs",
  "scripts/production-maintenance-lease-control.test.mjs",
  "scripts/production-maintenance-workflow-contract.test.mjs"
]);
export const MAINTENANCE_LEASE_MIGRATION_WINDOW = Object.freeze({
  runId: "34721155156", attempt: 1, startedAt: "2026-09-12T21:52:38.000Z", endedBefore: "2026-09-12T21:52:46.000Z",
});
const PREVIOUS = "3614f5bfc85cf72d064732141a9998a0bbaec513", CREATED_AT = 1789236034129;
...
const fail = () => { throw new Error("maintenance_lease_evidence_unverified"); };
...
export function readMaintenanceLeaseSourceProof(request, executeFixed = runGit) {
  try {
    ...
    if (!fields.length || fields.length % 2 || fields.length > MAINTENANCE_LEASE_SOURCE_PATHS.length * 2) fail();
    for (let index = 0; index < fields.length; index += 2) {
      const match = /^:(000000|100644|100755) (100644|100755) ([0-9a-f]{40}) ([0-9a-f]{40}) ([AM])$/.exec(fields[index]), file = fields[index + 1];
      if (!match || !MAINTENANCE_LEASE_SOURCE_PATHS.includes(file) || changes.some(value => value.path === file)) fail();
...
export const MAINTENANCE_LEASE_MIGRATION_SQL = "BEGIN READ ONLY; SET LOCAL search_path=pg_catalog; SET LOCAL timezone='UTC'; " +
...
export function validateMaintenanceLeaseMigrationProof(value, databaseOid, createdAt, request, executeFixed = runGit) {
  try {
    ...
    const start = BigInt(Date.parse(MAINTENANCE_LEASE_MIGRATION_WINDOW.startedAt)) * 1000n;
    const end = BigInt(Date.parse(MAINTENANCE_LEASE_MIGRATION_WINDOW.endedBefore)) * 1000n;
