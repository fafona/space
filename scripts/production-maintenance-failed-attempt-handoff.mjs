...
import { validateFailedAttemptBaseline, verifyFailedAttemptBaseline } from "./production-maintenance-failed-attempt-inspection.mjs";

// Only a private disk handoff for the exact stopped T6. No original runtime
...
const HISTORICAL_TARGETS = [T6, OLD, "f3104de19aa59e527c7b94a99850d151448da8cd",
...
const fail = () => { throw new Error("maintenance_failed_attempt_handoff_unverified"); };
...
function fields(raw, baseline) {
  ...
  if (value.PREVIOUS_LINK_TARGET !== baseline.current.target || value.PREVIOUS_RUNTIME_DIR !== baseline.current.target ||
      ...
      value.PREVIOUS_BUILD_ID !== T6 || value.PREVIOUS_BUILD_PREFIX !== T6.slice(0, 12) ||
...
export function validateFailedAttemptHandoffReport(raw, request) {
  try {
    const value = object(raw, ["version", "operationId", "targetSha", "expectedOldSha", "state", "fields", "attemptBaseline"]);
    ...
    if (value.version !== 2 || value.state !== "held" || value.operationId !== OPERATION || value.expectedOldSha !== OLD ||
    ...
    const reference = object(value.attemptBaseline, ["version", "predecessorStateDigest", "previousTargetSha", "stoppedBaseline"]);
    if (reference.version !== 2 || reference.predecessorStateDigest !== PIN || reference.previousTargetSha !== T6) fail();
    reference.stoppedBaseline = validateFailedAttemptBaseline(reference.stoppedBaseline);
    value.fields = fields(value.fields, reference.stoppedBaseline); value.attemptBaseline = Object.freeze(reference);
    if (Buffer.byteLength(JSON.stringify(value)) > 262144) fail();
    return Object.freeze(value);
  } catch { fail(); }
}

export async function readFailedAttemptHandoffFields(originalRuntime, rawPredecessor, rawBaseline, observations = {}) {
  try {
    const baseline = validateFailedAttemptBaseline(rawBaseline);
    await verifyFailedAttemptBaseline(rawPredecessor, baseline, observations);
    ...
    if (!isDeepStrictEqual(proof, rawPredecessor.runtime)) fail();
    // O is checked against O, not T6's legitimately changed environment.
    await runtime.readRuntimeHandoffEnvironment(proof, observations.runtime);
    const candidate = runtime.validateCandidateProof(rawPredecessor.candidate, proof);
    const candidateOnly = { version: 1, input: { ...proof.input, expectedOldSha: T6 }, bootId: proof.bootId,
    ...
    const result = fields(output, baseline);
    await verifyFailedAttemptBaseline(rawPredecessor, baseline, observations);
    return result;
  } catch { fail(); }
}
