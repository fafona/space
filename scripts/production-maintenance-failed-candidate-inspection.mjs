...
/** Read-only observation of ONE stopped, previously launched candidate.
 ...
 * stopped assertion: its real T5 disk/process fields are never relabelled O,
 * persisted as the original runtime, or used to call a process actuator.
 */
const ERROR = "production_maintenance_failed_candidate_unverified";
...
const TARGET = "f3104de19aa59e527c7b94a99850d151448da8cd";
...
function current(value) {
  if (!exact(value, ["target", "linkIdentity", "runtimeIdentity"]) || typeof value.target !== "string" ||
      ...
      !/^f3104de19aa5-[0-9]{14}$/.test(posix.basename(value.target))) fail();
  identity(value.linkIdentity, "link"); identity(value.runtimeIdentity, "directory");
  return value;
}
export function validateFailedCandidateBaseline(raw) {
  const value = bounded(raw);
  if (!exact(value, BASELINE_KEYS) || value.version !== 1 || value.stateDigest !== PIN || value.bootId !== BOOT ||
...
async function checkedState(raw, runtimeApi) {
  const state = bounded(raw);
  if (!exact(state, STATE_KEYS) || hash(state) !== PIN || state.version !== 5 || state.revision !== 15 || state.phase !== "failed-held" ||
...
async function prepare(rawState, overrides) {
  ...
  return { io, runtimeApi, ...await checkedState(rawState, runtimeApi) };
}
async function assertBothStopped({ io, runtimeApi, proof, candidate }) {
  ...
  if (await runtimeApi.assertRuntimeStopped(proof, runtimeIo) !== true) fail();
  ...
  if (boot() !== BOOT || observed.length < 2 || observed.some(value => value !== observed[0])) fail();
  return observed[0];
}
/** For a later active attempt whose current link has legitimately moved. This
 * still proves BOTH historical generations stopped, frozen files and no port
 * listener. It does not attest the new current target, ingress or permission. */
export async function assertFailedCandidateStopped(rawState, overrides = {}) {
  try { await assertBothStopped(await prepare(rawState, overrides)); return true; }
  catch { fail(); }
}
/** During a NEW candidate's lifetime only. This verifies the historical T5
...
export async function assertFailedCandidateGenerationStopped(rawState, overrides = {}) {
  try {
    const { io, runtimeApi, proof, candidate } = await prepare(rawState, overrides);
...
export async function captureFailedCandidateBaseline(rawState, overrides = {}) {
  try {
    ...
    const registryDigest = await assertBothStopped(prepared);
    const after = current(bounded(io.readCurrent(APP, candidate.disk.runtime)));
    if (!isDeepStrictEqual(before, after)) fail();
    return validateFailedCandidateBaseline({ version: 1, stateDigest: PIN, candidateDigest: hash(state.candidate),
...
export async function verifyFailedCandidateBaseline(state, rawBaseline, overrides = {}) {
  try {
    const baseline = validateFailedCandidateBaseline(rawBaseline);
    const fresh = await captureFailedCandidateBaseline(state, overrides);
