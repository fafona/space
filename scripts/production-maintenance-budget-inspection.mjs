...
import { validateMaintenanceBudgetRecoveryPredecessor } from "./production-maintenance-budget-recovery.mjs";
 ...
 */
const ERROR = "production_maintenance_budget_inspection_unverified";
...
export function validateBudgetRecoveryBaseline(raw) {
...
async function checkedState(raw, runtimeApi, now) {
  const state = validateMaintenanceBudgetRecoveryPredecessor(bounded(raw), { bootId: BOOT, now });
  ...
  return { state, proof, candidate, projections };
}
...
async function assertAllStopped({ io, runtimeApi, state, proof, projections }) {
  ...
  validateMaintenanceBudgetRecoveryPredecessor(state, { bootId: BOOT, now: io.now() });
  return observed[0];
}
export async function assertBudgetRecoveryStopped(rawState, overrides = {}) {
...
export async function assertBudgetRecoveryGenerationsStopped(rawState, overrides = {}) {
  try {
    const { io, runtimeApi, state, proof, projections } = await prepare(rawState, overrides);
...
export async function captureBudgetRecoveryBaseline(rawState, overrides = {}) {
  try {
    ...
    return validateBudgetRecoveryBaseline({ version: 3, stateDigest: PIN, candidateDigest: hash(state.candidate),
...
export async function verifyBudgetRecoveryBaseline(state, rawBaseline, overrides = {}) {
  try {
    const baseline = validateBudgetRecoveryBaseline(rawBaseline);
    const fresh = await captureBudgetRecoveryBaseline(state, overrides);
