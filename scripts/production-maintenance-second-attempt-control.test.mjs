...
import { MAINTENANCE_SECOND_ATTEMPT_RECOVERY_INCIDENT as SECOND, MAINTENANCE_SECOND_ATTEMPT_RECOVERY_AUTHORIZATION as AUTH,
  createMaintenanceSecondAttemptRecoveryInspection, buildMaintenanceSecondAttemptRecoveredState, encodeMaintenanceSecondAttemptRecoveryEvidence } from "./production-maintenance-second-attempt-recovery.mjs";
...
function fixture(seed) {
  const state = copy(seed), baseline = { version: 2, stateDigest: SECOND.stateDigest, candidateDigest: hash(state.candidate),
    launchDiskDigest: hash(state.launchDisk), launchJournalDigest: hash(state.launchJournal), runtimeDigest: hash(state.runtime),
    current: { target: state.launchDisk.runtime, linkIdentity: ID, runtimeIdentity: ID }, bootId: SECOND.bootId,
    pm2RegistryDigest: "a".repeat(64), observedAt: NOW - 1000 };
  const context = { operationId: SECOND.operationId, previousTargetSha: SECOND.previousTargetSha, targetSha: TARGET,
    expectedOldSha: SECOND.expectedOldSha, expectedRevision: 23, expectedDigest: SECOND.stateDigest, bootId: SECOND.bootId,
  ...
  const evidence = { ...inspection, toolsSha: TARGET, secondAttemptRecoveryRunId: "34790000001", secondAttemptRecoveryRunAttempt: 1,
...
function request(action, f) {
  const args = [action, "--app-dir", "/srv/faolla", "--app-name", "faolla", "--app-port", "3000", "--target-sha", TARGET,
    "--expected-old-sha", SECOND.expectedOldSha, "--expected-operation-id", SECOND.operationId, "--json"];
  if (["inspect-second-attempt-recovery", "recover-second-attempt"].includes(action)) args.push("--previous-target-sha", SECOND.previousTargetSha);
  if (action === "recover-second-attempt") args.push("--second-attempt-recovery-evidence", encodeMaintenanceSecondAttemptRecoveryEvidence(f.evidence));
...
function rehearsal(f, initial = f.state) {
  ...
  const validateState = state => {
    validateMaintenanceState(state, { ...state, action: state.version === 6 ? "inspect-second-attempt-recovery" : "check-held",
      previousTargetSha: SECOND.previousTargetSha }, SECOND.bootId, now);
  ...
  const ops = {
    load, save, now: () => now, bootId: () => SECOND.bootId, readToken: () => "synthetic-control-token",
    ...
    verifyIngress: record("ingress"), assertRuntimeStopped: record("originalStopped"), async assertFailedAttemptStopped(state) {
      assert.deepEqual(state, f.state); proofs(state); events.push("bothStopped");
      for (const name of ["O", "T5", "T6"]) { events.push("stopped:" + name); if (revived === name) throw new Error("synthetic " + name + " revival"); }
    },
    async assertFailedAttemptGenerationsStopped(state) {
      assert.deepEqual(state, f.state); proofs(state); events.push("historicalGenerationStopped");
      for (const name of ["O", "T5", "T6"]) { events.push("generation:" + name); if (revived === name) throw new Error("synthetic " + name + " revival"); }
    },
    assertDatabaseQuiet: record("databaseQuiet"),
    async readSecondAttemptRecoverySnapshot() { const state = await load(), snapshot = baselines.get(state); return { ...snapshot, state }; },
    captureFailedAttemptBaseline: record("captureBaseline", f.baseline),
    async verifyFailedAttemptBaseline(state, baseline) { events.push("verifyBaseline"); assert.deepEqual(state, f.state); assert.deepEqual(baseline, f.baseline); return true; },
    readSecondAttemptRecoverySourceProof: record("source", { sourceDiffDigest: f.context.sourceDiffDigest }),
    readSecondAttemptRecoveryMigrationProof: record("migration", f.context.migrationDigest),
    ...
    async recloseAttemptIngress(state) { events.push("recloseIngress"); assert.deepEqual(state.secondAttemptRecovery.predecessor.state, f.state); return state.ingress; },
    stopCandidate: record("stopCandidate"),
    stopRuntime: async () => { assert.fail("v7 cleanup must never resume or target original O"); },
...
test("isolated v6 to v7 controller, pure model and real journal storage composition", { concurrency: false }, async t => {
  ...
  const seed = syntheticV6(oldSeed); mappings.set(JSON.stringify(seed), SECOND.stateDigest);

  await t.test("inspection has zero writes/sends and returns the actual fixture baseline binding", async () => {
    const f = fixture(seed), r = rehearsal(f), report = await runMaintenanceAction(request("inspect-second-attempt-recovery", f), r.ops);
  ...
  await t.test("recovery uses one durable CAS and retains the whole consumed journal and four audits", async () => {
    const f = fixture(seed), r = rehearsal(f), next = buildMaintenanceSecondAttemptRecoveredState(f.state, f.evidence, f.context);
    assert.equal((await runMaintenanceAction(request("recover-second-attempt", f), r.ops)).state, "held");
    ...
    assert.deepEqual(r.saved().secondAttemptRecovery.predecessor.state.launchJournal, f.state.launchJournal);
  ...
  await t.test("wrong target, invalid evidence, stale history or failed host proof never writes", async () => {
    for (const alter of [
      ...
      (r, q) => { q.secondAttemptRecoveryEvidence.historyDigest = "invalid"; },
      (r, q) => { q.secondAttemptRecoveryEvidence.historyCheckedAt = NOW - 300001; },
      ...
      const f = fixture(seed), r = rehearsal(f), q = copy(request("recover-second-attempt", f)); alter(r, q);
  ...
  await t.test("second full proof round rechecks source, migration and real final deadline before CAS", async () => {
    for (const kind of ["source", "migration", "deadline"]) {
      ...
      if (kind === "source") r.ops.readSecondAttemptRecoverySourceProof = async () => ({ sourceDiffDigest: ++calls === 1 ? f.context.sourceDiffDigest : "e".repeat(64) });
      if (kind === "migration") r.ops.readSecondAttemptRecoveryMigrationProof = async () => ++calls === 1 ? f.context.migrationDigest : "e".repeat(64);
      if (kind === "deadline") r.ops.assertDatabaseQuiet = async () => { if (++calls === 2) r.setNow(AUTH.expiresAt); };
      await assert.rejects(runMaintenanceAction(request("recover-second-attempt", f), r.ops)); assert.equal(r.writes(), 0);
    }
  });
  await t.test("file fsync and post-rename uncertainty produce no recovery success and no send/retry", async () => {
    for (const stage of ["fileSync", "renameAfter", "directorySync"]) {
      ...
      await assert.rejects(runMaintenanceAction(request("recover-second-attempt", f), r.ops), /storage_unconfirmed/);
      ...
      assert.equal(r.saved().version, stage === "fileSync" ? 6 : 7); assert.equal(r.fds.size, 0);
    }
  });
  await t.test("later v7 start persists its own attempt before one send while both predecessors stay immutable", async () => {
    const f = fixture(seed), initial = buildMaintenanceSecondAttemptRecoveredState(f.state, f.evidence, f.context), r = rehearsal(f, initial);
  ...
  await t.test("any O/T5/T6 revival blocks inspect, recovery and launch before writes or sends", async () => {
    for (const name of ["O", "T5", "T6"]) {
      for (const action of ["inspect-second-attempt-recovery", "recover-second-attempt", "start-candidate"]) {
        const f = fixture(seed), initial = action === "start-candidate" ? buildMaintenanceSecondAttemptRecoveredState(f.state, f.evidence, f.context) : f.state;
      ...
      const f = fixture(seed), r = rehearsal(f, buildMaintenanceSecondAttemptRecoveredState(f.state, f.evidence, f.context));
      await runMaintenanceAction(request("start-candidate", f), r.ops); r.setRevived(name);
      for (const action of ["check-candidate", "snapshot-web", "snapshot-worker", "register-candidate", "candidate-handoff", "end"]) {
        r.clearEvents(); const writes = r.writes();
        await assert.rejects(runMaintenanceAction(request(action, f), r.ops), /revival/);
        assert.equal(r.writes(), writes);
        assert.equal(r.events.some(event => ["verifyCandidate", "handoff", "snapshot:web", "snapshot:worker", "resumeCandidate", "restoreIngress"].includes(event)), false);
      }
    }
  });
  await t.test("every candidate snapshot, check, registration and handoff excludes O/T5/T6 before reading T7", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceSecondAttemptRecoveredState(f.state, f.evidence, f.context));
    ...
    for (const action of ["snapshot-web", "snapshot-worker", "check-candidate", "register-candidate", "candidate-handoff"]) {
      r.clearEvents(); const writes = r.writes(); await runMaintenanceAction(request(action, f), r.ops);
      const effects = r.events.filter(event => ["proofs", "ingress", "historicalGenerationStopped", "verifyCandidate", "handoff", "snapshot:web", "snapshot:worker"].includes(event));
    ...
    for (const action of ["snapshot-web", "snapshot-worker", "check-candidate", "register-candidate", "candidate-handoff", "end"]) {
      r.clearEvents(); const writes = r.writes();
      r.ops.assertFailedAttemptGenerationsStopped = async () => { r.events.push("historicalGenerationStopped"); throw new Error("synthetic historical revival"); };
      await assert.rejects(runMaintenanceAction(request(action, f), r.ops), /historical revival/);
      assert.equal(r.writes(), writes);
      assert.equal(r.events.some(event => ["verifyCandidate", "handoff", "snapshot:web", "snapshot:worker", "resumeCandidate", "restoreIngress"].includes(event)), false);
    }
  });
  await t.test("end preserves all three journals and checks historical absence before dump, before reopen and before ended", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceSecondAttemptRecoveredState(f.state, f.evidence, f.context));
    ...
    assert.deepEqual(r.saved().secondAttemptRecovery.predecessor.state, f.state);
  ...
  await t.test("historical revival after resume or before reopen blocks restore and stops only the new exact resumed proof", async () => {
    for (const rejectAt of [2, 3]) {
      ...
      r.ops.assertFailedAttemptGenerationsStopped = async () => { r.events.push("historicalGenerationStopped");
      ...
      r.ops.assertFailedAttemptStopped = async () => { r.events.push("bothStopped"); throw new Error("synthetic historical remains alive"); };
      ...
      assert.equal(r.events.includes("persistDump"), rejectAt === 3); assert.deepEqual(r.saved().secondAttemptRecovery.predecessor.state, f.state);
    }
  });
  ...
  await t.test("unknown ACK is reconciled by an observation and stopped, never launched again", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceSecondAttemptRecoveredState(f.state, f.evidence, f.context)); r.setOutcome("unknown");
  ...
  await t.test("real storage refuses v7 journal reset, audit deletion and predecessor reuse", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceSecondAttemptRecoveredState(f.state, f.evidence, f.context));
    await runMaintenanceAction(request("start-candidate", f), r.ops);
    ...
    for (const alter of [s => { s.launchJournal = null; s.launchDisk = null; s.candidate = null; s.phase = "held"; },
      s => { delete s.secondAttemptRecovery; }, s => { s.secondAttemptRecovery.predecessor.state.launchJournal.slots["paused-web"].phase = "planned"; }]) {
