...
// Only this reviewed post-launch attempt/diagnostics task. No application,
// dependency, migration, compiler, runtime-control or ingress-rule change.
export const MAINTENANCE_SECOND_ATTEMPT_RECOVERY_SOURCE_PATHS = Object.freeze([
  ".github/workflows/deploy.yml",
  ".github/workflows/production-maintenance.yml",
  "docs/production-maintenance.md",
  "scripts/deploy-booking-diagnostics-transport.test.mjs",
  "scripts/deploy-production-contract.test.mjs",
  "scripts/deploy.production.sh",
  "scripts/maintenance-effective-public-gateway.test.mjs",
  "scripts/production-maintenance-control.mjs",
  "scripts/production-maintenance-deploy-read.mjs",
  "scripts/production-maintenance-deploy-read.test.mjs",
  "scripts/production-maintenance-failed-attempt-handoff.mjs",
  "scripts/production-maintenance-failed-attempt-handoff.test.mjs",
  "scripts/production-maintenance-failed-attempt-inspection.mjs",
  "scripts/production-maintenance-failed-attempt-inspection.test.mjs",
  ...
  "scripts/production-maintenance-second-attempt-control.test.mjs",
  "scripts/production-maintenance-second-attempt-reclose.test.mjs",
  "scripts/production-maintenance-second-attempt-recovery-evidence.mjs",
  "scripts/production-maintenance-second-attempt-recovery-evidence.test.mjs",
  "scripts/production-maintenance-second-attempt-recovery-workflow.mjs",
  "scripts/production-maintenance-second-attempt-recovery-workflow.test.mjs",
  "scripts/production-maintenance-second-attempt-recovery.mjs",
  "scripts/production-maintenance-second-attempt-recovery.test.mjs",
  "scripts/production-maintenance-second-attempt-workflow-contract.test.mjs",
  "scripts/production-maintenance-workflow-contract.test.mjs"
]);
export const MAINTENANCE_SECOND_ATTEMPT_RECOVERY_MIGRATION_WINDOW = Object.freeze({
  runId: "34721155156", attempt: 1, startedAt: "2026-09-12T21:52:38.000Z", endedBefore: "2026-09-12T21:52:46.000Z",
});
const PREVIOUS = "3af8fa6ba6644593e10bef0a391389b2b34e926a", CREATED_AT = 1789236034129;
...
const fail = () => { throw new Error("maintenance_second_attempt_recovery_evidence_unverified"); };
...
export function readMaintenanceSecondAttemptRecoverySourceProof(request, executeFixed = runGit) {
  try {
    ...
    if (!fields.length || fields.length % 2 || fields.length > MAINTENANCE_SECOND_ATTEMPT_RECOVERY_SOURCE_PATHS.length * 2) fail();
    for (let index = 0; index < fields.length; index += 2) {
      const match = /^:(000000|100644|100755) (100644|100755) ([0-9a-f]{40}) ([0-9a-f]{40}) ([AM])$/.exec(fields[index]), file = fields[index + 1];
      if (!match || !MAINTENANCE_SECOND_ATTEMPT_RECOVERY_SOURCE_PATHS.includes(file) || changes.some(value => value.path === file)) fail();
...
export const MAINTENANCE_SECOND_ATTEMPT_RECOVERY_MIGRATION_SQL = "BEGIN READ ONLY; SET LOCAL search_path=pg_catalog; SET LOCAL timezone='UTC'; " +
...
export function validateMaintenanceSecondAttemptRecoveryMigrationProof(value, databaseOid, createdAt, request, executeFixed = runGit) {
  try {
    ...
    const start = BigInt(Date.parse(MAINTENANCE_SECOND_ATTEMPT_RECOVERY_MIGRATION_WINDOW.startedAt)) * 1000n;
    const end = BigInt(Date.parse(MAINTENANCE_SECOND_ATTEMPT_RECOVERY_MIGRATION_WINDOW.endedBefore)) * 1000n;
