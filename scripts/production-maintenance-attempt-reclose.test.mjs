...
import { MAINTENANCE_ATTEMPT_RECOVERY_DEADLINE_AUTHORIZATION as AUTH } from "./production-maintenance-attempt-recovery.mjs";
...
function fixture() {
  ...
  const baseline = { version: 6, revision: 25, operationId: AUTH.operationId, targetSha: "e".repeat(40),
    ...
    attemptRecovery: { fixture: "immutable-original-predecessor-and-consumed-journal" },
  ...
  const store = {
    async readOperationUnderExistingOperationLock() {
      ...
      // Full actual pure validation is covered by attempt-control.test.mjs.
...
test("every fixed binding and unsupported request action rejects before host I/O", async () => {
  for (const key of ["version", "operationId", "targetSha", "expectedOldSha", "appDir", "appName", "appPort", "bootId", "createdAt", "runtime", "tokenHash", "attemptRecovery"]) {
  ...
  for (const action of ["prepare", "recover-attempt", "check-held", "runtime-handoff", "snapshot-web", ""] ) {
