...
import { MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION as AUTH } from "./production-maintenance-window-renewal.mjs";
...
function fixture() {
  ...
  const baseline = { version: 9, revision: 44, activeAttempt: 3, operationId: AUTH.operationId, targetSha: "e".repeat(40),
    ...
    windowRenewal: { fixture: "immutable-unused-v8-compact-predecessor-and-new-authority" },
  ...
  const store = {
    async readOperationUnderExistingOperationLock() {
      ...
      // Full actual pure validation is covered by window-control.test.mjs.
...
test("every fixed binding and unsupported request action rejects before host I/O", async () => {
  for (const key of ["version", "operationId", "targetSha", "expectedOldSha", "appDir", "appName", "appPort", "bootId", "createdAt", "runtime", "tokenHash", "activeAttempt", "attemptRecovery", "secondAttemptRecovery", "budgetRecovery", "windowRenewal"]) {
  ...
  for (const action of ["renew-window", "inspect-window-renewal", "prepare", "recover-attempt", "recover-second-attempt", "recover-budget", "inspect-budget-recovery", "snapshot-pair", "check-held", "runtime-handoff", "snapshot-web", ""] ) {
