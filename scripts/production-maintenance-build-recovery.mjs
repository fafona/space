 ...
 * not authorization. The fixed user-authorized extension below preserves the
 * original deadline as audit data; no creation time, audit or proof is replaced.
 */
...
function checkActualClock(clock) {
  checkClock(clock);
...
function checkContext(value) {
  ...
  checkActualClock({ bootId: value.bootId, now: value.now });
}
function checkPredecessor(state, clock) {
  checkActualClock(clock); checkOriginalContinuation(state, historicalAuditClock(clock));
...
function inspectionFor(state, context) {
  checkContext(context); checkPredecessor(state, { bootId: context.bootId, now: context.now });
 ...
 * predecessor exception. Ordinary v2/v3/v4 commands retain their old TTL. */
export function validateMaintenanceBuildRecoveryPredecessor(rawState, rawClock) {
  const state = bounded(rawState), clock = bounded(rawClock); checkPredecessor(state, clock); return freeze(state);
}
...
export function buildMaintenanceBuildRecoveredState(rawState, rawEvidence, rawContext) {
  ...
  const next = { ...state, version: 5, revision: state.revision + 1, targetSha: context.targetSha, phase: "held",
    buildRecovery: { version: 1, evidence, recoveredAt: context.now }, deadlineExtension: { ...EXTENSION } };
...
export function validateMaintenanceBuildRecoveryState(rawState, rawClock) {
  const state = bounded(rawState), clock = bounded(rawClock); checkActualClock(clock);
  if (!record(state) || state.version !== 5 || !exact(state.buildRecovery, ["version", "evidence", "recoveredAt"]) ||
      state.buildRecovery.version !== 1 || !time(state.buildRecovery.recoveredAt) || !isDeepStrictEqual(state.deadlineExtension, EXTENSION)) fail();
  ...
  if (state.targetSha !== evidence.targetSha || !integer(state.revision) || state.revision < INCIDENT.revision + 1 || recoveredAt > clock.now ||
      ...
      recoveredAt < EXTENSION.authorizedAt || recoveredAt >= EXTENSION.expiresAt) fail();
  ...
  const original = { ...state, version: 4, targetSha: evidence.previousTargetSha };
  delete original.buildRecovery; delete original.deadlineExtension;
  checkOriginalContinuation(original, historicalAuditClock(clock));
 ...
 * or the fixed deadline extension. This structural storage guard cannot read
 * actual time: the controller must validate the real clock immediately before
 * persistence and every operational action, never reuse the audit clock.
...
export function assertMaintenanceBuildRecoveryProgress(rawPrevious, rawNext) {
  ...
  if (previous.version !== 5 && next.version !== 5) {
    if ([previous, next].some(state => Object.hasOwn(state, "buildRecovery") || Object.hasOwn(state, "deadlineExtension"))) fail();
