...
function fixture() {
  ...
  const stoppedBaseline = { version: 3, stateDigest: pin, candidateDigest: "a".repeat(64), launchDiskDigest: "b".repeat(64),
    ...
    bootId: "e6531ec9-db4a-4216-b87a-7cc858197eaa", pm2RegistryDigest: "e".repeat(64), observedAt: Date.parse("2026-09-14T07:20:00Z") };
...
const rejects = (value, bound = request) => assert.throws(() => validateBudgetRecoveryHandoffReport(value, bound), { message: "maintenance_budget_handoff_unverified" });
test("small report retains fixed T7 physical baseline without complete predecessors", () => {
  const value = fixture(), result = validateBudgetRecoveryHandoffReport(value, request);
