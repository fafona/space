...
// Only this reviewed post-launch budget/pair-observation and recovery task.
// No application, dependency, migration, compiler, PM2 actuation or ingress-rule change.
export const MAINTENANCE_BUDGET_RECOVERY_SOURCE_PATHS = Object.freeze([
  ...
  "scripts/production-maintenance-budget-control.test.mjs",
  "scripts/production-maintenance-budget-handoff.mjs",
  "scripts/production-maintenance-budget-handoff.test.mjs",
  "scripts/production-maintenance-budget-inspection.mjs",
  "scripts/production-maintenance-budget-inspection.test.mjs",
  "scripts/production-maintenance-budget-reclose.test.mjs",
  "scripts/production-maintenance-budget-recovery-evidence.mjs",
  "scripts/production-maintenance-budget-recovery-evidence.test.mjs",
  "scripts/production-maintenance-budget-recovery-workflow.mjs",
  "scripts/production-maintenance-budget-recovery-workflow.test.mjs",
  "scripts/production-maintenance-budget-recovery.mjs",
  "scripts/production-maintenance-budget-recovery.test.mjs",
  "scripts/production-maintenance-budget-workflow-contract.test.mjs",
...
export const MAINTENANCE_BUDGET_RECOVERY_MIGRATION_WINDOW = Object.freeze({
  runId: "34721155156", attempt: 1, startedAt: "2026-09-12T21:52:38.000Z", endedBefore: "2026-09-12T21:52:46.000Z",
});
const PREVIOUS = "d9de5fe689226fcdd13a1e95039901b5d0f39167", CREATED_AT = 1789236034129;
...
const fail = () => { throw new Error("maintenance_budget_recovery_evidence_unverified"); };
...
export function readMaintenanceBudgetRecoverySourceProof(request, executeFixed = runGit) {
  try {
    ...
    if (!fields.length || fields.length % 2 || fields.length > MAINTENANCE_BUDGET_RECOVERY_SOURCE_PATHS.length * 2) fail();
    for (let index = 0; index < fields.length; index += 2) {
      const match = /^:(000000|100644|100755) (100644|100755) ([0-9a-f]{40}) ([0-9a-f]{40}) ([AM])$/.exec(fields[index]), file = fields[index + 1];
      if (!match || !MAINTENANCE_BUDGET_RECOVERY_SOURCE_PATHS.includes(file) || changes.some(value => value.path === file)) fail();
...
export const MAINTENANCE_BUDGET_RECOVERY_MIGRATION_SQL = "BEGIN READ ONLY; SET LOCAL search_path=pg_catalog; SET LOCAL timezone='UTC'; " +
...
export function validateMaintenanceBudgetRecoveryMigrationProof(value, databaseOid, createdAt, request, executeFixed = runGit) {
  try {
    ...
    const start = BigInt(Date.parse(MAINTENANCE_BUDGET_RECOVERY_MIGRATION_WINDOW.startedAt)) * 1000n;
    const end = BigInt(Date.parse(MAINTENANCE_BUDGET_RECOVERY_MIGRATION_WINDOW.endedBefore)) * 1000n;
