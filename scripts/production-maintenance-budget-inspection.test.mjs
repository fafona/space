...
import { assertBudgetRecoveryGenerationsStopped, assertBudgetRecoveryStopped, captureBudgetRecoveryBaseline, validateBudgetRecoveryBaseline, verifyBudgetRecoveryBaseline } from "./production-maintenance-budget-inspection.mjs";
...
const NOW = Date.parse("2026-09-14T07:20:00Z"), copy = value => structuredClone(value);
...
test("fixed failed T7 composes actual O/T5/T6/T7 read-only proofs", { concurrency: false }, async t => {
  ...
  const seed = assembleV7(seed6); mappings.set(JSON.stringify(seed.state), PIN);
  const fixture = () => assembleV7(assembleV6(fixtureV5()));
  const reject = promise => assert.rejects(promise, /budget_(?:inspection_unverified|handoff_unverified)/);
  await t.test("only three exact synthetic states map, real private state is never used", () => {
  ...
  await t.test("capture observes all four real frozen directories/generations and port; signed baseline timestamp retained", async () => {
    const f = fixture(), bytes = JSON.stringify(f.state), baseline = await captureBudgetRecoveryBaseline(f.state, f.io);
    assert.equal(baseline.version, 3); assert.equal(baseline.stateDigest, PIN); assert.equal(baseline.current.target, f.state.candidate.disk.runtime);
    ...
    f.io.now = () => NOW + 1; assert.equal(await verifyBudgetRecoveryBaseline(f.state, baseline, f.io), true); assert.equal(baseline.observedAt, NOW);
  });
  await t.test("O web/worker, T5, T6 and T7 generations or descendants cannot revive", async () => {
    for (const select of [f => f.state.runtime.web.processes[0], f => f.state.runtime.worker.managed.processes[0],
      f => f.state.attemptRecovery.predecessor.state.candidate.web.processes[0],
      f => f.state.secondAttemptRecovery.predecessor.state.candidate.web.processes[0], f => f.state.candidate.web.processes[0],
      f => ({ ...f.state.candidate.web.processes[0], pid: 402, parentPid: 401, cwd: f.state.candidate.disk.runtime + "/subdirectory" })]) {
      const f = fixture(), fact = select(f); f.facts.set(fact.pid, fact); await reject(captureBudgetRecoveryBaseline(f.state, f.io));
    }
  });
  await t.test("later full-stop assertion does not consult current; it still requires empty port", async () => {
    ...
    assert.equal(await assertBudgetRecoveryStopped(f.state, f.io), true);
    f.io.runtime.portEmpty = () => false; await reject(assertBudgetRecoveryStopped(f.state, f.io));
  });
  await t.test("generation-only checks allow T8 listener and same app, but exclude all histories and registry drift", async () => {
    ...
    const make = () => {
      ...
      f.facts.set(501, { ...f.state.candidate.web.processes[0], pid: 501, startTicks: "999", cwd: newPath });
    ...
    assert.equal(await assertBudgetRecoveryGenerationsStopped(good.f.state, good.f.io), true);
    ...
    for (const mutate of [
      ...
      ({ f }) => f.facts.set(301, f.state.secondAttemptRecovery.predecessor.state.candidate.web.processes[0]),
      ({ f }) => f.facts.set(401, f.state.candidate.web.processes[0]),
    ...
    ]) { const changed = make(); mutate(changed); await reject(assertBudgetRecoveryGenerationsStopped(changed.f.state, changed.f.io)); }
    for (const sha of [O, T5, T6, T7]) {
      const changed = make(); changed.row.pm2_env.pm_exec_path = changed.f.disks.get(sha).nextEntryPath;
      await reject(assertBudgetRecoveryGenerationsStopped(changed.f.state, changed.f.io));
      const descendant = make(); descendant.f.facts.set(601, { ...descendant.f.state.candidate.web.processes[0], pid: 601,
        cwd: descendant.f.disks.get(sha).runtime + "/descendant" });
      await reject(assertBudgetRecoveryGenerationsStopped(descendant.f.state, descendant.f.io));
    }
  });
  await t.test("each original/candidate disk and environment is checked in full-stop and generation-only paths", async () => {
    for (const observe of [captureBudgetRecoveryBaseline, assertBudgetRecoveryGenerationsStopped]) {
      for (const sha of [O, T5, T6, T7]) {
  ...
  await t.test("fresh daemon/boot, registry and current observations cannot drift within a check", async () => {
    ...
    await reject(captureBudgetRecoveryBaseline(current.state, current.io));
    const registry = fixture(); let registryReads = 0;
    registry.io.runtime.pm2Registry = async () => ++registryReads === 4 ? [{ malformed: true }] : [];
    await reject(captureBudgetRecoveryBaseline(registry.state, registry.io));
  ...
  await t.test("T7 handoff returns only 27 physical/configuration fields and original O worker policy", async () => {
    const f = fixture(), baseline = await captureBudgetRecoveryBaseline(f.state, f.io), bytes = JSON.stringify(f.state);
    const fields = await readBudgetRecoveryHandoffFields(f.state.runtime, f.state, baseline, f.io);
    ...
    await reject(readBudgetRecoveryHandoffFields({ ...f.state.runtime, bootId: "bad" }, f.state, baseline, f.io));
    let n = 0; f.io.readCurrent = () => ({ ...f.current(), linkIdentity: id(++n < 3 ? 92 : 93, "link") });
    await reject(readBudgetRecoveryHandoffFields(f.state.runtime, f.state, baseline, f.io));
  });
  await t.test("wrong current, file/env drift, boot, journal, audit, getters and mutations reject", async () => {
    ...
    ]) { const f = fixture(); f.state = copy(f.state); mutate(f); await reject(captureBudgetRecoveryBaseline(f.state, f.io)); }
    ...
    await reject(captureBudgetRecoveryBaseline(hostile, f.io)); assert.equal(touched, 0);
    await reject(captureBudgetRecoveryBaseline(new Proxy(f.state, {}), f.io));
    await reject(captureBudgetRecoveryBaseline(f.state, { ...f.io, runtime: { ...f.io.runtime, pm2Control() { assert.fail(); } } }));
  });
  await t.test("actual deadline is enforced before and after observations; shape never authorizes extension", async () => {
    const f = fixture(), baseline = await captureBudgetRecoveryBaseline(f.state, f.io);
    assert.throws(() => validateMaintenanceSecondAttemptRecoveryState(f.state, { bootId: BOOT, now: NOW }));
    assert.equal(f.state.secondAttemptRecovery.authorization.expiresAt, Date.parse("2026-09-14T04:00:00Z"));
    assert.throws(() => validateBudgetRecoveryBaseline({ ...baseline, version: 1 }), /budget_inspection_unverified/);
    for (const tooEarly of ["2026-09-14T04:00:00Z", "2026-09-14T07:13:56.999Z"]) {
      f.io.now = () => Date.parse(tooEarly); await reject(captureBudgetRecoveryBaseline(f.state, f.io));
    }
    f.io.now = () => Date.parse("2026-09-14T10:00:00Z"); await reject(captureBudgetRecoveryBaseline(f.state, f.io));
    let n = 0; f.io.now = () => ++n === 1 ? NOW : Date.parse("2026-09-14T10:00:00Z");
    await reject(captureBudgetRecoveryBaseline(f.state, f.io));
    n = 0; await reject(assertBudgetRecoveryGenerationsStopped(f.state, f.io));
    f.io.now = () => NOW - 1; await reject(verifyBudgetRecoveryBaseline(f.state, baseline, f.io));
  });
  await t.test("read-only source cannot acquire locks or actuate and covers all historical candidates", () => {
    const source = readFileSync(new URL("./production-maintenance-budget-inspection.mjs", import.meta.url), "utf8");
