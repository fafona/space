...
test("maintenance control is a fixed manual current-main exact-CI pinned-SSH workflow", () => {
  ...
  assert.deepEqual(workflows["production-maintenance"].on.workflow_dispatch.inputs.action.options, ["diagnose-runtime", "diagnose-pm2-peer", "plan", "prepare", "recover-held", "continue-held", "recover-build", "recover-attempt", "recover-second-attempt", "recover-budget", "renew-window", "check", "end"]);
...
test("build recovery sends one bounded grant only to its explicit action and never auto-retries or cleanup-mutates it", () => {
  ...
  const fixed = { ACTION: "recover-build", APP_NAME: "merchant-space", APP_PORT: "3000", CHECK_STATE: "held",
    ...
    RECOVERY_EVIDENCE: "", CONTINUATION_EVIDENCE: "", BUILD_RECOVERY_EVIDENCE: "YQ", ATTEMPT_RECOVERY_EVIDENCE: "", SECOND_ATTEMPT_RECOVERY_EVIDENCE: "", BUDGET_RECOVERY_EVIDENCE: "", WINDOW_RENEWAL_EVIDENCE: "" };
