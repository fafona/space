...
import { MAINTENANCE_PREFLIGHT_RECOVERY_AUTHORIZATION as AUTH } from "./production-maintenance-preflight-recovery.mjs";
...
function fixture() {
  ...
  const baseline = { version: 11, revision: 44, activeAttempt: 3, operationId: AUTH.operationId, targetSha: "f".repeat(40),
    ...
    windowRenewal: { fixture: "immutable-original-window-renewal" },
    preflightRecovery: { fixture: "immutable-unused-v8-compact-predecessor-and-new-authority" },
  ...
  const store = {
    async readOperationUnderExistingOperationLock() {
      ...
      // Full actual pure validation is covered by preflight-control.test.mjs.
...
test("every fixed binding and unsupported request action rejects before host I/O", async () => {
  for (const key of ["version", "operationId", "targetSha", "expectedOldSha", "appDir", "appName", "appPort", "bootId", "createdAt", "runtime", "tokenHash", "activeAttempt", "attemptRecovery", "secondAttemptRecovery", "budgetRecovery", "windowRenewal", "preflightRecovery"]) {
  ...
  for (const action of ["recover-preflight", "inspect-preflight-recovery", "prepare", "recover-attempt", "recover-second-attempt", "recover-budget", "inspect-budget-recovery", "snapshot-pair", "check-held", "runtime-handoff", "snapshot-web", ""] ) {
