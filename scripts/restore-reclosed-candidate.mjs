...
import { CANDIDATE_RECOVERY as P, extractRecoveryShellFunction } from "./recover-maintenance-candidate.mjs";

export const RESTORATION = Object.freeze({
  operationId: "1b03580b-4c5d-480b-a047-5139b25a45e0",
  predecessorDigest: "b4e2b4fe6dba69f9287d533b67176d2bb41971cfad6ca8278c7272674eb3c7f2",
  failedRunId: "35069590176", predecessorRevision: 70,
  directory: "/var/lib/faolla-restoration-35069590176",
  archive: "/var/lib/faolla-maintenance/merchant-space.archived-35069590176",
});
...
async function database(state, quiet = true) {
  ...
  if (hash(originalBytes) !== "87439b0b31ad6bfec4f17fe1085ae0c9b5363a6716186c009a090ee2e11dc0c4") fail("baseline");
...
async function dependencies(state) {
  const runtime = await load("production-maintenance-runtime"), ingress = await load("production-maintenance-ingress"), history = await load("production-maintenance-startup-inspection");
  ...
  try {
    ...
    await restorationSequence({
      ...
      failClosed: async () => {
        ...
        await d.runtime.assertRuntimeStopped(old.runtime); record.phase = "failed-held"; await save();
      },
    });
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  try {
    ...
    else if (action === "restore" && confirmation === "RESTORE_VERIFIED_RECLOSED_T17" && resolve(file) === resolve(dirname(process.argv[1]), "restoration-receipt.json")) process.stdout.write(JSON.stringify(await restore(sha, runId, file)) + "\n");
    else fail("arguments");
