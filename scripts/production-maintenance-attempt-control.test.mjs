...
import { MAINTENANCE_ATTEMPT_RECOVERY_INCIDENT as INCIDENT, MAINTENANCE_ATTEMPT_RECOVERY_DEADLINE_AUTHORIZATION as AUTH,
  createMaintenanceAttemptRecoveryInspection, buildMaintenanceAttemptRecoveredState, encodeMaintenanceAttemptRecoveryEvidence } from "./production-maintenance-attempt-recovery.mjs";
...
function request(action, f) {
  const args = [action, "--app-dir", "/srv/faolla", "--app-name", "faolla", "--app-port", "3000", "--target-sha", TARGET,
    "--expected-old-sha", INCIDENT.expectedOldSha, "--expected-operation-id", INCIDENT.operationId, "--json"];
  if (["inspect-attempt-recovery", "recover-attempt"].includes(action)) args.push("--previous-target-sha", INCIDENT.previousTargetSha);
  if (action === "recover-attempt") args.push("--attempt-recovery-evidence", encodeMaintenanceAttemptRecoveryEvidence(f.evidence));
...
function rehearsal(f, initial = f.state) {
  ...
  let inode = 0, fd = 10, locked = false, writes = 0, now = NOW, fault = null, nonce = 20, outcome = "success";
  ...
  const validateState = state => {
    validateMaintenanceState(state, { ...state, action: state.version === 5 ? "inspect-attempt-recovery" : "check-held",
      previousTargetSha: INCIDENT.previousTargetSha }, INCIDENT.bootId, now);
  ...
  const ops = {
    load, save, now: () => now, bootId: () => INCIDENT.bootId, readToken: () => "synthetic-control-token",
    ...
    verifyIngress: record("ingress"), assertRuntimeStopped: record("originalStopped"), assertFailedCandidateStopped: record("bothStopped"),
    assertFailedCandidateGenerationStopped: record("historicalGenerationStopped"),
    assertDatabaseQuiet: record("databaseQuiet"),
    async readAttemptRecoverySnapshot() { const state = await load(), snapshot = baselines.get(state); return { ...snapshot, state }; },
    captureFailedCandidateBaseline: record("captureBaseline", f.baseline),
    async verifyFailedCandidateBaseline(state, baseline) { events.push("verifyBaseline"); assert.deepEqual(state, f.state); assert.deepEqual(baseline, f.baseline); return true; },
    readAttemptRecoverySourceProof: record("source", { sourceDiffDigest: f.context.sourceDiffDigest }),
    readAttemptRecoveryMigrationProof: record("migration", f.context.migrationDigest),
    ...
    async recloseAttemptIngress(state) { events.push("recloseIngress"); assert.deepEqual(state.attemptRecovery.predecessor.state, f.state); return state.ingress; },
    stopCandidate: record("stopCandidate"),
    stopRuntime: async () => { assert.fail("v6 cleanup must never resume or target original O"); },
...
test("isolated v5 to v6 controller, pure model and real journal storage composition", { concurrency: false }, async t => {
  const seed = syntheticPredecessor(), mapped = Buffer.from(JSON.stringify(seed));
  t.mock.method(crypto, "createHash", (algorithm, options) => {
    ...
    actual.digest = encoding => { const real = digest(encoding); return algorithm === "sha256" && encoding === "hex" && Buffer.concat(chunks).equals(mapped) ? INCIDENT.stateDigest : real; };
    return actual;
  ...
  await t.test("inspection has zero writes/sends and returns the actual fixture baseline binding", async () => {
    const f = fixture(seed), r = rehearsal(f), report = await runMaintenanceAction(request("inspect-attempt-recovery", f), r.ops);
  ...
  await t.test("recovery uses one durable CAS and retains the whole consumed journal and four audits", async () => {
    const f = fixture(seed), r = rehearsal(f), next = buildMaintenanceAttemptRecoveredState(f.state, f.evidence, f.context);
    assert.equal((await runMaintenanceAction(request("recover-attempt", f), r.ops)).state, "held");
    assert.deepEqual(r.saved(), next); assert.equal(r.writes(), 1); assert.equal(r.events.filter(event => event === "recoveryCAS").length, 1);
    assert.equal(r.events.filter(event => event === "originalStopped").length, 2); assert.equal(r.events.filter(event => event === "migration").length, 2);
    assert.deepEqual(r.saved().attemptRecovery.predecessor.state.launchJournal, f.state.launchJournal);
  ...
  await t.test("wrong target, invalid evidence, stale history or failed host proof never writes", async () => {
    for (const alter of [
      ...
      (r, q) => { q.attemptRecoveryEvidence.historyDigest = "invalid"; },
      (r, q) => { q.attemptRecoveryEvidence.historyCheckedAt = NOW - 300001; },
      ...
      const f = fixture(seed), r = rehearsal(f), q = copy(request("recover-attempt", f)); alter(r, q);
  ...
  await t.test("second full proof round rechecks source, migration and real final deadline before CAS", async () => {
    for (const kind of ["source", "migration", "deadline"]) {
      ...
      if (kind === "source") r.ops.readAttemptRecoverySourceProof = async () => ({ sourceDiffDigest: ++calls === 1 ? f.context.sourceDiffDigest : "e".repeat(64) });
      if (kind === "migration") r.ops.readAttemptRecoveryMigrationProof = async () => ++calls === 1 ? f.context.migrationDigest : "e".repeat(64);
      if (kind === "deadline") r.ops.assertDatabaseQuiet = async () => { if (++calls === 2) r.setNow(AUTH.expiresAt); };
      await assert.rejects(runMaintenanceAction(request("recover-attempt", f), r.ops)); assert.equal(r.writes(), 0);
    }
  });
  await t.test("file fsync and post-rename uncertainty produce no recovery success and no send/retry", async () => {
    for (const stage of ["fileSync", "renameAfter", "directorySync"]) {
      ...
      await assert.rejects(runMaintenanceAction(request("recover-attempt", f), r.ops), /storage_unconfirmed/);
      ...
      assert.equal(r.saved().version, stage === "fileSync" ? 5 : 6); assert.equal(r.fds.size, 0);
    }
  });
  await t.test("later v6 start persists its own attempt before one send while predecessor stays immutable", async () => {
    const f = fixture(seed), initial = buildMaintenanceAttemptRecoveredState(f.state, f.evidence, f.context), r = rehearsal(f, initial);
  ...
  await t.test("every candidate snapshot, check, registration and handoff excludes T5 before reading T6", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceAttemptRecoveredState(f.state, f.evidence, f.context));
    ...
    for (const action of ["snapshot-web", "snapshot-worker", "check-candidate", "register-candidate", "candidate-handoff", "end"]) {
      ...
      r.ops.assertFailedCandidateGenerationStopped = async () => { r.events.push("historicalGenerationStopped"); throw new Error("synthetic T5 revival"); };
      await assert.rejects(runMaintenanceAction(request(action, f), r.ops), /T5 revival/);
  ...
  await t.test("end preserves both journals and checks historical absence before dump, before reopen and before ended", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceAttemptRecoveredState(f.state, f.evidence, f.context));
    ...
    assert.deepEqual(r.saved().attemptRecovery.predecessor.state, f.state);
  ...
  await t.test("T5 revival after resume or before reopen blocks restore and stops only the new exact resumed proof", async () => {
    for (const rejectAt of [2, 3]) {
      const f = fixture(seed), r = rehearsal(f, buildMaintenanceAttemptRecoveredState(f.state, f.evidence, f.context));
      await runMaintenanceAction(request("start-candidate", f), r.ops); r.clearEvents(); let observations = 0;
      r.ops.assertFailedCandidateGenerationStopped = async () => { r.events.push("historicalGenerationStopped");
        if (++observations === rejectAt) throw new Error("synthetic T5 revival"); };
      // Full cleanup must also refuse to certify held while T5 remains alive.
      r.ops.assertFailedCandidateStopped = async () => { r.events.push("bothStopped"); throw new Error("synthetic T5 remains alive"); };
      ...
      assert.equal(r.events.includes("persistDump"), rejectAt === 3); assert.deepEqual(r.saved().attemptRecovery.predecessor.state, f.state);
    }
  });
  ...
  await t.test("unknown ACK is reconciled by an observation and stopped, never launched again", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceAttemptRecoveredState(f.state, f.evidence, f.context)); r.setOutcome("unknown");
  ...
  await t.test("real storage refuses v6 journal reset, audit deletion and predecessor reuse", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceAttemptRecoveredState(f.state, f.evidence, f.context));
    await runMaintenanceAction(request("start-candidate", f), r.ops);
    ...
    for (const alter of [s => { s.launchJournal = null; s.launchDisk = null; s.candidate = null; s.phase = "held"; },
      s => { delete s.attemptRecovery; }, s => { s.attemptRecovery.predecessor.state.launchJournal.slots["paused-web"].phase = "planned"; }]) {
