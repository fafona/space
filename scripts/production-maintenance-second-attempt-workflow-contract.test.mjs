...
const confirmation = "RECOVER_SECOND_ATTEMPT_PRODUCTION_MAINTENANCE_UNTIL_20260914T040000Z";
const env = {
  GITHUB_REPOSITORY: "fafona/space", GITHUB_EVENT_NAME: "workflow_dispatch", GITHUB_REF: "refs/heads/main", GITHUB_RUN_ATTEMPT: "1",
  ACTION: "recover-second-attempt", CONFIRMATION: confirmation, TARGET_SHA: "a".repeat(40), GITHUB_SHA: "a".repeat(40),
  MAINTENANCE_OPERATION_ID: "eb81284a-09c4-4514-8f16-38eaf6acc1e4",
  PREVIOUS_TARGET_SHA: "3af8fa6ba6644593e10bef0a391389b2b34e926a", EXPECTED_OLD_SHA: "cd943076ebda758b70bf2f2270a508c774b726d6",
...
test("one fixed additional attempt requires exact manual authorization and current main", () => {
  ...
  for (const overrides of [
    ...
    { MAINTENANCE_OPERATION_ID: "ab81284a-09c4-4514-8f16-38eaf6acc1e4" }, { DEPLOY_RUN_ID: "34790827235" },
...
test("historical signed artifacts precede fresh private baseline and fully locked history", () => {
  const names = ["Verify Second Launched Incident Signed Backup And Readiness Bindings", "Verify Second Attempt Recovery Historical Additional Backup",
    "Inspect Stopped Second Launched Candidate Recovery State", "Verify Exact Second Attempt Recovery History Under Production Lock", "Execute Fixed Maintenance Transition"];
  const positions = names.map(name => steps.indexOf(step(name)));
  assert.deepEqual([...positions].sort((a, b) => a - b), positions);
  for (const name of names.slice(0, 4)) assert.equal(step(name).if, "inputs.action == 'recover-second-attempt'");
  const bindings = step(names[0]).run;
  for (const value of ["34789894868", "34790775352", "gh attestation verify", '--source-digest "$PREVIOUS_TARGET_SHA"',
  ...
  assert.match(historical, /validateMaintenanceSecondAttemptRecoveryAdditionalBackup\(readMaintenanceSecondAttemptRecoveryAdditionalBackup\(process\.argv\[2\]\), Date\.now\(\)\)/);
  const inspection = step(names[2]).run;
  assert.match(inspection, /umask 077/);
  assert.match(inspection, /inspect-second-attempt-recovery .*--target-sha .*--previous-target-sha .*--expected-old-sha .*--expected-operation-id/);
  assert.match(inspection, /> "\$capture_dir\/out" 2> "\$capture_dir\/err"/);
  assert.doesNotMatch(inspection, /cat |tee |recover-second-attempt --|fail-held/);
  const history = step(names[3]);
  assert.equal(history.env.INSPECTION_DIR, "${{ steps.second-attempt-recovery-inspection.outputs.capture_dir }}");
  assert.equal(history.env.PRIOR_BINDINGS_DIR, "${{ steps.second-attempt-recovery-prior-bindings.outputs.capture_dir }}");
  assert.equal(history.env.ADDITIONAL_BACKUP_DIR, "${{ steps.second-attempt-recovery-additional-backup.outputs.capture_dir }}");
  assert.match(history.run, /node scripts\/production-maintenance-second-attempt-recovery-workflow\.mjs/);
});
...
test("only the fixed transition receives the private attempt grant; end retains independent gates", () => {
  ...
  assert.equal(transition.env.SECOND_ATTEMPT_RECOVERY_EVIDENCE, "${{ steps.second-attempt-recovery-evidence.outputs.second_attempt_recovery_evidence }}");
  assert.match(transition.run, /recover-second-attempt\) command=recover-second-attempt; expected_state=held/);
  assert.match(transition.run, /test "\$\{#SECOND_ATTEMPT_RECOVERY_EVIDENCE\}" -le 16384/);
  assert.match(transition.run, /--previous-target-sha "\$PREVIOUS_TARGET_SHA" --second-attempt-recovery-evidence "\$SECOND_ATTEMPT_RECOVERY_EVIDENCE"/);
  assert.match(transition.run, /else\n\s+test -z "\$SECOND_ATTEMPT_RECOVERY_EVIDENCE"/);
...
test("attempt cleanup stays within its three exact private runner directories", () => {
  const cleanup = step("Remove Runner Second Attempt Recovery Evidence");
  assert.equal(cleanup.if, "always() && inputs.action == 'recover-second-attempt'");
  assert.deepEqual(Object.keys(cleanup.env), ["INSPECTION_DIR", "PRIOR_BINDINGS_DIR", "ADDITIONAL_BACKUP_DIR"]);
  for (const prefix of ["maintenance-second-attempt-recovery", "maintenance-second-attempt-prior-bindings", "maintenance-second-attempt-additional-backup"])
    assert.ok(cleanup.run.includes('"$RUNNER_TEMP"/' + prefix + '.????????'));
  assert.doesNotMatch(cleanup.run, /rm -rf|sudo|ssh |state\.json|maintenance\.json|fail-held|recover-second-attempt --/);
...
test("every new embedded shell block parses without running a deployment", () => {
  for (const item of steps.filter(value => value.if?.includes("recover-second-attempt") || value.name === "Execute Fixed Maintenance Transition" || value.name === "Validate Fixed Manual Transition")) {
