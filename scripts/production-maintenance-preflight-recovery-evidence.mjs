...
// Exact preflight-only recovery delta; the launch opportunity and previous
// source/SQL audits are preserved, never generalized to arbitrary changes.
export const MAINTENANCE_PREFLIGHT_RECOVERY_SOURCE_PATHS = Object.freeze([
  ...
  "scripts/production-maintenance-preflight-recovery.mjs",
  "scripts/production-maintenance-preflight-recovery.test.mjs",
  "scripts/production-maintenance-preflight-recovery-evidence.mjs",
  "scripts/production-maintenance-preflight-recovery-evidence.test.mjs",
  "scripts/production-maintenance-preflight-recovery-workflow.mjs",
  "scripts/production-maintenance-preflight-recovery-workflow.test.mjs",
  "scripts/production-maintenance-preflight-inspection.mjs",
  "scripts/production-maintenance-preflight-inspection.test.mjs",
  "scripts/production-maintenance-preflight-handoff.mjs",
  "scripts/production-maintenance-preflight-handoff.test.mjs",
  "scripts/production-maintenance-preflight-control.test.mjs",
  "scripts/production-maintenance-preflight-reclose.test.mjs",
  "scripts/production-maintenance-preflight-disk.mjs",
  "scripts/production-maintenance-preflight-disk.test.mjs",
...
export const MAINTENANCE_PREFLIGHT_RECOVERY_MIGRATION_WINDOW = Object.freeze({
  runId: "34721155156", attempt: 1, startedAt: "2026-09-12T21:52:38.000Z", endedBefore: "2026-09-12T21:52:46.000Z",
});
const PREVIOUS = "5c1130adf37308e30e5c656f3d894e096cd65b7f", CREATED_AT = 1789236034129;
...
const fail = () => { throw new Error("maintenance_preflight_recovery_evidence_unverified"); };
...
export function readMaintenancePreflightRecoverySourceProof(request, executeFixed = runGit) {
  try {
    ...
    if (!fields.length || fields.length % 2 || fields.length > MAINTENANCE_PREFLIGHT_RECOVERY_SOURCE_PATHS.length * 2) fail();
    for (let index = 0; index < fields.length; index += 2) {
      const match = /^:(000000|100644|100755) (100644|100755) ([0-9a-f]{40}) ([0-9a-f]{40}) ([AM])$/.exec(fields[index]), file = fields[index + 1];
      if (!match || !MAINTENANCE_PREFLIGHT_RECOVERY_SOURCE_PATHS.includes(file) || changes.some(value => value.path === file)) fail();
...
export const MAINTENANCE_PREFLIGHT_RECOVERY_MIGRATION_SQL = "BEGIN READ ONLY; SET LOCAL search_path=pg_catalog; SET LOCAL timezone='UTC'; " +
...
export function validateMaintenancePreflightRecoveryMigrationProof(value, databaseOid, createdAt, request, executeFixed = runGit) {
  try {
    ...
    const start = BigInt(Date.parse(MAINTENANCE_PREFLIGHT_RECOVERY_MIGRATION_WINDOW.startedAt)) * 1000n;
    const end = BigInt(Date.parse(MAINTENANCE_PREFLIGHT_RECOVERY_MIGRATION_WINDOW.endedBefore)) * 1000n;
