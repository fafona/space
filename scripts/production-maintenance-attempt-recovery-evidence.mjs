...
export const MAINTENANCE_ATTEMPT_RECOVERY_SOURCE_PATHS = Object.freeze([
  ...
  "scripts/production-maintenance-attempt-control.test.mjs",
  "scripts/production-maintenance-attempt-handoff.mjs",
  "scripts/production-maintenance-attempt-reclose.test.mjs",
  "scripts/production-maintenance-attempt-recovery-evidence.mjs",
  "scripts/production-maintenance-attempt-recovery-evidence.test.mjs",
  "scripts/production-maintenance-attempt-recovery-workflow.mjs",
  "scripts/production-maintenance-attempt-recovery-workflow.test.mjs",
  "scripts/production-maintenance-attempt-recovery.mjs",
  "scripts/production-maintenance-attempt-recovery.test.mjs",
  "scripts/production-maintenance-attempt-workflow-contract.test.mjs",
  ...
  "scripts/production-maintenance-control.test.mjs",
  "scripts/production-maintenance-deploy-read.mjs",
  "scripts/production-maintenance-deploy-read.test.mjs",
  "scripts/production-maintenance-failed-candidate-inspection.mjs",
  "scripts/production-maintenance-failed-candidate-inspection.test.mjs",
  "scripts/production-maintenance-launch-journal-storage.mjs",
  "scripts/production-maintenance-snapshot-composition.test.mjs",
  "scripts/production-maintenance-workflow-contract.test.mjs"
]);
export const MAINTENANCE_ATTEMPT_RECOVERY_MIGRATION_WINDOW = Object.freeze({
  runId: "34721155156", attempt: 1, startedAt: "2026-09-12T21:52:38.000Z", endedBefore: "2026-09-12T21:52:46.000Z",
});
const PREVIOUS = "f3104de19aa59e527c7b94a99850d151448da8cd", CREATED_AT = 1789236034129;
...
const fail = () => { throw new Error("maintenance_attempt_recovery_evidence_unverified"); };
...
export function readMaintenanceAttemptRecoverySourceProof(request, executeFixed = runGit) {
  try {
    ...
    if (!fields.length || fields.length % 2 || fields.length > MAINTENANCE_ATTEMPT_RECOVERY_SOURCE_PATHS.length * 2) fail();
    for (let index = 0; index < fields.length; index += 2) {
      const match = /^:(000000|100644|100755) (100644|100755) ([0-9a-f]{40}) ([0-9a-f]{40}) ([AM])$/.exec(fields[index]), file = fields[index + 1];
      if (!match || !MAINTENANCE_ATTEMPT_RECOVERY_SOURCE_PATHS.includes(file) || changes.some(value => value.path === file)) fail();
...
export const MAINTENANCE_ATTEMPT_RECOVERY_MIGRATION_SQL = "BEGIN READ ONLY; SET LOCAL search_path=pg_catalog; SET LOCAL timezone='UTC'; " +
...
export function validateMaintenanceAttemptRecoveryMigrationProof(value, databaseOid, createdAt, request, executeFixed = runGit) {
  try {
    ...
    const start = BigInt(Date.parse(MAINTENANCE_ATTEMPT_RECOVERY_MIGRATION_WINDOW.startedAt)) * 1000n;
    const end = BigInt(Date.parse(MAINTENANCE_ATTEMPT_RECOVERY_MIGRATION_WINDOW.endedBefore)) * 1000n;
