...
import { validateMaintenanceSecondAttemptRecoveryInspection, encodeMaintenanceSecondAttemptRecoveryEvidence,
  decodeMaintenanceSecondAttemptRecoveryEvidence, MAINTENANCE_SECOND_ATTEMPT_RECOVERY_AUTHORIZATION,
  MAINTENANCE_SECOND_ATTEMPT_RECOVERY_AUTHORIZATION_DIGEST,
  MAINTENANCE_SECOND_ATTEMPT_RECOVERY_INCIDENT as INCIDENT } from "./production-maintenance-second-attempt-recovery.mjs";
...
import { validateMaintenanceSecondAttemptRecoveryPriorBindings, inspectMaintenanceSecondAttemptRecoveryHistory,
  createMaintenanceSecondAttemptRecoveryWorkflowEvidence, validateMaintenanceSecondAttemptRecoveryAdditionalBackup,
  readMaintenanceSecondAttemptRecoveryAdditionalBackup, MAINTENANCE_ADDITIONAL_BACKUP_SMALL_ARTIFACTS } from "./production-maintenance-second-attempt-recovery-workflow.mjs";
...
const NOW = Date.parse("2026-09-14T02:05:00Z");
const baseline = { version: 2, stateDigest: INCIDENT.stateDigest, candidateDigest: "a".repeat(64), launchDiskDigest: "b".repeat(64),
  launchJournalDigest: "c".repeat(64), runtimeDigest: "d".repeat(64), current: {
    target: "/srv/faolla.releases/3af8fa6ba664-20260913235400", linkIdentity: "1:2:3:4:5:1:0:41471", runtimeIdentity: "1:2:3:4:5:2:0:16877" },
  bootId: INCIDENT.bootId, pm2RegistryDigest: "e".repeat(64), observedAt: NOW - 1000 };
const inspection = validateMaintenanceSecondAttemptRecoveryInspection({
  version: 1, state: "second-attempt-recovery-inspected", operationId: INCIDENT.operationId, targetSha: TARGET, previousTargetSha: PREVIOUS,
  expectedOldSha: INCIDENT.expectedOldSha, revision: 23, stateDigest: INCIDENT.stateDigest, createdAt: INCIDENT.createdAt, activeAttempt: 2,
  ...
  authorizationDigest: MAINTENANCE_SECOND_ATTEMPT_RECOVERY_AUTHORIZATION_DIGEST,
...
const env = { GITHUB_REPOSITORY: "fafona/space", GITHUB_EVENT_NAME: "workflow_dispatch", GITHUB_REF: "refs/heads/main",
  ...
  ACTION: "recover-second-attempt", CONFIRMATION: "RECOVER_SECOND_ATTEMPT_PRODUCTION_MAINTENANCE_UNTIL_20260914T040000Z" };
...
const incidents = [
  ...
  ["34789894868", 0, PREVIOUS, "2026-09-13T23:30:29Z", "2026-09-13T23:47:14Z"],
  ["34790775352", 2, PREVIOUS, "2026-09-13T23:48:52Z", "2026-09-13T23:49:59Z"],
  ["34790827235", 3, PREVIOUS, "2026-09-13T23:50:01Z", "2026-09-13T23:57:59Z"],
];
const required = [
...
function run(spec, patch = {}) {
  ...
  return { id: Number(id), run_attempt: 1, name: names[index], path: ".github/workflows/" + files[index],
    ...
    status: "completed", conclusion: index === 3 ? "failure" : "success", created_at: ["34789894868", "34790775352", "34790827235"].includes(id) ? started : iso(Date.parse(started) - 1000),
...
function priorAttemptRun(key) {
  ...
  return { ...priorRun(recovery ? "prior-recovery" : "prior-ci"), id: recovery ? 34789744074 : 34789133814,
    head_sha: PREVIOUS, created_at: recovery ? "2026-09-13T23:27:15Z" : "2026-09-13T23:14:38Z",
...
function priorAttemptJobs(key) {
  if (key === "prior-attempt-ci") return { total_count: 10, jobs: Array.from({ length: 10 }, (_, index) => ({
    id: 91000000000 + index, name: "Synthetic CI " + index, run_id: 34789133814, head_sha: PREVIOUS,
  ...
  return { total_count: 1, jobs: [{ id: 103811645002, run_id: 34789744074, head_sha: PREVIOUS, status: "completed", conclusion: "success",
...
// hard-pinned by readMaintenanceSecondAttemptRecoveryAdditionalBackup, not this fixture.
...
const history = transform => inspectMaintenanceSecondAttemptRecoveryHistory(inspection, fixture(transform).api, NOW);
const evidence = (transform, patch = {}, prior = bindings()) => createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, { ...env, ...patch }, fixture(transform).api, prior, NOW, additionalRecords());
...
test("all thirteen exact incidents produce canonical evidence without relabelling the T6 signed subjects", async () => {
  const f = fixture(), prior = bindings(), result = await createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, env, f.api, prior, NOW, additionalRecords());
  assert.equal(result.secondAttemptRecoveryRunId, env.GITHUB_RUN_ID); assert.equal(result.secondAttemptRecoveryRunAttempt, 1);
  assert.equal(result.mainCIrunId, "34728900000"); assert.equal(result.toolsSha, TARGET); assert.equal(result.historyCheckedAt, NOW);
  assert.equal(result.authorizationDigest, MAINTENANCE_SECOND_ATTEMPT_RECOVERY_AUTHORIZATION_DIGEST);
  assert.match(result.historyDigest, /^[0-9a-f]{64}$/);
  assert.deepEqual(decodeMaintenanceSecondAttemptRecoveryEvidence(encodeMaintenanceSecondAttemptRecoveryEvidence(result)), result);
  assert.equal(f.calls.filter(value => value.endsWith("commits/main")).length, 2);
  assert.equal(f.calls.filter(value => value.includes("/attempts/1/jobs?")).length, 18);
  assert.deepEqual((await history()).incidents.map(value => value.id), incidents.map(value => value[0]));
  assert.equal(validateMaintenanceSecondAttemptRecoveryPriorBindings(inspection, prior).backup.binding.targetSha, PREVIOUS);
  assert.equal(validateMaintenanceSecondAttemptRecoveryPriorBindings(inspection, prior).readiness.binding.backupRunId, inspection.backupRunId);
});
...
test("fixed action, confirmation, repository, run attempt and target binding reject before any API request", async () => {
  for (const patch of [{ ACTION: "continue-held" }, { CONFIRMATION: "CONTINUE_MIGRATED_PRODUCTION_MAINTENANCE" },
    ...
    const f = fixture(); await assert.rejects(createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, { ...env, ...patch }, f.api, bindings(), NOW, additionalRecords()));
...
test("two complete pages commit the old tail to history, not just the newest fixed incidents", async () => {
  ...
  const result = await createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, env, f.api, bindings(), NOW, additionalRecords());
...
test("foreign identity, invalid dates, future or inverted timestamps, and in-progress activity reject", async () => {
  ...
  for (const now of [NaN, Infinity, 1.5, inspection.createdAt - 1, MAINTENANCE_SECOND_ATTEMPT_RECOVERY_AUTHORIZATION.authorizedAt - 1,
    MAINTENANCE_SECOND_ATTEMPT_RECOVERY_AUTHORIZATION.expiresAt])
    await assert.rejects(createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, env, fixture().api, bindings(), now, additionalRecords()));
});

test("the additional attempt uses actual 02:01:33 to 04 UTC clock and a fresh stopped baseline", async () => {
  const auth = MAINTENANCE_SECOND_ATTEMPT_RECOVERY_AUTHORIZATION;
  ...
  for (const now of [auth.authorizedAt - 1, Date.parse("2026-09-14T01:12:17Z"), auth.expiresAt, auth.expiresAt + 1, NOW + 300000, NOW - 1001]) {
    const f = fixture();
    await assert.rejects(createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, env, f.api, bindings(), now, additionalRecords()));
    assert.equal(f.calls.length, 0);
  }
  assert.equal((await evidence()).authorizationDigest, inspection.authorizationDigest);
  assert.throws(() => validateMaintenanceSecondAttemptRecoveryAdditionalBackup(additionalRecords(), auth.expiresAt));
});
...
test("extension does not erase new scheduled activity after the original deadline or change the seven exceptions", async () => {
  ...
  for (const digest of [undefined, "0".repeat(64), "bad"]) {
    const f = fixture();
    await assert.rejects(createMaintenanceSecondAttemptRecoveryWorkflowEvidence({ ...inspection, authorizationDigest: digest }, env, f.api, bindings(), NOW, additionalRecords()));
    assert.equal(f.calls.length, 0);
  }
});

test("all thirteen genuine snake-case jobs, one-job inventories and required successful guard steps are checked", async () => {
  for (const spec of incidents) {
...
test("all four failed deployments fail only Deploy To Server; later candidate, public and attestation steps stay skipped", async () => {
...
test("only signed B6/R6 subjects match; neither T2 evidence nor T4 relabelling is accepted", async () => {
  for (const phase of ["backup", "readiness"]) for (const patch of [
    ...
    const f = fixture(); await assert.rejects(createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, env, f.api, prior, NOW, additionalRecords()));
    assert.equal(f.calls.length, 0);
  }
});
...
test("malformed, noncanonical, oversized and missing subjects or wrong provenance never yield an evidence", () => {
  ...
  ]) { const prior = bindings(); mutate(prior); assert.throws(() => validateMaintenanceSecondAttemptRecoveryPriorBindings(inspection, prior)); }
});
...
test("malformed fixed inspection starts zero requests and any API failure returns no grant", async () => {
  for (const patch of [{ targetSha: PREVIOUS }, { backupRunAttempt: 2 }, { migrationRunId: "123" }, { revision: 8 },
    ...
    const f = fixture(); await assert.rejects(createMaintenanceSecondAttemptRecoveryWorkflowEvidence({ ...inspection, ...patch }, env, f.api, bindings(), NOW, additionalRecords()));
    assert.equal(f.calls.length, 0);
  }
  let requests = 0;
  await assert.rejects(createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, env, async () => { requests++; throw new Error("offline"); }, bindings(), NOW, additionalRecords()), /offline/);
...
test("input bindings remain unchanged and original historic job details remain in the history digest", async () => {
  ...
  await createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, env, fixture().api, prior, NOW, additionalRecords());
...
test("CLI rejects invalid invocation with only the fixed error, without credential or source output", () => {
  const result = spawnSync(process.execPath, [fileURLToPath(new URL("./production-maintenance-second-attempt-recovery-workflow.mjs", import.meta.url))], {
  ...
  assert.equal(result.status, 1); assert.equal(result.stdout, ""); assert.equal(result.stderr, "maintenance_second_attempt_recovery_workflow_unverified\n");
});
...
test("fixed small artifact inventory binds live IDs, names, sizes, digests and source without fetching payload bytes", async () => {
  ...
  const f = fixture(), result = await createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, env, f.api, bindings(), NOW, additionalRecords());
  assert.match(result.historyDigest, /^[a-f0-9]{64}$/);
  assert.equal(result.backupRunId, "34789894868"); assert.equal(result.readinessRunId, "34790775352");
...
test("additional off binding, hosted schedule provenance and exact report hashes must all match", async () => {
  const summary = validateMaintenanceSecondAttemptRecoveryAdditionalBackup(additionalRecords(), NOW);
  ...
  ]) { const records = additionalRecords(); mutate(records); assert.throws(() => validateMaintenanceSecondAttemptRecoveryAdditionalBackup(records, NOW)); }
  const f = fixture(); await assert.rejects(createMaintenanceSecondAttemptRecoveryWorkflowEvidence(inspection, env, f.api, bindings(), NOW)); assert.equal(f.calls.length, 0);
  assert.throws(() => readMaintenanceSecondAttemptRecoveryAdditionalBackup(fileURLToPath(new URL(".", import.meta.url))));
});
...
test("B6 and R6 require hosted exact workflow/source/invocation in addition to their byte subject", () => {
  for (const phase of ["backup", "readiness"]) for (const mutate of [
    ...
    assert.throws(() => validateMaintenanceSecondAttemptRecoveryPriorBindings(inspection, prior));
  }
});

test("the immediately prior attempt CAS and ten-job T6 CI are immutable and required", async () => {
...
test("the second attempt confirmation cannot reuse the first and an unknown 02:17 schedule is refused", async () => {
  ...
  await assert.rejects(inspectMaintenanceSecondAttemptRecoveryHistory(inspection, fixture((key, value) => key === files[0] ? {
  ...
  } : value).api, Date.parse("2026-09-14T02:30:00Z")));
