...
import { MAINTENANCE_LEASE_AUTHORIZATION as AUTHORIZATION,
  validateMaintenanceLeasePredecessor, reconstructMaintenanceLeasePredecessor, maintenanceLeaseHistoricalState } from "./production-maintenance-lease.mjs";
 ...
 */
const ERROR = "production_maintenance_lease_inspection_unverified";
...
export function validateLeaseBaseline(raw) {
  ...
  if (!exact(value, BASELINE_KEYS) || value.version !== 3 || value.stateDigest !== PIN || value.bootId !== BOOT ||
      ...
      !Number.isSafeInteger(value.observedAt) || value.observedAt < AUTHORIZATION.authorizedAt) fail();
...
export async function assertLeaseGenerationsStopped(rawState, overrides = {}) {
  try {
...
export async function captureLeaseBaseline(rawState, overrides = {}) {
  try {
    ...
    const baseline = validateLeaseBaseline({ version: 3, stateDigest: PIN, candidateDigest: hash(state.candidate),
    ...
    const previous = predecessor.preflightRecovery.stoppedBaseline;
    if (!isDeepStrictEqual({ ...baseline, observedAt: previous.observedAt }, previous)) fail();
    return baseline;
  } catch { fail(); }
}
export async function verifyLeaseBaseline(state, rawBaseline, overrides = {}) {
  try {
    const baseline = validateLeaseBaseline(rawBaseline);
    const fresh = await captureLeaseBaseline(state, overrides);
