...
import { assertMaintenanceBudgetRecoveryProgress, validateMaintenanceBudgetRecoveryState,
  MAINTENANCE_BUDGET_RECOVERY_INCIDENT as BUDGET } from "./production-maintenance-budget-recovery.mjs";
...
/** Pure renewal of the SAME unused activeAttempt 3; no I/O or authentication.
 * The caller proves current stopped generations, ingress, database, exact new
 * main/CI/history and then uses the existing lock and raw-byte/revision CAS.
 * The original expired v8 is reconstructed byte-for-byte, not copied into v9.
 * Only its historical audit uses its original recorded clock. The actual v9
 * launch/journal are never projected to an older target or passed a fake clock.
 */
export const MAINTENANCE_WINDOW_RENEWAL_INCIDENT = Object.freeze({
  operationId: BUDGET.operationId, bootId: BUDGET.bootId,
  previousTargetSha: "3b6c55ea4397c4505e75c59bb3bb56d7dd7d1cc0",
  expectedOldSha: BUDGET.expectedOldSha, revision: 32, createdAt: BUDGET.createdAt,
  stateBytes: 1570550, stateDigest: "333658652c3dc9e244b7c30848abb716c133f3b5bf0220ea9deab255e696667e",
  backupRunId: BUDGET.backupRunId, backupRunAttempt: 1,
  migrationRunId: BUDGET.migrationRunId, migrationRunAttempt: 1,
  readinessRunId: BUDGET.readinessRunId, readinessRunAttempt: 1,
  failedDeployRunId: BUDGET.failedDeployRunId, failedDeployRunAttempt: 1,
  failedBackupRunId: "34826545330", failedBackupRunAttempt: 1,
  priorRecoveryRunId: "34825984301", priorRecoveryRunAttempt: 1,
  priorMainCIrunId: "34824742841", priorMainCIrunAttempt: 1,
});
const INCIDENT = MAINTENANCE_WINDOW_RENEWAL_INCIDENT;
export const MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION = Object.freeze({
  ...
  authorizedAt: Date.parse("2026-09-14T12:31:04.000Z"),
  previousExpiresAt: Date.parse("2026-09-14T10:00:00.000Z"),
  expiresAt: Date.parse("2026-09-14T16:00:00.000Z"),
  previousActiveAttempt: 3, activeAttempt: 3, maximumAdditionalAttempts: 0, maximumRemainingAttempts: 1,
});
const AUTHORIZATION = MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION;
export const MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION_DIGEST = createHash("sha256").update(JSON.stringify(AUTHORIZATION)).digest("hex");
export const MAINTENANCE_WINDOW_RENEWAL_MAX_EVIDENCE_BYTES = 16 * 1024;
export const MAINTENANCE_WINDOW_RENEWAL_HISTORY_MAX_AGE_MS = 5 * 60 * 1000;
const MAX_STATE_BYTES = 4 * 1024 * 1024;
const SHA = /^[0-9a-f]{40}$/, DIGEST = /^[0-9a-f]{64}$/, RUN = /^[1-9][0-9]{0,19}$/;
const IDENTITY = /^(?:0|[1-9][0-9]{0,24})(?::(?:0|[1-9][0-9]{0,24})){7}$/;
// This order is the original compact v8 JSON order. Reconstruction must match
// BOTH the entire raw digest and byte count; no canonical rewriting is allowed.
const STATE_KEYS = ["version", "revision", "operationId", "targetSha", "expectedOldSha", "appDir", "appName", "appPort", "bootId", "createdAt", "phase", "runtime", "ingress", "database", "publicSupabaseUrl", "tokenHash", "candidate", "resumed", "launchDisk", "launchJournal", "finalDump", "recovery", "continuation", "buildRecovery", "deadlineExtension", "activeAttempt", "attemptRecovery", "secondAttemptRecovery", "budgetRecovery"];
const LAUNCH_KEYS = ["candidate", "resumed", "launchDisk", "launchJournal", "finalDump"];
const RUN_KEYS = ["backupRunId", "backupRunAttempt", "migrationRunId", "migrationRunAttempt", "readinessRunId", "readinessRunAttempt", "failedDeployRunId", "failedDeployRunAttempt", "failedBackupRunId", "failedBackupRunAttempt", "priorRecoveryRunId", "priorRecoveryRunAttempt", "priorMainCIrunId", "priorMainCIrunAttempt"];
...
const EVIDENCE_KEYS = [...INSPECTION_KEYS, "toolsSha", "windowRenewalRunId", "windowRenewalRunAttempt", "mainCIrunId", "historyDigest", "historyCheckedAt"];
...
const AUDIT_KEYS = ["version", "predecessor", "evidence", "renewedAt", "stoppedBaseline", "authorization"];
const PREDECESSOR_KEYS = ["version", "revision", "targetSha", "phase", "stateDigest", "stateBytes", "ingress"];
const FIXED_KEYS = ["operationId", "expectedOldSha", "appDir", "appName", "appPort", "bootId", "createdAt", "runtime", "database", "publicSupabaseUrl", "tokenHash", "recovery", "continuation", "buildRecovery", "deadlineExtension", "attemptRecovery", "secondAttemptRecovery", "budgetRecovery"];
const PHASES = ["held", "candidate", "resuming", "ended", "failed-held", "failed-unknown"];
const PRIOR_RUNS = ["34715768455", "34715352249", "34715932102", "34721155156", "34721256683", "34721317710", "34724808528", "34724337523", "34724943157", "34728212357", "34728263285", "34745334237", "34742540111", "34777790522", "34778424264", "34778579797", "34781336277", "34781392661", "34789133814", "34789744074", "34789894868", "34790775352", "34790827235", "34799821827", "34800461043", "34800653808", "34802075500", "34802138869", "34820083043", "34821029270", "34825984301", "34824742841", "34826545330"];
...
const fail = () => { throw new Error("maintenance_window_renewal_invalid"); };
...
function predecessorAudit(state) {
  if (!exact(state, STATE_KEYS) || state.version !== 8 || state.activeAttempt !== 3 ||
      state.revision !== INCIDENT.revision || state.phase !== "held" ||
  ...
  const historicalAuditClock = state.budgetRecovery?.recoveredAt;
  if (!time(historicalAuditClock) || historicalAuditClock >= AUTHORIZATION.previousExpiresAt ||
      state.budgetRecovery?.authorization?.expiresAt !== AUTHORIZATION.previousExpiresAt ||
      state.budgetRecovery?.evidence?.budgetRecoveryRunId !== INCIDENT.priorRecoveryRunId ||
      state.budgetRecovery?.evidence?.mainCIrunId !== INCIDENT.priorMainCIrunId) fail();
  // Only the exact original, unused v8 is evaluated at this immutable old audit
  // time. Host observations, new state, grants and actuation always use now.
  try { validateMaintenanceBudgetRecoveryState(state, { bootId: INCIDENT.bootId, now: historicalAuditClock }); } catch { fail(); }
  return state;
}
function compactPredecessor(state) {
  return { version: 8, revision: INCIDENT.revision, targetSha: INCIDENT.previousTargetSha, phase: "held",
    stateDigest: INCIDENT.stateDigest, stateBytes: INCIDENT.stateBytes, ingress: state.ingress };
}
function projectOriginal(state) {
  const compact = state.windowRenewal?.predecessor;
  if (!exact(compact, PREDECESSOR_KEYS) || compact.version !== 8 || compact.revision !== INCIDENT.revision ||
      compact.targetSha !== INCIDENT.previousTargetSha || compact.phase !== "held" || compact.stateDigest !== INCIDENT.stateDigest ||
  ...
  // New live launch evidence is NOT discarded: it remains on the actual v9.
  const original = { ...project(state, STATE_KEYS), version: 8, revision: INCIDENT.revision,
    targetSha: INCIDENT.previousTargetSha, phase: "held", ingress: compact.ingress,
...
function baselineFor(value, predecessor, now) {
  ...
  baselineShape(value);
  // Renewal is not permission to re-anchor the still-unused attempt. Only
  ...
  if (!isDeepStrictEqual(project(value, stableKeys), project(predecessor.budgetRecovery.stoppedBaseline, stableKeys))) fail();
  if (
      ...
      value.observedAt > now || now - value.observedAt > MAINTENANCE_WINDOW_RENEWAL_HISTORY_MAX_AGE_MS ||
...
function checkInspection(value) {
  if (!exact(value, INSPECTION_KEYS) || value.version !== 1 || value.state !== "window-renewal-inspected" ||
      ...
      value.authorizationDigest !== MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION_DIGEST ||
...
function checkEvidence(value) {
  ...
  if (value.toolsSha !== value.targetSha || !matches(RUN, value.windowRenewalRunId) || value.windowRenewalRunAttempt !== 1 ||
      !matches(RUN, value.mainCIrunId) || value.windowRenewalRunId === value.mainCIrunId ||
      PRIOR_RUNS.includes(value.windowRenewalRunId) || PRIOR_RUNS.includes(value.mainCIrunId) || !matches(DIGEST, value.historyDigest) ||
...
function inspectionFor(state, context) {
  ...
  return checkInspection({ version: 1, state: "window-renewal-inspected", operationId: INCIDENT.operationId, targetSha: context.targetSha,
    ...
    authorizationDigest: MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION_DIGEST, ...project(INCIDENT, RUN_KEYS) });
}

export function validateMaintenanceWindowRenewalPredecessor(rawState, rawClock) {
  const state = bounded(rawState), clock = bounded(rawClock); actualClock(clock); predecessorAudit(state); return freeze(state);
}
export function createMaintenanceWindowRenewalInspection(rawState, rawContext) {
  return freeze(inspectionFor(bounded(rawState), bounded(rawContext)));
}
export function validateMaintenanceWindowRenewalInspection(value) {
  return freeze(checkInspection(bounded(value, MAINTENANCE_WINDOW_RENEWAL_MAX_EVIDENCE_BYTES)));
}
export function validateMaintenanceWindowRenewalEvidence(value) {
  return freeze(checkEvidence(bounded(value, MAINTENANCE_WINDOW_RENEWAL_MAX_EVIDENCE_BYTES)));
}
export function encodeMaintenanceWindowRenewalEvidence(value) {
  const encoded = Buffer.from(JSON.stringify(validateMaintenanceWindowRenewalEvidence(value))).toString("base64url");
  if (encoded.length > MAINTENANCE_WINDOW_RENEWAL_MAX_EVIDENCE_BYTES) fail(); return encoded;
}
export function decodeMaintenanceWindowRenewalEvidence(value) {
  if (typeof value !== "string" || !value || value.length > MAINTENANCE_WINDOW_RENEWAL_MAX_EVIDENCE_BYTES || !/^[A-Za-z0-9_-]+$/.test(value)) fail();
  try {
    const decoded = new TextDecoder("utf-8", { fatal: true }).decode(Buffer.from(value, "base64url"));
    const result = validateMaintenanceWindowRenewalEvidence(JSON.parse(decoded));
    if (encodeMaintenanceWindowRenewalEvidence(result) !== value) fail(); return result;
  } catch { fail(); }
}
export function buildMaintenanceWindowRenewedState(rawState, rawEvidence, rawContext) {
  const state = bounded(rawState), context = bounded(rawContext), evidence = checkEvidence(bounded(rawEvidence, MAINTENANCE_WINDOW_RENEWAL_MAX_EVIDENCE_BYTES));
  ...
  if (!isDeepStrictEqual(inspection, project(evidence, INSPECTION_KEYS)) || evidence.historyCheckedAt > context.now ||
      context.now - evidence.historyCheckedAt > MAINTENANCE_WINDOW_RENEWAL_HISTORY_MAX_AGE_MS) fail();
  ...
  const next = { ...state, version: 9, revision: state.revision + 1, targetSha: context.targetSha,
    windowRenewal: { version: 1, predecessor: compactPredecessor(state), evidence,
      renewedAt: context.now, stoppedBaseline: context.stoppedBaseline, authorization: { ...AUTHORIZATION } } };
  return validateMaintenanceWindowRenewalState(next, { bootId: context.bootId, now: context.now });
}
export function validateMaintenanceWindowRenewalState(rawState, rawClock) {
  const state = bounded(rawState), clock = bounded(rawClock); actualClock(clock);
  if (!exact(state, [...STATE_KEYS, "windowRenewal"]) || state.version !== 9 || state.activeAttempt !== 3 ||
      !integer(state.revision) || state.revision <= INCIDENT.revision || !PHASES.includes(state.phase) ||
      !exact(state.windowRenewal, AUDIT_KEYS)) fail();
  const audit = state.windowRenewal;
  if (audit.version !== 1 || !isDeepStrictEqual(audit.authorization, AUTHORIZATION) || !time(audit.renewedAt) ||
      audit.renewedAt < AUTHORIZATION.authorizedAt || audit.renewedAt > clock.now || audit.renewedAt >= AUTHORIZATION.expiresAt) fail();
  const predecessor = reconstructPredecessor(state), evidence = checkEvidence(audit.evidence);
  baselineFor(audit.stoppedBaseline, predecessor, audit.renewedAt);
  if (!record(state.ingress) || state.targetSha !== evidence.targetSha ||
      ...
      evidence.historyCheckedAt > audit.renewedAt || audit.renewedAt - evidence.historyCheckedAt > MAINTENANCE_WINDOW_RENEWAL_HISTORY_MAX_AGE_MS ||
...
export function reconstructMaintenanceWindowRenewalPredecessor(rawState, rawClock) {
  ...
  return freeze(projectOriginal(validateMaintenanceWindowRenewalState(rawState, rawClock)));
}
...
export function assertMaintenanceWindowRenewalProgress(rawPrevious, rawNext) {
  const previous = bounded(rawPrevious), next = bounded(rawNext);
  if (!record(previous) || !record(next)) fail();
  if (previous.version !== 9 && next.version !== 9) {
    if ([previous, next].some(state => Object.hasOwn(state, "windowRenewal"))) fail();
    return assertMaintenanceBudgetRecoveryProgress(previous, next);
  }
  if (previous.version === 8 && next.version === 9) {
    if (!exact(next.windowRenewal, AUDIT_KEYS)) fail();
    const audit = next.windowRenewal, evidence = checkEvidence(audit.evidence);
    const expected = buildMaintenanceWindowRenewedState(previous, evidence, { operationId: evidence.operationId,
      previousTargetSha: evidence.previousTargetSha, targetSha: evidence.targetSha, expectedOldSha: evidence.expectedOldSha,
      expectedRevision: evidence.revision, expectedDigest: evidence.stateDigest, bootId: previous.bootId, now: audit.renewedAt,
  ...
  if (previous.version !== 9 || next.version !== 9 || !integer(previous.revision) || previous.revision === Number.MAX_SAFE_INTEGER ||
      next.revision !== previous.revision + 1 || [...FIXED_KEYS, "targetSha", "activeAttempt", "windowRenewal"].some(key => !isDeepStrictEqual(previous[key], next[key])) ||
  ...
  const auditClock = { bootId: INCIDENT.bootId, now: previous.windowRenewal?.renewedAt };
  validateMaintenanceWindowRenewalState(previous, auditClock); validateMaintenanceWindowRenewalState(next, auditClock);
