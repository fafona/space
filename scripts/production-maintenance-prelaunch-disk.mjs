...
export const PRELAUNCH_MIN_FREE_BYTES = 5120n * 1024n * 1024n;
const APP = "/www/wwwroot/merchant-space";
const fail = () => { throw new Error("maintenance_prelaunch_disk_headroom_unverified"); };
export function validatePrelaunchDiskHeadroom(stat) {
  if (!stat || ["bsize", "blocks", "bfree", "bavail"].some(key => typeof stat[key] !== "bigint") ||
      ...
      stat.bavail * stat.bsize < PRELAUNCH_MIN_FREE_BYTES) fail();
  const used = stat.blocks - stat.bfree, usable = used + stat.bavail;
  if (usable <= 0n || (100n * used + usable - 1n) / usable >= 90n) fail();
  return true;
}
export function assertPrelaunchDiskHeadroom(state) {
  if (state?.appDir !== APP || ![9, 10].includes(state.version)) fail();
  for (const target of [APP, APP + ".releases"]) {
    ...
    validatePrelaunchDiskHeadroom(statfsSync(target, { bigint: true }));
