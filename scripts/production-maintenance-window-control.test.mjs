...
import { MAINTENANCE_WINDOW_RENEWAL_INCIDENT as BUDGET, MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION as AUTH,
 buildMaintenanceWindowRenewedState, encodeMaintenanceWindowRenewalEvidence, reconstructMaintenanceWindowRenewalPredecessor } from "./production-maintenance-window-renewal.mjs";
...
const NOW = Date.parse("2026-09-14T12:40:00Z"), TARGET = "a".repeat(40), ID = "1:2:3:4:5:1:0:33188";
...
function request(action, f) {
  ...
  if (["inspect-window-renewal", "renew-window"].includes(action)) args.push("--previous-target-sha", BUDGET.previousTargetSha);
  if (action === "renew-window") args.push("--window-renewal-evidence", encodeMaintenanceWindowRenewalEvidence(f.evidence));
...
function proofs(state) {
  ...
  if ([8, 9].includes(state.version)) proofs(state.budgetRecovery.predecessor.state);
...
function rehearsal(f, initial = f.state) {
  ...
  const validateState = state => {
    validateMaintenanceState(state, { ...state, action: state.version === 8 ? "inspect-window-renewal" : "check-held",
  ...
  const ops = {
    ...
    verifyIngress: record("ingress"), assertRuntimeStopped: record("originalStopped"), async assertWindowRenewalStopped(state) {
    ...
    async assertWindowRenewalGenerationsStopped(state) {
    ...
    async readWindowRenewalSnapshot() { const state = await load(), snapshot = baselines.get(state); return { ...snapshot, state }; },
    captureWindowRenewalBaseline: record("captureBaseline", f.baseline),
    async verifyWindowRenewalBaseline(state, baseline) { events.push("verifyBaseline"); assert.deepEqual(state.budgetRecovery, f.state.budgetRecovery); assert.deepEqual(baseline, f.baseline); return true; },
    readWindowRenewalSourceProof: record("source", { sourceDiffDigest: f.context.sourceDiffDigest }),
    readWindowRenewalMigrationProof: record("migration", f.context.migrationDigest),
    ...
    stopRuntime: async () => { assert.fail("v9 cleanup must never resume or target original O"); },
...
test("isolated unused v8 to v9 renewal, controller, typed runtime and real journal storage composition", { concurrency: false }, async t => {
 await withWindowRenewalFixture(t, async ({ predecessor: seed, fixture: rawFixture, hash }) => {
 const fixture = () => { const f = rawFixture(); return { ...f, baseline: f.context.stoppedBaseline }; };
  await t.test("new authority is exact while ordinary v8 remains expired at the real current clock", async () => {
    assert.equal(AUTH.authorizedAt, Date.parse("2026-09-14T12:31:04Z"));
    assert.equal(AUTH.expiresAt, Date.parse("2026-09-14T16:00:00Z"));
    assert.equal(Buffer.byteLength(JSON.stringify(seed)), 1570550);
    ...
    for (const action of ["check-held", "check-runtime-held", "runtime-handoff", "start-candidate", "end", "fail-held"]) {
      ...
      assert.throws(() => validateMaintenanceState(seed, ordinary, BUDGET.bootId, NOW), /maintenance_budget_recovery_invalid/);
    }
    for (const now of [AUTH.authorizedAt - 1, AUTH.expiresAt]) {
      const f = fixture(seed), r = rehearsal(f); r.setNow(now);
      await assert.rejects(runMaintenanceAction(request("inspect-window-renewal", f), r.ops));
  ...
  await t.test("inspection has zero writes/sends and returns the actual fixture baseline binding", async () => {
    const f = fixture(seed), r = rehearsal(f), report = await runMaintenanceAction(request("inspect-window-renewal", f), r.ops);
  ...
  await t.test("recovery uses one durable CAS and retains the whole consumed journal and all prior audits", async () => {
    const f = fixture(seed), r = rehearsal(f), next = buildMaintenanceWindowRenewedState(f.state, f.evidence, f.context);
    assert.equal((await runMaintenanceAction(request("renew-window", f), r.ops)).state, "held");
    assert.deepEqual(r.saved(), next); assert.deepEqual(reconstructMaintenanceWindowRenewalPredecessor(next, { bootId: BUDGET.bootId, now: NOW }), f.state); assert.equal(r.writes(), 1); assert.equal(r.events.filter(event => event === "recoveryCAS").length, 1);
    ...
    assert.equal(r.saved().version, 9); assert.equal(r.saved().revision, 33); assert.equal(r.saved().activeAttempt, 3);
    for (const key of ["recovery", "continuation", "buildRecovery", "deadlineExtension", "attemptRecovery", "secondAttemptRecovery", "budgetRecovery"])
    ...
    await assert.rejects(runMaintenanceAction(request("renew-window", f), r.ops));
  ...
  await t.test("wrong target, invalid evidence, stale history or failed host proof never writes", async () => {
    for (const alter of [
      ...
      (r, q) => { q.windowRenewalEvidence.historyDigest = "invalid"; },
      (r, q) => { q.windowRenewalEvidence.historyCheckedAt = NOW - 300001; },
      ...
      const f = fixture(seed), r = rehearsal(f), q = copy(request("renew-window", f)); alter(r, q);
  ...
  await t.test("second full proof round rechecks source, migration and real final deadline before CAS", async () => {
    for (const kind of ["source", "migration", "deadline"]) {
      ...
      if (kind === "source") r.ops.readWindowRenewalSourceProof = async () => ({ sourceDiffDigest: ++calls === 1 ? f.context.sourceDiffDigest : "e".repeat(64) });
      if (kind === "migration") r.ops.readWindowRenewalMigrationProof = async () => ++calls === 1 ? f.context.migrationDigest : "e".repeat(64);
      if (kind === "deadline") r.ops.assertDatabaseQuiet = async () => { if (++calls === 2) r.setNow(AUTH.expiresAt); };
      await assert.rejects(runMaintenanceAction(request("renew-window", f), r.ops)); assert.equal(r.writes(), 0);
    }
  });
  await t.test("file fsync and post-rename uncertainty produce no recovery success and no send/retry", async () => {
    for (const stage of ["fileSync", "renameAfter", "directorySync"]) {
      ...
      await assert.rejects(runMaintenanceAction(request("renew-window", f), r.ops), /storage_unconfirmed/);
      ...
      assert.equal(r.saved().version, stage === "fileSync" ? 8 : 9); assert.equal(r.fds.size, 0);
    }
  });
  await t.test("later v9 start persists its own attempt before one send while all three predecessors stay immutable", async () => {
    const f = fixture(seed), initial = buildMaintenanceWindowRenewedState(f.state, f.evidence, f.context), r = rehearsal(f, initial);
  ...
  await t.test("any O/T5/T6/T7 revival blocks inspect, recovery and launch before writes or sends", async () => {
    for (const name of ["O", "T5", "T6", "T7"]) {
      for (const action of ["inspect-window-renewal", "renew-window", "start-candidate"]) {
        const f = fixture(seed), initial = action === "start-candidate" ? buildMaintenanceWindowRenewedState(f.state, f.evidence, f.context) : f.state;
      ...
      const f = fixture(seed), r = rehearsal(f, buildMaintenanceWindowRenewedState(f.state, f.evidence, f.context));
  ...
  await t.test("every candidate snapshot, check, registration and handoff excludes O/T5/T6/T7 before reading renewed target", async () => {
    ...
    for (const action of ["snapshot-web", "snapshot-worker", "check-candidate", "register-candidate", "candidate-handoff", "end"]) {
      r.clearEvents(); const writes = r.writes();
      r.ops.assertWindowRenewalGenerationsStopped = async () => { r.events.push("historicalGenerationStopped"); throw new Error("synthetic historical revival"); };
  ...
  await t.test("historical revival after resume or before reopen blocks restore and stops only the new exact resumed proof", async () => {
    for (const rejectAt of [2, 3]) {
      ...
      r.ops.assertWindowRenewalGenerationsStopped = async () => { r.events.push("historicalGenerationStopped");
      ...
      r.ops.assertWindowRenewalStopped = async () => { r.events.push("bothStopped"); throw new Error("synthetic historical remains alive"); };
  ...
  await t.test("unknown ACK is reconciled by an observation and stopped, never launched again", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceWindowRenewedState(f.state, f.evidence, f.context)); r.setOutcome("unknown");
  ...
  await t.test("real storage refuses v9 journal reset, audit deletion and predecessor reuse", async () => {
    const f = fixture(seed), r = rehearsal(f, buildMaintenanceWindowRenewedState(f.state, f.evidence, f.context));
    await runMaintenanceAction(request("start-candidate", f), r.ops);
    ...
    for (const alter of [s => { s.launchJournal = null; s.launchDisk = null; s.candidate = null; s.phase = "held"; },
      s => { delete s.budgetRecovery; }, s => { s.version = 8; delete s.windowRenewal; }, s => { delete s.windowRenewal; }, s => { s.windowRenewal.authorization.expiresAt++; }, s => { s.activeAttempt++; },
