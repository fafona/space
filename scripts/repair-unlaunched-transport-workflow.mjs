...
import { TRANSPORT_REPAIR as P, readTransportRepairSource, validateTransportRepairAuthority } from "./repair-unlaunched-transport.mjs";

const fail = () => { throw new Error("transport_workflow_unverified"); };
...
export const TRANSPORT_CI_JOBS = Object.freeze(["Checkout Context PostgreSQL Acceptance", "Enterprise Browser Journeys", "Isolated Maintenance Ingress Acceptance",
...
export function validateTransportRun(value, expected) {
...
export function validateTransportCiJobs(value, sha) {
  if (value.total_count !== 10 || value.jobs?.length !== 10 || value.jobs.map(j => j.name).sort().join() !== TRANSPORT_CI_JOBS.join() ||
      value.jobs.some(j => j.status !== "completed" || j.conclusion !== "success" || j.head_sha !== sha)) fail();
}
export function validateTransportProvenance(bytes, results) {
  const subjects = results?.[0]?.verificationResult?.statement?.subject;
  if (!Array.isArray(results) || results.length !== 1 || subjects?.length !== 1 || subjects[0].name !== "transport-authority.json" ||
      subjects[0].digest?.sha256 !== hash(bytes)) fail();
}
export function validateTransportHistoryRun(r, currentRunId) {
  if (String(r.id) === currentRunId) return;
  if (r.status !== "completed" || !Number.isFinite(Date.parse(r.created_at))) fail();
  if (Date.parse(r.created_at) >= P.createdAt && ![P.failedRunId, P.backupRunId, P.readinessRunId, "35129584317", "35132895108"].includes(String(r.id))) fail();
}
export function transportMetadata(sha, runId, read = api, now = Date.now()) {
  ...
  const ci = (id, target) => {
    ...
    validateTransportRun(r, { id, sha: target, event: "push", file: "ci.yml", conclusion: "success" });
    validateTransportCiJobs(read(`actions/runs/${id}/jobs?per_page=100`), target);
  ...
  ci(mainCIrunId, sha); ci("35126695924", P.previousTargetSha);
  for (const [id, file, event, conclusion] of [[P.backupRunId, "database-backup.yml", "workflow_dispatch", "success"],
    ...
    const r = read("actions/runs/" + id);
    validateTransportRun(r, { id, sha: P.previousTargetSha, file, event, conclusion });
  ...
  if (current.id !== Number(runId) || current.head_sha !== sha || current.event !== "workflow_dispatch" || current.run_attempt !== 1 ||
      current.head_branch !== "main" || current.path !== ".github/workflows/repair-unlaunched-transport.yml" || current.status !== "in_progress" ||
  ...
  for (const file of ["deploy.yml", "production-maintenance.yml", "database-migrate.yml", "database-backup.yml", "ordinary-account-cutover-readiness.yml",
    "recover-maintenance-candidate.yml", "restore-reclosed-candidate.yml", "repair-unlaunched-transport.yml",
    ...
    for (let page = 1; page <= 20; page++) {
      ...
      for (const r of value.workflow_runs) { validateTransportHistoryRun(r, runId); }
  ...
  return validateTransportRepairAuthority({ version: 1, kind: "faolla-unlaunched-transport-repair", targetSha: sha, runId, runAttempt: 1,
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  try {
    ...
    if (action === "authority") {
      readTransportRepairSource(sha);
      const value = transportMetadata(sha, runId);
      writeFileSync(file, JSON.stringify(value), { flag: "wx", mode: 0o600 });
    } else if (action === "verify") {
      const bytes = readFileSync(file); if (bytes.length > 8192) fail();
      const value = validateTransportRepairAuthority(JSON.parse(bytes), sha, runId);
      validateTransportProvenance(bytes, JSON.parse(readFileSync(provenance)));
      const fresh = transportMetadata(sha, runId);
      if (fresh.historyDigest !== value.historyDigest || fresh.mainCIrunId !== value.mainCIrunId) fail();
    } else fail();
  } catch { process.stderr.write("transport_workflow_unverified\n"); process.exitCode = 1; }
}
