...
import { MAINTENANCE_WINDOW_RENEWAL_PRIOR_RUNS as SPECS,
  inspectMaintenanceWindowRenewalHistory, createMaintenanceWindowRenewalWorkflowEvidence } from "./production-maintenance-window-renewal-workflow.mjs";
import { validateMaintenanceWindowRenewalInspection, MAINTENANCE_WINDOW_RENEWAL_INCIDENT as INCIDENT,
  MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION as AUTH, MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION_DIGEST,
  encodeMaintenanceWindowRenewalEvidence, decodeMaintenanceWindowRenewalEvidence } from "./production-maintenance-window-renewal.mjs";
...
const NOW = Date.parse("2026-09-14T13:00:00Z"), TARGET = "a".repeat(40), SELF = "99999999999", CI = "99999999998";
...
function inspection() {
  ...
  return validateMaintenanceWindowRenewalInspection({ version: 1, state: "window-renewal-inspected",
    ...
    revision: 32, stateDigest: INCIDENT.stateDigest, createdAt: INCIDENT.createdAt, activeAttempt: 3,
    sourceDiffDigest: "f".repeat(64), migrationDigest: "a".repeat(64), predecessorJournalDigest: hash(null),
    stoppedBaselineDigest: hash(stoppedBaseline), stoppedBaseline, authorizationDigest: MAINTENANCE_WINDOW_RENEWAL_AUTHORIZATION_DIGEST,
...
const env = () => ({ GITHUB_REPOSITORY: "fafona/space", GITHUB_EVENT_NAME: "workflow_dispatch", GITHUB_REF: "refs/heads/main",
  ...
  EXPECTED_OLD_SHA: INCIDENT.expectedOldSha, MAINTENANCE_OPERATION_ID: INCIDENT.operationId, ACTION: "renew-window",
  CONFIRMATION: "RENEW_PRODUCTION_MAINTENANCE_UNTIL_20260914T160000Z" });
...
function fixture(mutate = (_key, value) => value) {
  ...
  for (const spec of SPECS) {
    ...
    if (spec.artifacts) db.set("artifacts:" + spec.run.id, { total_count: 6, artifacts: clone(spec.artifacts) });
  }
  const self = addRepo({ ...SPECS[1].run, id: Number(SELF), head_sha: TARGET, status: "in_progress", conclusion: null,
    created_at: "2026-09-14T12:40:00Z", run_started_at: "2026-09-14T12:40:00Z", updated_at: "2026-09-14T12:40:01Z" });
  const ci = addRepo({ ...SPECS[2].run, id: Number(CI), head_sha: TARGET,
    created_at: "2026-09-14T12:32:00Z", run_started_at: "2026-09-14T12:32:00Z", updated_at: "2026-09-14T12:39:00Z" });
  db.set("run:" + SELF, self);
  db.set("ci", { total_count: 1, workflow_runs: [ci] });
  db.set("jobs:" + CI, { total_count: 10, jobs: SPECS[2].jobs.map((job, index) => ({ ...clone(job), id: 9000 + index,
    run_id: Number(CI), head_sha: TARGET, started_at: "2026-09-14T12:32:05Z", completed_at: "2026-09-14T12:38:59Z" })) });
  db.set("main", { sha: TARGET });
  for (const [index, file] of FILES.entries()) {
    const runs = index === 0 ? [db.get("run:" + SPECS[0].run.id)] : index === 4 ? [self, db.get("run:" + SPECS[1].run.id)] : [];
...
const history = mutate => inspectMaintenanceWindowRenewalHistory(inspection(), fixture(mutate).api, NOW, SELF, () => NOW);
const evidence = (mutate, override = {}) => createMaintenanceWindowRenewalWorkflowEvidence(inspection(), { ...env(), ...override }, fixture(mutate).api, NOW, () => NOW);

test("renewal chains exact prior recovery, full CI and failed B8 without a valid-backup claim", async () => {
  const result = await history();
  assert.equal(result.records.length, 3);
  assert.equal(result.records[0].jobs[0].steps.length, 30);
  assert.equal(result.records[1].jobs[0].steps.length, 43);
  assert.equal(result.records[2].jobs.length, 10);
  assert.equal(result.cutoff, Date.parse("2026-09-14T09:04:41Z"));
  assert.equal(result.predecessorStateDigest, INCIDENT.stateDigest);
  assert.equal(result.failedBackupPurpose, "historical-failure-not-valid-backup");
  assert.equal(result.records[0].artifacts.length, 6);
  assert.ok(!result.records[0].artifacts.some(value => value.name.includes("maintenance-backup-binding")));
  const e = await evidence();
  assert.equal(e.activeAttempt, 3);
  assert.equal(e.windowRenewalRunId, SELF);
  assert.deepEqual(decodeMaintenanceWindowRenewalEvidence(encodeMaintenanceWindowRenewalEvidence(e)), e);
  assert.equal(SPECS[0].run.id, 34826545330);
...
test("every fixed run event, head, identity, attempt, terminal result and timestamp is exact", async () => {
  for (const spec of SPECS) for (const patch of [
    ...
    { created_at: "2026-09-14T09:04:42Z" }, { run_started_at: "2026-09-14T09:04:42Z" },
    { updated_at: "2026-09-14T12:00:00Z" }, { path: ".github/workflows/other.yml" },
  ]) {
  ...
  for (const spec of SPECS) {
    ...
    for (let index = 0; index < spec.jobs[0].steps.length; index++) {
      for (const change of [s => s.number++, s => s.name += " changed", s => s.status = "in_progress",
        ...
        s => s.started_at = "2026-09-14T12:00:00Z", s => s.completed_at = "2026-09-14T12:00:00Z"])
...
test("B8 artifact inventory cannot gain a binding or change an identity, digest, expiry or byte count", async () => {
  for (const change of [v => v.total_count++, v => v.artifacts.pop(),
    v => v.artifacts.push({ name: "faolla-maintenance-backup-binding-34826545330-1" }),
    ...
    await assert.rejects(history((key, value) => { if (key === "artifacts:34826545330") change(value); return value; }));
});

test("all five histories reject unknown same-second, later, old retried or nonterminal activity", async () => {
  for (const [index, file] of FILES.entries()) for (const time of ["2026-09-14T09:04:41Z", "2026-09-14T12:00:00Z"]) {
...
test("pagination counts, duplicate IDs, missing pages and the complete second observation fail closed", async () => {
  ...
  await assert.rejects(history((key, value, count) => {
    if (key === FILES[0] + ":1" && count === 2) { value.workflow_runs[0].updated_at = "2026-09-14T12:00:00Z"; } return value;
  }));
  for (const keyName of ["run:34826545330", "jobs:34825984301", "jobs:34824742841", "artifacts:34826545330"])
    await assert.rejects(history((key, value, count) => {
      if (key === keyName && count === 2) { if (value.jobs) value.jobs[0].id++; else if (value.artifacts) value.artifacts[0].id++; else value.run_attempt++; } return value;
    }));
  const f = fixture();
  ...
  assert.equal((await inspectMaintenanceWindowRenewalHistory(inspection(), f.api, NOW, SELF, () => NOW)).rows.length, 104);
  f.db.get(FILES[1] + ":2").total_count = 102;
  await assert.rejects(inspectMaintenanceWindowRenewalHistory(inspection(), f.api, NOW, SELF, () => NOW));
});

test("only exact active renewal workflow env may authorize the same unused attempt", async () => {
  for (const patch of [{ GITHUB_REPOSITORY: "other/repo" }, { GITHUB_EVENT_NAME: "push" }, { GITHUB_REF: "refs/heads/feature" },
    { GITHUB_RUN_ATTEMPT: "2" }, { GITHUB_RUN_ID: INCIDENT.failedBackupRunId }, { GITHUB_SHA: INCIDENT.previousTargetSha },
  ...
  for (const patch of [{ event: "schedule" }, { run_attempt: 2 }, { head_sha: INCIDENT.previousTargetSha },
    { status: "completed", conclusion: "success" }, { created_at: "2026-09-14T12:30:00Z" }])
...
test("current main must be stable with all ten exact successful CI jobs and prior completed CI", async () => {
  ...
  for (const change of [v => v.total_count--, v => v.jobs.pop(), v => v.jobs[0].name = "fake",
    ...
    v => v.jobs[0].completed_at = "2026-09-14T12:40:00Z"])
...
test("actual authorization bounds and five-minute baseline never use an old clock", async () => {
  for (const now of [AUTH.authorizedAt - 1, AUTH.expiresAt, AUTH.expiresAt + 1])
    await assert.rejects(createMaintenanceWindowRenewalWorkflowEvidence(inspection(), env(), fixture().api, now));
  await assert.rejects(createMaintenanceWindowRenewalWorkflowEvidence(inspection(), env(), fixture().api, NOW + 300001));
  await assert.rejects(createMaintenanceWindowRenewalWorkflowEvidence(inspection(), env(), fixture().api, NOW - 1001));
});
...
test("current-run updates use each actual observation clock without moving the original history time", async () => {
  ...
  const value = await createMaintenanceWindowRenewalWorkflowEvidence(inspection(), env(), f.api, NOW, () => NOW + elapsed);
  ...
  const future = fixture((key, value) => {
    if (key === FILES[4] + ":1") value.workflow_runs.find(run => String(run.id) === SELF).updated_at = "2026-09-14T13:00:02Z";
    return value;
  });
  await assert.rejects(createMaintenanceWindowRenewalWorkflowEvidence(inspection(), env(), future.api, NOW, () => NOW + 1000));
});

test("CLI remains GET-only, bounded, masks before output, and never downloads B8 payloads or raw logs", () => {
  const source = readFileSync(new URL("./production-maintenance-window-renewal-workflow.mjs", import.meta.url), "utf8");
  ...
  assert.match(source, /maintenance_window_renewal_workflow_unverified/);
});
