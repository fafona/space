...
## Prepare temporary inputs locally

Normalize the email exactly as `trim().toLowerCase()`, SHA-256 it locally, and pipe the lowercase 64-hex digest into the temporary secret without putting the value in a command argument:
...
The personal ID must be exactly eight digits in the personal range (`50010105` through `59999999`). Generate an ephemeral RSA key pair on the operator workstation; keep the private key outside the repository and runner:
...
- `operator_public_key_pem`: the public PEM only.
...
3. Delete the two temporary candidate secrets, the downloaded encrypted artifact, and the ephemeral local private/public key using the workstation's approved secure-delete or credential-handling procedure.
