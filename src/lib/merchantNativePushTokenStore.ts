import {
  MERCHANT_NATIVE_PUSH_TOKENS_PAGE_SLUG,
  createEmptyMerchantNativePushTokenPayload,
  normalizeMerchantNativePushTokenPayload,
  type MerchantNativePushTokenPayload,
} from "@/lib/merchantNativePushTokens";

type StoreErrorLike = { message?: string } | null;

type NativePushQueryBuilder = PromiseLike<{ data?: unknown; error: StoreErrorLike }> & {
  select: (columns: string) => NativePushQueryBuilder;
  update: (payload: Record<string, unknown>) => NativePushQueryBuilder;
  insert: (payload: Record<string, unknown>) => Promise<{ data?: unknown; error: StoreErrorLike }>;
  is: (column: string, value: unknown) => NativePushQueryBuilder;
  eq: (column: string, value: unknown) => NativePushQueryBuilder;
  limit: (value: number) => NativePushQueryBuilder;
  maybeSingle: () => Promise<{ data?: unknown; error: StoreErrorLike }>;
};

export type MerchantNativePushTokenStoreClient = {
  from: (table: string) => NativePushQueryBuilder;
};

const MERCHANT_NATIVE_PUSH_TOKEN_CACHE_TTL_MS = 8_000;
let merchantNativePushTokenCache:
  | {
      expiresAt: number;
      value: MerchantNativePushTokenPayload;
    }
  | null = null;
...
export async function loadStoredMerchantNativePushTokens(
  supabase: MerchantNativePushTokenStoreClient,
): Promise<MerchantNativePushTokenPayload> {
  if (merchantNativePushTokenCache && merchantNativePushTokenCache.expiresAt > Date.now()) {
    return merchantNativePushTokenCache.value;
  }
  ...
  const initialQuery = await supabase
    ...
    .select("blocks")
    .is("merchant_id", null)
    .eq("slug", MERCHANT_NATIVE_PUSH_TOKENS_PAGE_SLUG)
    .limit(1)
    .maybeSingle();
  ...
  if (error) {
    ...
    if (isMissingMerchantIdColumn(message)) {
      ...
      return createEmptyMerchantNativePushTokenPayload();
    } else {
      return createEmptyMerchantNativePushTokenPayload();
    }
  }

  if (error) return createEmptyMerchantNativePushTokenPayload();
  const payload = normalizeMerchantNativePushTokenPayload(data?.blocks);
  merchantNativePushTokenCache = {
    expiresAt: Date.now() + MERCHANT_NATIVE_PUSH_TOKEN_CACHE_TTL_MS,
    value: payload,
  };
  return payload;
}

export async function saveStoredMerchantNativePushTokens(
  supabase: MerchantNativePushTokenStoreClient,
  ...
  const updatePayload = async (body: Record<string, unknown>) => {
    ...
    if (existing.supportsSlug) {
      const inserted = await supabase.from("pages").insert({
        ...body,
        slug: MERCHANT_NATIVE_PUSH_TOKENS_PAGE_SLUG,
