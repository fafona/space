...
export function resolveBusinessCardContactWebsite(settings: BusinessCardDestinationSettings | undefined, assignedWebsite: string, sharedWebsite?: string): string {
  // An explicit saved address (including an intentional reset to blank) takes
  // precedence over a legacy share manifest regenerated by an older client.
  if (typeof settings?.websiteAddress === "string") return resolveBusinessCardWebsiteAddress(settings, assignedWebsite);
  return sharedWebsite?.trim() ? normalizeBusinessCardWebsiteAddress(sharedWebsite) : assignedWebsite;
}
