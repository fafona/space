...
function getFilterChipClass(filter: MerchantBookingFilter, key: MerchantBookingFilter) {
  ...
  if (key === "no_show") {
    return isActive
      ? "border-2 border-rose-500 bg-rose-500 text-white shadow-[0_8px_18px_rgba(244,63,94,0.24)]"
...
export default function MerchantBookingManagerDialog({
  ...
  const attentionScopedRecords = useMemo(
    () =>
      historyFilteredRecords.filter((item) => {
        if (filter === "all") return selectedStatuses.includes(item.status);
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {MERCHANT_BOOKING_STATUSES.map((status) => (
                <button
                  key={status}
                  type="button"
                  className={`inline-flex h-10 items-center justify-center rounded-full px-3 py-2 text-sm font-semibold transition-colors ${getFilterChipClass(filter, status)}`}
                  onClick={() => {
                    setFilter(status);
                    ...
                    {selectionMode ? (
                      <label className="mt-4 inline-flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full border border-slate-300 bg-white shadow-sm transition hover:border-slate-500">
                        <input
                          ...
                          aria-label={locale.startsWith("es") ? "Seleccionar reserva" : "Select booking"}
                        />
                        <span
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="flex min-w-0 flex-1 flex-wrap items-start gap-x-5 gap-y-2">
                        ...
                        <div className="grid min-w-[580px] grid-cols-[5.75rem_2rem_minmax(12rem,1fr)_2rem_9rem_2rem] items-center gap-2 text-[13px] leading-5 text-slate-700">
                          ...
                          <span className="min-w-0 truncate text-right">{record.phone || "-"}</span>
                          {record.phone ? (
                            <a
